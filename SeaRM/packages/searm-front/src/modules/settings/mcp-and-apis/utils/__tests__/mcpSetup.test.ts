...
const mcpServerUrl = 'https://api.searm.com/mcp';
...
describe('buildMcpServerUrl', () => {
  it('appends /mcp after trimming trailing slashes', () => {
    expect(buildMcpServerUrl('https://api.searm.com///')).toBe(mcpServerUrl);
  });
});
...
describe('isHttpsUrl', () => {
  it('returns true only for valid HTTPS urls', () => {
    ...
    expect(isHttpsUrl('http://api.searm.com/mcp')).toBe(false);
...
describe('buildClaudeInstallLink', () => {
  it('builds a Claude custom connector link', () => {
    ...
    expect(params.get('connectorName')).toBe('SeaRM');
...
describe('buildCursorInstallLink', () => {
  it('base64-encodes the remote MCP config', () => {
    ...
    expect(new URLSearchParams(link.split('?')[1]).get('name')).toBe('searm');
...
describe('buildGooseInstallLink', () => {
  it('builds a Goose streamable HTTP extension link', () => {
    ...
    expect(params.get('id')).toBe('searm');
    expect(params.get('name')).toBe('SeaRM');
