...
describe('getImageIdentifierFieldValue', () => {
  ...
  describe('LINKS', () => {
    it('returns the favicon url when requests to searm-icons are allowed', () => {
      expect(
        getImageIdentifierFieldValue(
          buildRecord({ primaryLinkUrl: 'searm.com' }),
          buildFieldMetadataItem(FieldMetadataType.LINKS),
          true,
        ),
      ).toBe('https://searm-icons.com/searm.com');
    });

    it('returns null when requests to searm-icons are not allowed', () => {
      expect(
        getImageIdentifierFieldValue(
          buildRecord({ primaryLinkUrl: 'searm.com' }),
          buildFieldMetadataItem(FieldMetadataType.LINKS),
