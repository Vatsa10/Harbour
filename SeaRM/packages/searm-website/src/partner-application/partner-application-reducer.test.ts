...
describe('partnerApplicationReducer', () => {
  ...
  it('starts at stepIndex 0 with empty fields', () => {
    ...
    expect(INITIAL_PARTNER_APPLICATION_STATE.searmExperience).toEqual([]);
  });
  ...
  it('TOGGLE_EXPERIENCE adds and then removes a milestone', () => {
    ...
    expect(added.searmExperience).toEqual(['CUSTOM_APPS']);
    const removed = partnerApplicationReducer(added, {
      type: 'TOGGLE_EXPERIENCE',
      value: 'CUSTOM_APPS',
    });
    expect(removed.searmExperience).toEqual([]);
  });
  ...
  it('GO_NEXT on Experience gates on milestones, narrative length, and proof URL', () => {
    ...
    expect(blocked.fieldErrors.searmExperience).toBe('required');
    expect(blocked.fieldErrors.searmExperienceNotes).toBe('required');
    expect(blocked.fieldErrors.searmExperienceProofLink).toBe('required');

    const shortNotes = partnerApplicationReducer(
      {
        ...onExperience,
        searmExperience: ['WORKFLOWS'],
        searmExperienceNotes: 'Too short.',
        searmExperienceProofLink: 'https://www.loom.com/share/example',
      },
      { type: 'GO_NEXT' },
    );
    expect(shortNotes.stepIndex).toBe(3);
    expect(shortNotes.fieldErrors.searmExperienceNotes).toBe('too_short');
