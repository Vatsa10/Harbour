...
export function partnerApplicationReducer(
  ...
  switch (action.type) {
    ...
    case 'TOGGLE_EXPERIENCE': {
      const next = state.searmExperience.includes(action.value)
        ? state.searmExperience.filter((value) => value !== action.value)
        : [...state.searmExperience, action.value];
      return {
        ...state,
        searmExperience: next,
        fieldErrors: dropError(state.fieldErrors, 'searmExperience'),
      };
    }
    case 'SET_SKILLS':
