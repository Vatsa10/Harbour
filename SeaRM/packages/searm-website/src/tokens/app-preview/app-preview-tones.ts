...
// straight from searm-ui: TAG (tag color3/11), SIDEBAR (TintedIconTile
...
import { THEME_LIGHT } from 'searm-ui/theme';

type ToneSurface = { background: string; border: string; color: string };
type TonePair = { background: string; color: string };

// Our tone names map 1:1 onto searm-ui's tag colors, except `teal` is the
// product's `turquoise`.
...
// searm-ui colors are `color(display-p3 r g b)` strings with 0–1 channels;
...
// The Ask-AI panel washes map onto searm-ui's transparent ramp + primary
...
// searm-front's graph-color registry assigns chart series from the color8
