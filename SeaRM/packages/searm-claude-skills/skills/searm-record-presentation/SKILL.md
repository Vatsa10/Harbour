---
name: searm-record-presentation
description: "Retrieve and present SeaRM CRM records as readable summaries or tables, using the connected SeaRM MCP server to discover fields, fetch relevant data, format dates and values, build record links, and avoid raw API output."
---

# SeaRM Record Presentation
...
- If no SeaRM MCP tools are available, say that no callable SeaRM MCP server is available in the current thread and ask the user to connect or expose the intended workspace.
...
- For absolute links, combine the workspace origin with that path, for example `https://example.searm.com/object/person/record-id`.
...
| :--- | :--- | ---: | :--- |
| [Acme renewal](https://example.searm.com/object/opportunity/record-id-1) | Negotiation | EUR 12,450 | May 5, 2026, 11:43 AM |
| [Globex expansion](https://example.searm.com/object/opportunity/record-id-2) | Discovery | EUR 8,000 | May 4, 2026, 4:10 PM |
```
