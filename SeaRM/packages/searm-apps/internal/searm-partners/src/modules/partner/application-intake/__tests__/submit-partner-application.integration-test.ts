...
describe('submit-partner-application handler — upsert', () => {
    ...
    const result = await handler(
      authedEvent(
        baseInput({
          ...
          partnerScope: ['ADVISORY', 'SOLUTIONING'],
          applicationNotes: 'Workspace https://app.searm.com/ws · refs: Acme',
        }),
      ),
    );
    expect(result.ok).toBe(true);
  ...
  it('persists searm experience fields on create', async () => {
    const searmExperienceNotes =
      'Implemented SeaRM for a logistics client: custom apps for shipment intake, ' +
    ...
    const result = await handler(
      authedEvent(
        baseInput({
          ...
          searmExperience: ['CUSTOM_APPS', 'DATA_MODELS', 'WORKFLOWS'],
          searmExperienceNotes,
          searmExperienceProofLink: 'https://www.loom.com/share/experience-create',
        }),
      ),
    );
    expect(result.ok).toBe(true);
    ...
    const fetched = await client.query({
      partner: {
        __args: { filter: { id: { eq: result.partnerId } } },
        searmExperience: true,
        searmExperienceNotes: true,
        searmExperienceProofLink: { primaryLinkUrl: true },
      },
    });
    const node = fetched.partner;
    expect(node?.searmExperience).toEqual(
      expect.arrayContaining(['CUSTOM_APPS', 'DATA_MODELS', 'WORKFLOWS']),
    );
    expect(node?.searmExperienceNotes).toBe(searmExperienceNotes);
    expect(node?.searmExperienceProofLink?.primaryLinkUrl).toBe(
      'https://www.loom.com/share/experience-create',
    );
  });

  it('persists searm experience fields on update for the same email', async () => {
    ...
    const second = await handler(
      authedEvent(
        baseInput({
          ...
          searmExperience: ['FRONT_COMPONENTS', 'WORKFLOWS'],
          searmExperienceNotes,
          searmExperienceProofLink: 'https://github.com/example/searm-case-study',
        }),
      ),
    );
    expect(second.ok).toBe(true);
    ...
    expect(node?.searmExperienceNotes).toBe(searmExperienceNotes);
    expect(node?.searmExperienceProofLink?.primaryLinkUrl).toBe(
      'https://github.com/example/searm-case-study',
    );
  });
