...
import { type RoutePayload } from 'searm-sdk/define';
...
describe('save-my-partner-profile', () => {
  ...
  const originalToken = process.env.SEARM_APP_ACCESS_TOKEN;
  ...
  afterEach(async () => {
    if (originalToken === undefined) delete process.env.SEARM_APP_ACCESS_TOKEN;
    else process.env.SEARM_APP_ACCESS_TOKEN = originalToken;
  ...
  it('rejects an invalid body before writing anything', async () => {
    ...
    process.env.SEARM_APP_ACCESS_TOKEN = makeToken({ userId: member.userId, userWorkspaceId });
