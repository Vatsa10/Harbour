...
describe('resolvePartnerFromRequest guards (no network)', () => {
  const original = process.env.SEARM_APP_ACCESS_TOKEN;
  afterEach(() => {
    if (original === undefined) delete process.env.SEARM_APP_ACCESS_TOKEN;
    else process.env.SEARM_APP_ACCESS_TOKEN = original;
  });
