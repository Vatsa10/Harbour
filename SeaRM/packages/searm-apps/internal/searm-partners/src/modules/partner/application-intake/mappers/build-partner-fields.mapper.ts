import { type CoreSchema } from 'searm-client-sdk/core';
...
export type PartnerFieldsForUpsert = {
  ...
  searmExperience?: CoreSchema.PartnerSearmExperienceEnum[];
  searmExperienceNotes?: string;
  searmExperienceProofLink?: { primaryLinkUrl: string };
};
...
export function buildPartnerFields(input: SubmitPartnerApplicationInput): PartnerFieldsForUpsert {
  ...
  if (input.searmExperience !== undefined && input.searmExperience.length > 0) {
    fields.searmExperience =
      input.searmExperience as CoreSchema.PartnerSearmExperienceEnum[];
  }
  if (isNonEmptyString(input.searmExperienceNotes)) {
    fields.searmExperienceNotes = input.searmExperienceNotes.trim();
  }
  if (isNonEmptyString(input.searmExperienceProofLink)) {
    fields.searmExperienceProofLink = {
      primaryLinkUrl: input.searmExperienceProofLink.trim(),
    };
  }
  return fields;
}
