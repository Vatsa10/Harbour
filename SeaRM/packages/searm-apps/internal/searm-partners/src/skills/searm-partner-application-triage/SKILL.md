---
name: searm-partner-application-triage
...
trigger: /searm-partner-application-triage
---

# searm-partner-application-triage

Rank `APPLICATION`-stage partners by the value they would *add* — geographies and
languages we don't yet cover, plus proof of real SeaRM work — and hand back a short
...
Needs `~/.searm/credentials.env` (same file the other partner skills use):
...
The key lives in `packages/searm-apps/internal/searm-partners/.env.prod` (gitignored) —
copy it to `~/.searm/credentials.env` on first setup.
...
SeaRM work" proof signal in the notes, scores, and prints ranked JSON. It does not call an
...
- **Rescue the motivated-but-unobvious.** Someone whose checkboxes are thin but whose notes
  show real intent, a live SeaRM instance, named customers, or a thoughtful pitch is a
