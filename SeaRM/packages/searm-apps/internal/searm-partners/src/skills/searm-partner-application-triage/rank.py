#!/usr/bin/env python3
"""Deterministic ranker for the searm-partner-application-triage skill.

Ranks APPLICATION-stage partners by NET-NEW value vs the current VALIDATED set:
geography and language we don't yet cover (weighted high), plus a "real SeaRM
...
Reads creds from ~/.searm/credentials.env. Emits ranked JSON to stdout.
...
# "real SeaRM work" signals in the free-text notes (the high-confidence axis).
PROOF_WORKSPACE = re.compile(r"https?://[^\s]*(searm|crm)[^\s]*", re.I)
...
PROOF_MIGRATION = re.compile(r"(switch\w*|migrat\w*|moved|replac\w*|our crm|own crm|we use searm|dogfood|managed service)", re.I)

CRED_PATH = os.path.expanduser("~/.searm/credentials.env")


def load_creds():
    if not os.path.exists(CRED_PATH):
        sys.exit(f"Missing {CRED_PATH}. Copy the partners key from the app's "
                 ".env.prod into it (SEARM_PARTNERS_API_URL / _API_KEY).")
    env = {}
    with open(CRED_PATH) as fh:
        for line in fh:
            ...
            if line and not line.startswith("#") and "=" in line:
                k, v = line.split("=", 1)
                env[k.strip()] = v.strip()
    url = env.get("SEARM_PARTNERS_API_URL")
    key = env.get("SEARM_PARTNERS_API_KEY")
    if not url or not key:
        sys.exit("credentials.env is missing SEARM_PARTNERS_API_URL or SEARM_PARTNERS_API_KEY.")
