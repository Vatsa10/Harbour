---
name: searm-partner-match
description: Match a qualified lead to SeaRM partners and generate all intro emails in one run. Use when a lead folder exists and you need to find the right implementation partner, either after running searm-partner-design-doc or standalone when partner-match-criteria.md is already present. Chains back into searm-lead-intro-call-summary and searm-partner-design-doc if the criteria file is missing.
trigger: /searm-partner-match
---

# searm-partner-match

Match a qualified lead to SeaRM partners and generate all intro emails in one run.

Chains after `searm-lead-intro-call-summary` → `searm-partner-design-doc`. Can be run standalone if `partner-match-criteria.md` already exists in the lead folder.

---

## Credentials

Before running, `~/.searm/credentials.env` must exist with:

```env
SEARM_PARTNERS_API_URL=https://partners.searm.com
SEARM_PARTNERS_API_KEY=<your key>

# Only needed if the input is a Fireflies link/ID
FIREFLIES_API_KEY=<your key>
```

The partners API key lives in `packages/searm-apps/internal/searm-partners/.env.prod` (gitignored) — copy it to `~/.searm/credentials.env` on first setup. The skill reads `credentials.env` at startup and stops cleanly if a required key is missing.
...
Read `~/.searm/credentials.env`. Verify `SEARM_PARTNERS_API_URL` and `SEARM_PARTNERS_API_KEY` are present. If not, stop and tell the user exactly which key is missing and where to add it.
...
- Fireflies input → fetch transcript via Fireflies API (requires `FIREFLIES_API_KEY`) → `/searm-lead-intro-call-summary` → `/searm-partner-design-doc`
- Text transcript → `/searm-lead-intro-call-summary` → `/searm-partner-design-doc`
- Existing summary → `/searm-partner-design-doc`

`partner-match-criteria.md` is produced by `/searm-partner-design-doc` as Step 8.
...
Paginate until `pageInfo.hasNextPage` is false, passing `pageInfo.endCursor` as `$after` each iteration. Endpoint: `$SEARM_PARTNERS_API_URL/graphql`.
...
- **Subject**: `[SeaRM] Partner opportunity: [Client], [one-line project description]`
...
- **CC**: client email
- **Subject**: `[SeaRM] [Client] x [Partner]: CRM project`
...
This skill is the third step in the SeaRM partner pipeline:

```
/searm-lead-intro-call-summary → /searm-partner-design-doc → /searm-partner-match
```

The `partner-match-criteria.md` produced by `/searm-partner-design-doc` (Step 8) is this skill's primary input. Keep the criteria file updated as you learn more about the lead — it feeds both this skill and the eventual in-product matching logic.
