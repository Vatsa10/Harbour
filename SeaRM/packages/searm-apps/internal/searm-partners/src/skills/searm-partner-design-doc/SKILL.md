---
name: searm-partner-design-doc
description: Use when turning a qualified SeaRM lead — a call-summary brief plus any client braindump/docs — into a partner brief a partner can scope and quote from. Trigger when pointed at a lead folder (e.g. partners-experience/<LEAD>/) and asked to "draft a design doc", "create a partner brief", "translate this into SeaRM terms", "scope this for a partner", or "prep the partner handoff". Default output is zero-inference (only what the client said; empty sections get a placeholder). Pass --full for inference mode. Chains after searm-lead-intro-call-summary.
trigger: /searm-partner-design-doc
---

# searm-partner-design-doc

Turn a qualified lead's materials into a **design doc**: a translation of the customer's needs into **SeaRM terms** that an implementation **partner reads to scope and quote** the work.
...
- A lead folder (e.g. `partners-experience/<LEAD>/`) containing a `searm-lead-intro-call-summary` output plus any braindump / docs / notes.
- If there is a raw transcript but no brief, run **searm-lead-intro-call-summary** first — this skill chains after it.
...
4. **Verify load-bearing claims live** — use **WebFetch** against the SeaRM doc map in the doctrine's Verification section before asserting any capability. Build the §11 appendix as you go.
...
6. **Resolve ❓ with the operator** — after a full v1 draft, use **AskUserQuestion** to ask the SeaRM team member the unknowns a SeaRM insider can answer; leave customer-facing unknowns as ❓. **If running autonomously** (no operator — a subagent/batch run), skip the questions and leave every unknown as ❓ in the body and §11.
...
- Chain: **searm-lead-intro-call-summary → searm-partner-design-doc**; the output feeds the opportunity→partner handover (`designDocStatus` / `designDocUrl`).
