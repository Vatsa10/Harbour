# Design-doc doctrine: translating a lead into SeaRM terms

**Portable doctrine.** This file is tool-agnostic. It defines *what* a SeaRM partner design doc is, its structure, the rules, and how to verify it. Two consumers use it: the Claude Code skill `searm-partner-design-doc` (see `SKILL.md` in this folder), and, in a later phase, the `content` of a SeaRM `defineSkill` driving an in-product agent. Keep it free of any one tool's mechanics (no file paths, no tool names).

## Purpose

Produce a **design doc** that translates a qualified lead's needs into **SeaRM terms** (data model, permissions, integrations, hosting, plus whatever else the client named) so an implementation **partner can scope and quote** the work.

**Core principle:** identify *all* the work with **no blindspots**, **ground every claim** in the source or in live SeaRM docs, and **present options rather than prescribe**. The partner quotes off this doc, so a confident-but-wrong capability claim or a hidden requirement is the worst failure.
...
- **Partner-facing voice.** Say **"SeaRM," never first person** ("we / our / ours"). **No local file paths** in the output: cite shareable `docs.searm.com` URLs only. (Customer quotes that contain "we/our" are fine: they are quotes.)
...
- **Verify before asserting capability.** Any "SeaRM can / can't / has / lacks X" that moves the quote must be verified live (see Verification). **Undocumented ≠ impossible.**
...
Any statement of the form **"SeaRM can / can't / has / lacks X"** that changes the partner's quote MUST be verified **live** before it is stated as fact. Model training is stale on a fast-moving product; the worst failure is a confident, authoritative-sounding claim that is wrong.

**Source hierarchy (what to trust, in order):**
1. **Live docs** (`docs.searm.com`): primary truth. For the highest-stakes claims, read the page's primary text rather than trust a summary.
2. **Established SeaRM SDK build patterns** (hands-on): build-level facts the customer docs omit (no formula fields; custom objects auto-get attachments/notes/tasks/timeline; two-file relations).
3. **The SeaRM operator** (a SeaRM team member): best for "is it shipped / internal / undocumented."
...
**References appendix format:** a `Claim (§) | Verified against` table, then an explicit "**Could not be confirmed in public docs (check with SeaRM directly):**" list. Unverified items stay **❓ open** in the body too, never silently promoted to fact. Cite the **human-readable (non-`.md`) URL** here: the `.md` twin is for *your* fetch, not for the partner (a `.md` link renders as raw markdown in a browser).

**Where to verify (SeaRM doc map):** docs base = `https://docs.searm.com/`. Fetch **`<path>.md`** for the clean markdown twin (the form to prefer). Paths:
...
- Pricing & plans: `https://searm.com/pricing` (**marketing page, no `.md`**; fetch as HTML).
...
| Mistake | Reality / fix |
|---|---|
| "SeaRM isn't a BI tool" / "can't do row-level" | Stale training. SeaRM has Dashboards; row-level is on the Organization plan. **Verify live.** |
...
| First-person "not ours" in a partner doc | Say "SeaRM." The doc is forwarded verbatim. |
| Local file paths in the output | Mean nothing to a partner. Cite `docs.searm.com` only. |
