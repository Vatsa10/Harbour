...
async function main() {
  const baseUrl = requireEnv('SEARM_PARTNERS_API_URL').replace(/\/$/, '');
  const apiKey = requireEnv('SEARM_PARTNERS_API_KEY');
  ...
  if (missingLocks.length > 0) {
    console.error(
      `\n[rls:configure] DRIFT DETECTED: ${missingLocks.length} Opportunity field(s) ` +
        ...
        `  yarn searm apply -r <remote>\n\n` +
        `(e.g. \`yarn searm apply\` for local, ` +
        `\`yarn searm apply -r partner-searm-com\` for prod)\n`,
    );
    process.exitCode = 1;
    return;
  }
