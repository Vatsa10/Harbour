...
//
// SeaRM SOFT-deletes (sets deletedAt); the row stays in the DB and keeps holding
...
async function main() {
  const url = requireEnv('SEARM_PARTNERS_API_URL');
  const key = requireEnv('SEARM_PARTNERS_API_KEY');
