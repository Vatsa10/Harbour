# Architecture — searm-partners

Vertical-slice modules, one folder per domain. Adapted from `Vatsa10/Harbour-eng`
(the largest SeaRM SDK app), which migrated flat → modular as it grew. Repo-wide
...
`<feature>/` level — `modules/<domain>/<feature>/<primitive>/` (the searm-eng standard,
