...
`CallRecording` records linked to the matching `CalendarEvent` in your SeaRM
...
1. **Provider-native event ID** — Fireflies' `calendar_id` / `cal_id` is
   ...
   events synced into SeaRM from Google Calendar (including individual
...
Both identifiers are populated by SeaRM's calendar drivers on every synced
...
## What gets added to your SeaRM workspace
...
1. Open **Settings → Applications** in your SeaRM workspace.
...
> webhook, your SeaRM admin needs to follow the
...
- **Per-user Fireflies accounts.** All transcripts come through one
  ...
  connections require extending SeaRM's connection provider system and are
  planned once we have evidence that workspace-shared is too coarse.
- **Editing transcripts or summaries in SeaRM.** The CallRecording fields
...
| Webhook returns `Invalid webhook signature` | `FIREFLIES_WEBHOOK_SECRET` mismatch between Fireflies and SeaRM | Re-copy the signing secret from the Fireflies webhook configuration and paste it into the SeaRM app settings |
| CallRecording is created but has no linked CalendarEvent | The meeting was never on a synced calendar in SeaRM, or the workspace has no Google/Outlook/CalDAV calendar connection set up | Connect the relevant calendar provider in **Settings → Accounts** so the calendar event lands in SeaRM with `eventExternalId` and `iCalUid` populated, then re-run **Sync Fireflies Call** for that transcript. Manually-created CalendarEvents are intentionally not matched in v1 |
...
| **List / Search** tools return `count: 0` for a contact you've definitely talked to | Email mismatch — Fireflies stores the address as the participant joined the meeting with, which may differ from the contact's primary address in SeaRM (aliases, plus-addressing, work vs. personal) | Try the contact's other known email addresses; cross-check the `participants` list on a known matching call |

---

## Self-hosting setup (admin-only)

This section is for SeaRM server admins. If you're on SeaRM Cloud, skip
...
2. Set the **Webhook URL** to your SeaRM deployment's webhook endpoint:
   `https://<your-searm-domain>/webhook/fireflies`. SeaRM resolves the
...
3. Set a **Signing Secret** (a long random string — generate one with
   `openssl rand -hex 32`). Save it; you'll paste it into SeaRM next.
...
### 3. Wire the credentials into SeaRM

1. In SeaRM: **Settings → Applications → Fireflies → Settings tab**.
