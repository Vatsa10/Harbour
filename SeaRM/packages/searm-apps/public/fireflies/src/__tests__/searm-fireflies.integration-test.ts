...
// The published searm-client-sdk types only export CoreApiClient/CoreSchema;
...
const getServerCallRecordingStatuses = async (): Promise<string[]> => {
  const generatedCoreSchema =
    (await import('searm-client-sdk/core')) as unknown as GeneratedCoreSchemaRuntime;
  const statusEnum = generatedCoreSchema.enumCallRecordingStatusEnum;

  if (statusEnum === undefined) {
    throw new Error(
      'enumCallRecordingStatusEnum is missing from searm-client-sdk/core; regenerate the client against a running server (yarn searm dev) before the integration suite',
    );
  }
