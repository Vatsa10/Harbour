...
export const getSlackWebhookSecret = ():
  ...
  if (!isNonEmptyString(secret)) {
    return {
      success: false,
      error:
        'SLACK_WEBHOOK_SECRET application variable is not set. Set it in the SeaRM Slack app settings, using the signing secret from your Slack app (Basic Information > App Credentials).',
    };
  }
