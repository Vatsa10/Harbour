...
Two parts: a **Slack app** you create, and the **SeaRM side** where you paste its credentials and connect. The conversational assistant needs a third part on top.

## 1. Slack app

1. Create a Slack app at [api.slack.com/apps](https://api.slack.com/apps). Use a dedicated app — do not reuse one across SeaRM apps.

2. **OAuth & Permissions → Bot Token Scopes.** SeaRM uses Slack's bot OAuth (`oauth/v2/authorize` with `scope=…`), so scopes must be added here and not only under **User Token Scopes**, otherwise Slack refuses the install with *"doesn't have a bot user to install"*.
...
3. **Redirect URL.** Set it to `<YOUR_SEARM_SERVER_URL>/auth/apps/callback` — the origin your SeaRM **server** uses for API routes, not the SPA. Local monorepo dev is usually `http://localhost:3000` (confirm the port `searm-server` / `SERVER_URL` actually uses).

   **PKCE and `localhost`:** if you enable **PKCE** on the Slack app, Slack treats `http://localhost…` as a *desktop* redirect, and desktop redirects cannot request bot scopes — OAuth will fail. For local dev either leave Slack's PKCE opt-in disabled, or use an `https://` redirect (ngrok, Cloudflare Tunnel), register it in the Slack app, and point `SERVER_URL` at the same base URL. See Slack's [Using PKCE](https://docs.slack.dev/authentication/using-pkce) docs. This is separate from SeaRM sending a PKCE challenge on the authorize request.

4. Copy the **Client ID** and **Client Secret**.

## 2. SeaRM

1. Install this app (`slack`) on your SeaRM server.
2. **Settings → Applications → SeaRM Slack → Application registration** (admin only), set `SLACK_CLIENT_ID` and `SLACK_CLIENT_SECRET`.
...
2. **Event subscriptions.** On the Slack app, enable **Event Subscriptions** and set the Request URL to:
   ...
   ```text
   <YOUR_SEARM_SERVER_URL>/webhooks/server/9ad6fa20-dff5-4d3f-ad5f-084f3c8b0b09
   ```
...
4. **Role.** The `slack-assistant` agent binds to the app's **Slack Assistant** role automatically on install and upgrade. Anyone who can message the bot acts with that role — Slack users are not mapped to individual SeaRM members yet, so keep the role scoped to what you're comfortable exposing.
...
- **One Slack workspace per SeaRM workspace.** Connecting Slack claims that Slack team for the connecting SeaRM workspace. On the same server, a second SeaRM workspace connecting the same Slack team is rejected. The claim is not released on disconnect yet, so moving a Slack workspace needs a server admin.
...
The **Send Slack message** command menu item is backed by two app routes, both requiring an authenticated SeaRM user and using the same Slack connection as the workflow steps:
