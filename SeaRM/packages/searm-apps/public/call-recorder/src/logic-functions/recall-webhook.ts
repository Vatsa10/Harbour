...
import { defineLogicFunction, type RoutePayload } from 'searm-sdk/define';
...
import { extractSearmWorkspaceIdFromRecallWebhook } from 'src/logic-functions/recall-api/extract-searm-workspace-id-from-recall-webhook.util';
...
export const recallWebhookRouteHandler = (
  ...
  const workspaceId = extractSearmWorkspaceIdFromRecallWebhook(body);

  if (!isNonEmptyString(workspaceId)) {
    throw new Error(
      'Webhook payload is missing the SeaRM workspace id in the Recall bot metadata',
    );
  }
