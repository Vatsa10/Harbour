...
const RECALL_ROUTING_METADATA = {
  ...
  searmCallRecordingId: 'call-recording-id',
};
const ENV_VAR_NAMES = [
...
describe('recall bot api', () => {
  ...
  it('lists scheduled bots in a join-at window and follows pagination', async () => {
    fetchMock
      .mockResolvedValueOnce({
        ...
        json: async () => ({
          ...
          results: [
            { id: 'bot-1', metadata: { searmCallRecordingId: 'recording-1' } },
          ],
        }),
      })
      .mockResolvedValueOnce({
    ...
    expect(result).toEqual({
      ...
      bots: [
        {
          id: 'bot-1',
          metadata: { searmCallRecordingId: 'recording-1' },
          statusChanges: [],
          recordings: [],
        },
  ...
  it('omits join-at bounds for metadata-only lookups', async () => {
    ...
    expect(requestParameters.get('metadata__searmCallRecordingId')).toBe(
      'recording-1',
    );
  });
