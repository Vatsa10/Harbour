...
This guide is for SeaRM **server admins**. It covers connecting Call Recorder
to the recording service (Recall.ai), the environment variables it reads, and
how to wire up the webhook.

If you're on **SeaRM Cloud**, these credentials may already be configured —
...
| `CALL_RECORDER_RECORDING_RETENTION_HOURS` | No | How long Recall.ai retains the source media after processing. Defaults to `166` hours (6 days 22 hours), just under Recall's 168-hour free-storage window. Values above `168` may incur Recall storage charges. SeaRM's ingested copy is unaffected. |
...
```text
https://<your-searm-host>/webhooks/server/9215afe6-1497-4149-a49d-e608e239bbaf
```
...
default) to stay inside its free-storage window. SeaRM ingests and stores the
