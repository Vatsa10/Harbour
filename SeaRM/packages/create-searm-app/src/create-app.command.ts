...
import { isDefined, normalizeUrl } from 'searm-shared/utils';
import {
...
const IMAGE = 'searmcrm/searm-app-dev:latest';
...
export class CreateAppCommand {
  ...
  async execute(options: CreateAppOptions = {}): Promise<void> {
    ...
    try {
      ...
      if (!skipLocalInstance) {
        this.logNextStep('Starting SeaRM server');
      ...
      if (serverReady && authSucceeded) {
        ...
        if (!syncSucceeded) {
          this.logDetail('Sync failed. Run `yarn searm dev --once` manually.');
          return;
        }
      } else {
  ...
  private async ensureDockerServer(
    ...
    this.logDetail('Ensuring latest SeaRM server image...');
  ...
  private async openMainPage(
    ...
    try {
      ...
      const token = config.searmCLIAccessToken ?? config.apiKey;
  ...
  private async syncApplication(appDirectory: string): Promise<boolean> {
    this.logDetail('Running `yarn searm dev --once`...');
    return new Promise((resolve) => {
      const child = spawn('yarn', ['searm', 'dev', '--once'], {
        cwd: appDirectory,
  ...
  private async tryExistingAuth(serverUrl: string): Promise<boolean> {
    try {
      ...
      for (const remoteName of remoteNames) {
        ...
        const token = remoteConfig.searmCLIAccessToken ?? remoteConfig.apiKey;
  ...
  private async authenticateWithDevKey(serverUrl: string): Promise<boolean> {
    try {
      ...
      console.log(
        chalk.yellow(
          '  Authentication failed. Run `yarn searm remote:add` manually.',
        ),
      );
  ...
  private async authenticateWithOAuth(serverUrl: string): Promise<boolean> {
    try {
      ...
      console.log(
        chalk.yellow(
          `  OAuth failed: ${result.error.message}\n` +
            `  Run \`yarn searm remote:add --url ${serverUrl}\` manually.`,
        ),
      );
      ...
      console.log(
        chalk.yellow(
          `  Authentication failed. Run \`yarn searm remote:add --url ${serverUrl}\` manually.`,
        ),
      );
  ...
  private logSuccess(
    ...
    if (!authSucceeded) {
      console.log(chalk.white(`  ${stepNumber}. Connect to a SeaRM instance`));
      console.log(
        chalk.cyan('     yarn searm remote:add --url <your-instance-url>\n'),
      );
      stepNumber++;
    }

    console.log(chalk.white(`  ${stepNumber}. Start developing`));
    console.log(chalk.cyan('     yarn searm dev\n'));
    stepNumber++;

    console.log(chalk.white(`  ${stepNumber}. Open your searm instance`));
    console.log(chalk.cyan(`     ${serverUrl}\n`));

    console.log(
      chalk.gray(
        '  Documentation: https://docs.searm.com/developers/extend/apps/getting-started/quick-start',
      ),
    );
  }
}
