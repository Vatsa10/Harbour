...
- Log and PID files live in `/tmp` in the container (`SEARM_UPGRADE_LOG_FILE`, `SEARM_UPGRADE_PID_FILE`) and are lost with the pod.
...
You can write a command for a version listed in `SEARM_NEXT_VERSIONS` — typically the second half of a zero-downtime migration, e.g. dropping a column one release after its replacement ships. Pass the target version to the generator:

```bash
npx nx run searm-server:database:migrate:generate --name <name> --type fast --version 2.20.0
```

It registers and boots (versions are validated against `SEARM_ALL_VERSIONS`) but stays **dormant** — the sequence only runs `SEARM_CROSS_UPGRADE_SUPPORTED_VERSIONS` (previous + current). It activates automatically when `nx version:bump` promotes the version to current.
