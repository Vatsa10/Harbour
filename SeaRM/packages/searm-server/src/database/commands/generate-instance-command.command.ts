...
export class GenerateInstanceCommandCommand extends CommandRunner {
  ...
  parseVersion(value: string): SearmAllVersion {
    if (
      !SEARM_ALL_VERSIONS.includes(
        value as (typeof SEARM_ALL_VERSIONS)[number],
      )
    ) {
      throw new Error(
        `Invalid version "${value}". Must be one of: ${SEARM_ALL_VERSIONS.join(', ')}`,
      );
    }

    return value as SearmAllVersion;
  }
  ...
  async run(
    ...
    const version = options.version ?? SEARM_CURRENT_VERSION;
