...
export class MigrateMessagingInfrastructureToMetadataCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
    private readonly searmORMGlobalManager: GlobalWorkspaceOrmManager,
  ...
  override async runOnWorkspace({
    ...
    const connectedAccountWorkspaceRepository =
      await this.searmORMGlobalManager.getRepository<LegacyConnectedAccountWorkspaceEntity>(
        workspaceId,
        'connectedAccount',
      );

    const messageChannelWorkspaceRepository =
      await this.searmORMGlobalManager.getRepository<MessageChannelEntity>(
        workspaceId,
        'messageChannel',
      );

    const calendarChannelWorkspaceRepository =
      await this.searmORMGlobalManager.getRepository<CalendarChannelEntity>(
        workspaceId,
        'calendarChannel',
      );

    const messageFolderWorkspaceRepository =
      await this.searmORMGlobalManager.getRepository<MessageFolderEntity>(
        workspaceId,
        'messageFolder',
      );
  ...
  private async buildWorkspaceMemberIdToUserWorkspaceIdMap(
    ...
    const workspaceMemberRepository =
      await this.searmORMGlobalManager.getRepository<WorkspaceMemberWorkspaceEntity>(
        workspaceId,
        'workspaceMember',
