...
export class WorkflowThrottlingWorkspaceService {
  ...
  async getRemainingRunsToEnqueueCount(workspaceId: string) {
    return this.throttlerService.getAvailableTokensCount(
      ...
      this.searmConfigService.get('WORKFLOW_EXEC_SOFT_THROTTLE_LIMIT'),
      this.searmConfigService.get('WORKFLOW_EXEC_SOFT_THROTTLE_TTL'),
    );
  }
  ...
  async throttleOrThrowIfHardLimitReached(workspaceId: string) {
    await this.throttlerService.tokenBucketThrottleOrThrow(
      ...
      1,
      this.searmConfigService.get('WORKFLOW_EXEC_HARD_THROTTLE_LIMIT'),
      this.searmConfigService.get('WORKFLOW_EXEC_HARD_THROTTLE_TTL'),
    );
  }
