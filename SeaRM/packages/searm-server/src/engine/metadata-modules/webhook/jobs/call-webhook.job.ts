...
import { ensureAbsoluteUrl } from 'searm-shared/utils';
...
export class CallWebhookJob {
  ...
  private async callWebhook(data: WebhookJobData): Promise<void> {
    ...
    try {
      ...
      if (secret) {
        headers['X-SeaRM-Webhook-Timestamp'] = Date.now().toString();
        headers['X-SeaRM-Webhook-Signature'] = this.generateSignature(
          payloadWithoutSecret,
          secret,
          headers['X-SeaRM-Webhook-Timestamp'],
        );
        headers['X-SeaRM-Webhook-Nonce'] = crypto
          .randomBytes(16)
          .toString('hex');
      }
