...
import { type AllStandardSkillName } from 'src/engine/workspace-manager/searm-standard-application/types/all-standard-skill-name.type';
import {
  type CreateStandardSkillArgs,
  createStandardSkillFlatMetadata,
} from 'src/engine/workspace-manager/searm-standard-application/utils/skill-metadata/create-standard-skill-flat-metadata.util';
...
The sandbox exposes a pre-bound \`searm\` object (see the code-interpreter skill). Use its bulk helpers instead of hand-rolling loops.
...
This is free — it runs before the sandbox and does not add a code_interpreter round-trip. You will know the exact field names before writing any code. Do NOT call \`searm.call_tool('learn_tools', ...)\` from inside the sandbox to learn schemas — that wastes a full sandbox round-trip per schema and pollutes the conversation context.
...
2. **Use schemas learned at step 0.** You already know the field names from the pre-flight \`learn_tools\` call. Do not call \`searm.call_tool('learn_tools', ...)\` inside the sandbox unless you genuinely missed a schema. If you do need it, call it once and cache the result in a Python variable.
3. **Resolve relations to IDs.** Relations link by ID, not by name. Build a lookup map ONCE for only the values referenced in the file:
   \`\`\`python
   company_ids = searm.lookup_by('companies', 'name', df['company'].dropna().unique().tolist())
   \`\`\`
...
4. **Resolve just-created IDs with a bounded \`lookup_by\` — never paginate the table.** \`bulk_upsert\` returns only a count summary (created / updated / failed), not the records, so to link subsequent records (e.g. people to the companies you just upserted) resolve the IDs you need with a \`lookup_by\` bounded to your own values:
   ...
   company_ids = searm.lookup_by('companies', 'name', [r['name'] for r in company_records])
...
7. **Write with bulk_upsert.** Prefer upsert so dedup on unique fields (e.g. email) is handled server-side and re-runs are idempotent:
   \`\`\`python
   summary = searm.bulk_upsert('people', records)
...
## Calling SeaRM Tools from Python (MCP Bridge)

**A \`searm\` variable is already bound in your code's scope.** Do NOT write
\`import searm\` — there is no Python package by that name. The helper is an
...
Call \`searm.list_tools()\` to discover exact names. Catalog tools are routed
...
tools = searm.list_tools()
print(f"{len(tools)} catalog tools available")
for tool in tools[:5]:
    print(f"- {tool['name']}")

# Find records — returns { 'records': [...], 'count': '5' }
companies = searm.call_tool('find_many_companies', {'limit': 5, 'offset': 0})
...
result = searm.call_tool('create_one_company', {
    'name': 'Acme Corp',
...
# Update a record
searm.call_tool('update_one_person', {
    'id': 'person-uuid-here',
    'jobTitle': 'CEO',
})
\`\`\`
...
Do NOT call \`searm.call_tool('learn_tools', ...)\` from inside the sandbox to learn schemas — that costs a full round-trip, adds output to the conversation context, and is unnecessary when you can inspect the schema for free before writing any code. Only use \`searm.call_tool('learn_tools', ...)\` inside the sandbox if you discover at runtime that you need a schema you could not have anticipated beforehand.
...
summary = searm.bulk_upsert('people', records)  # { 'created': C, 'updated': U, 'upserted': N, 'failed': 0, 'errors': [] }
...
company_ids = searm.lookup_by('companies', 'name', ['Acme', 'Globex'])
\`\`\`
