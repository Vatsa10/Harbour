...
import { ALL_METADATA_NAME } from 'searm-shared/metadata';
...
import { isCallerSearmStandardApp } from 'src/engine/metadata-modules/utils/is-caller-searm-standard-app.util';
...
export class FlatSkillValidatorService {
  ...
  >): FailedFlatEntityValidation<'skill', 'delete'> {
    ...
    if (
      !isCallerSearmStandardApp(buildOptions) &&
      belongsToSearmStandardApp({
  ...
  >): FailedFlatEntityValidation<'skill', 'update'> {
    ...
    const isSearmStandardSkill = belongsToSearmStandardApp({
