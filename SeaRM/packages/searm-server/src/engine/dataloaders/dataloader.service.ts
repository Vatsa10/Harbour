...
import { type APP_LOCALES, SOURCE_LOCALE } from 'searm-shared/translations';
...
import { belongsToSearmStandardApp } from 'src/engine/metadata-modules/utils/belongs-to-searm-standard-app.util';
import { getSearmStandardApplicationIdOrThrow } from 'src/engine/metadata-modules/utils/get-searm-standard-application-id-or-throw.util';
            ...
            const overriddenFieldMetadataEntities =
              objectFlatFieldMetadatas.map((flatFieldMetadata) => {
                ...
                const i18nContext = {
                  locale,
                  i18nInstance,
                  isStandardApp: belongsToSearmStandardApp(flatFieldMetadata),
                  applicationCatalog,
                };
    ...
    return new DataLoader<StandardApplicationIdLoaderPayload, string>(
      async (params: StandardApplicationIdLoaderPayload[]) => {
        ...
        const standardApplicationId =
          getSearmStandardApplicationIdOrThrow(flatApplicationMaps);
