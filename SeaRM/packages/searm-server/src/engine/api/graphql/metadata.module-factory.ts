...
export const metadataModuleFactory = async (
  ...
  const config: YogaDriverConfig = {
    ...
    plugins: [
      ...
      useDisableIntrospectionAndSuggestionsForUnauthenticatedUsers(
        searmConfigService.get('NODE_ENV') === NodeEnvironment.PRODUCTION,
      ),
      useValidateGraphqlQueryComplexity({
        maximumAllowedFields: searmConfigService.get('GRAPHQL_MAX_FIELDS'),
  ...
  if (searmConfigService.get('NODE_ENV') === NodeEnvironment.DEVELOPMENT) {
