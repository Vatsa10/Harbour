...
export class LocalChildProcessRunnerService {
  // Symlinks everything from the deps layer except searm-client-sdk,
  ...
  async assembleNodeModules({
    ...
    const symlinkPromises = entries
      .filter((entry) => entry.name !== 'searm-client-sdk')
      .map((entry) =>
        fs.symlink(
    ...
    await fs.symlink(
      join(sdkNodeModules, 'searm-client-sdk'),
      join(execNodeModules, 'searm-client-sdk'),
      'dir',
    );
  }
