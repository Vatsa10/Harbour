...
import { FeatureFlagKey } from 'searm-shared/types';
...
export class LogicFunctionExecutorService {
  ...
  private async throttleExecution(workspaceId: string) {
    try {
      await this.throttlerService.tokenBucketThrottleOrThrow(
        ...
        1,
        this.searmConfigService.get('LOGIC_FUNCTION_EXEC_THROTTLE_LIMIT'),
        this.searmConfigService.get('LOGIC_FUNCTION_EXEC_THROTTLE_TTL'),
      );
    } catch {
  ...
  private async getExecutionEnvVariables({
    ...
    const baseUrl = cleanServerUrl(this.searmConfigService.get('SERVER_URL'));
