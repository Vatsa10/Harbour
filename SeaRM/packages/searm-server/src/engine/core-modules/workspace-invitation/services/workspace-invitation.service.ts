...
import ms from 'ms';
import { SendInviteLinkEmail, renderEmail } from 'searm-emails';
import { AppPath, FileFolder } from 'searm-shared/types';
import { getAppPath, isDefined } from 'searm-shared/utils';
...
export class WorkspaceInvitationService {
  ...
  async sendInvitations(
    ...
    const isOnboardingInviteReward =
      this.searmConfigService.get('IS_BILLING_ENABLED') &&
    ...
    for (const invitation of invitationResults) {
      if (invitation.status === 'fulfilled') {
        ...
        const joinTeamMsg = msg`Join your team on SeaRM`;
        const i18n = this.i18nService.getI18nInstance(sender.locale);
        const subject = i18n._(joinTeamMsg);

        await this.emailService.send({
          from: `${sender.name.firstName} ${sender.name.lastName} (via SeaRM) <${this.searmConfigService.get('EMAIL_FROM_ADDRESS')}>`,
  ...
  async generateInvitationToken(
    ...
    const expiresIn = this.searmConfigService.get(
  ...
  private async throwIfOnboardingInvitationLimitReached(
    ...
    const maxOnboardingInvitations = this.searmConfigService.get(
