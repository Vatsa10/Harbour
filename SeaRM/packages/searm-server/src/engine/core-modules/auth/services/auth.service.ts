...
import ms from 'ms';
import { PasswordUpdateNotifyEmail, renderEmail } from 'searm-emails';
import { PermissionFlagType } from 'searm-shared/constants';
import { AppPath, ConnectedAccountProvider } from 'searm-shared/types';
import { isNonEmptyString } from '@sniptt/guards';
import { assertIsDefinedOrThrow, isDefined } from 'searm-shared/utils';
...
export class AuthService {
  ...
  async checkIsEmailVerified(isEmailVerified: boolean) {
    const isEmailVerificationRequired = this.searmConfigService.get(
  ...
  async updatePassword(
    ...
    await this.emailService.send({
      from: `${this.searmConfigService.get(
        'EMAIL_FROM_NAME',
      )} <${this.searmConfigService.get('EMAIL_FROM_ADDRESS')}>`,
      to: user.email,
      subject,
      text,
      html,
