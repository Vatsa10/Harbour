...
export class GoogleApisServiceAvailabilityService {
  ...
  async checkServicesAvailability(
    ...
    const oAuth2Client = new google.auth.OAuth2({
      clientId: this.searmConfigService.get('AUTH_GOOGLE_CLIENT_ID'),
      clientSecret: this.searmConfigService.get('AUTH_GOOGLE_CLIENT_SECRET'),
  ...
  private async checkMessagingAvailability(
    ...
    if (!this.searmConfigService.get('MESSAGING_PROVIDER_GMAIL_ENABLED')) {
      return false;
    }
  ...
  private async checkCalendarAvailability(
    ...
    if (!this.searmConfigService.get('CALENDAR_PROVIDER_GOOGLE_ENABLED')) {
      return false;
    }
