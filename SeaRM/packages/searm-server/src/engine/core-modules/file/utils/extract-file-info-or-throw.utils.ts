...
import { SEARM_MIME_POLICY } from 'src/engine/core-modules/file/constants/searm-mime-policy.constant';
...
export const extractFileInfoOrThrow = async ({
  ...
  if (isNonEmptyString(ext)) {
    // SeaRM policy wins over the ext-based fallback for the (small) set of
    ...
    // source upload.
    const policyMime = SEARM_MIME_POLICY[ext];
