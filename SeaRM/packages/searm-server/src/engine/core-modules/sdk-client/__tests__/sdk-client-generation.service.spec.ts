...
describe('SdkClientGenerationService', () => {
  ...
  beforeEach(async () => {
    applicationService = {
      findWorkspaceSearmStandardAndCustomApplicationOrThrow: jest.fn(),
    };
    messageQueueService = {
  ...
  describe('enqueueSdkClientGenerationForWorkspace', () => {
    ...
    const apps = {
      searmStandardFlatApplication: {
        id: 'std-app-id',
        universalIdentifier: 'searm-standard',
      },
    ...
    it('enqueues one job per application with dedup id and retry limit', async () => {
      applicationService.findWorkspaceSearmStandardAndCustomApplicationOrThrow.mockResolvedValue(
        apps as never,
      );
      ...
      expect(messageQueueService.add).toHaveBeenNthCalledWith(
        ...
        GENERATE_SDK_CLIENT_JOB_NAME,
        {
          ...
          applicationUniversalIdentifier: 'searm-standard',
