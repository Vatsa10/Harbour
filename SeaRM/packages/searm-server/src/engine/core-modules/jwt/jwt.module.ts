...
const InternalJwtModule = NestJwtModule.registerAsync({
  useFactory: async (searmConfigService: SearmConfigService) => {
    return {
      secret: searmConfigService.get('APP_SECRET'),
      signOptions: {
        algorithm: JWT_LEGACY_ALGORITHM,
        expiresIn: searmConfigService.get('ACCESS_TOKEN_EXPIRES_IN'),
      },
      verifyOptions: {
