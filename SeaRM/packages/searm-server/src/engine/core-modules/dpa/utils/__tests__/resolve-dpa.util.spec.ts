...
describe('resolveDpa', () => {
  it('defaults to EU and resolves the EU Processor entity, law and dormant SCC state', () => {
    ...
    expect(resolved.values.PROCESSOR_ENTITY).toBe('SeaRM.com SAS');
  ...
  it('resolves the US Processor entity, hosting and active SCC state', () => {
    ...
    expect(resolved.values.PROCESSOR_ENTITY).toBe('SeaRM.com PBC');
  ...
  it('substitutes the Processor entity into the contracting clause per region', () => {
    ...
    // Assert on the contracting clause (block 0), not the whole doc: "SeaRM.com PBC"
    // also appears verbatim in Annex A's "for US deployments" note.
    expect(eu.blocks[0].text).toContain('between SeaRM.com SAS (');
    expect(us.blocks[0].text).toContain('between SeaRM.com PBC (');
  });
  ...
  it('renders the DPO name and contact', () => {
    ...
    expect(text).toContain('privacy@searm.com');
  });
