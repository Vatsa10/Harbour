...
const SELF_HOSTED_NOTICE =
  'NOT A VALID AGREEMENT — SELF-HOSTED DEPLOYMENT. This SeaRM instance is self-hosted. For self-hosted deployments SeaRM does not host or process Customer Personal Data and is not the Processor, so this Data Processing Agreement does not apply. This copy is generated for reference only and does not constitute an executed agreement with SeaRM.';
...
const buildExecutionBlocks = (
  ...
  const blocks: ResolvedDpaBlock[] = [
    ...
    },
    {
      ...
      value: `Signed on behalf of ${values.PROCESSOR_ENTITY} (pre-signed by SeaRM)\nName: ${SEARM_PRESIGNED_SIGNATORY.name}\nTitle: ${SEARM_PRESIGNED_SIGNATORY.title}`,
    },
    {
      kind: 'signatureField',
      text: '',
