...
export class DpaService {
  ...
  private isSelfHosted(): boolean {
    return this.searmConfigService.get('IS_MULTIWORKSPACE_ENABLED') !== true;
  }
  ...
  async generateSignedDpa({
    ...
    if (this.isSelfHosted()) {
      throw new BadRequestException(
        'DPA signing is not available for self-hosted deployments: SeaRM does not host or process Customer Personal Data and is not the Processor.',
      );
    }
    ...
    const [pdfBuffer, { workspaceCustomFlatApplication }] = await Promise.all([
      ...
      this.applicationService.findWorkspaceSearmStandardAndCustomApplicationOrThrow(
