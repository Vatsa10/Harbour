...
import { OBJECTS_WITH_CHANNEL_VISIBILITY_CONSTRAINTS } from 'searm-shared/constants';
import {
...
export class SearchService {
  ...
  private async buildIlikeFallbackQuery<Entity extends ObjectLiteral>({
    ...
    const timeoutMs = this.searmConfigService.get(
  ...
  async getImageIdentifierValue(
    ...
    switch (imageIdentifierField.type) {
      ...
      case FieldMetadataType.LINKS: {
        if (!this.searmConfigService.get('ALLOW_REQUESTS_TO_SEARM_ICONS')) {
          return '';
        }
