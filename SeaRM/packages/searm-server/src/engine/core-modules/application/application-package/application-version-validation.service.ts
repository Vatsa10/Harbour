  ...
  async validateServerCompatibility(
    ...
    if (!isDefined(semver.validRange(requiredServerVersion))) {
      return {
        ...
        message: `App manifest declares invalid engines.searm value "${requiredServerVersion}". Must be a valid semver range.`,
      };
    }
    ...
    if (!semver.satisfies(version, requiredVersionRange)) {
      return scope === 'workspace'
        ? {
            ...
            message: `App requires SeaRM server ${requiredVersionRange} but this workspace has only completed the upgrade to ${version}.`,
          }
        : {
            compatible: false,
            reason: 'INSTANCE_INCOMPATIBLE',
            message: `App requires SeaRM server ${requiredVersionRange} but this server is ${version}.`,
          };
    }
