...
import { SEARM_CLI_APPLICATION_REGISTRATION } from 'src/engine/workspace-manager/searm-standard-application/constants/searm-cli-application-registration.constant';
...
export class ApplicationRegistrationService {
  ...
  async createCliRegistrationIfNotExists(): Promise<ApplicationRegistrationEntity | null> {
    const existing = await this.findOneByUniversalIdentifier(
      SEARM_CLI_APPLICATION_REGISTRATION.universalIdentifier,
    );
    ...
    const registration = this.applicationRegistrationRepository.create({
      universalIdentifier:
        SEARM_CLI_APPLICATION_REGISTRATION.universalIdentifier,
      name: SEARM_CLI_APPLICATION_REGISTRATION.name,
      oAuthClientId: v4(),
      oAuthClientSecretHash: null,
      oAuthRedirectUris: [],
      oAuthScopes: SEARM_CLI_APPLICATION_REGISTRATION.oAuthScopes,
      ownerWorkspaceId: null,
