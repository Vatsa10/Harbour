...
import type { ApplicationManifest, Manifest } from 'searm-shared/application';
...
export class ApplicationTarballService {
  ...
  async uploadTarball(params: {
    ...
    const tempDir = join(tmpdir(), 'searm-tarball-upload', v4());
  ...
  private async extractAndValidateTarball(
    ...
    packageJson: { version: string; engines?: { searm?: string } } | null;
  }> {
