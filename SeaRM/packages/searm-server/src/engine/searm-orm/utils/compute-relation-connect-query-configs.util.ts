...
import { type ConnectObject } from 'src/engine/searm-orm/entity-manager/types/query-deep-partial-entity-with-nested-relation-fields.type';
import {
  type RelationConnectQueryConfig,
  type UniqueConstraintCondition,
} from 'src/engine/searm-orm/entity-manager/types/relation-connect-query-config.type';
import { type RelationConnectQueryFieldsByEntityIndex } from 'src/engine/searm-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import {
  SearmORMException,
  SearmORMExceptionCode,
} from 'src/engine/searm-orm/exceptions/searm-orm.exception';
import { formatCompositeField } from 'src/engine/searm-orm/utils/format-data.util';
...
const computeRecordToConnectCondition = (
  ...
  if (
    ...
    throw new SearmORMException(
      ...
      SearmORMExceptionCode.CONNECT_NOT_ALLOWED,
      {
...
const checkUniqueConstraintFullyPopulated = (
  ...
  if (!hasUniqueConstraintFieldFullyPopulated) {
    throw new SearmORMException(
      ...
      SearmORMExceptionCode.CONNECT_UNIQUE_CONSTRAINT_ERROR,
      {
