...
yarn patch-commit <temp-folder> > packages/searm-server/patches/<package+name+version>.patch
yarn install --mode update-lockfile && yarn install
```

Reference the patch in `packages/searm-server/package.json`:
...
yarn patch-commit <CLEAN> > packages/searm-server/patches/<package+name+version>.patch
yarn install --mode update-lockfile && yarn install
```
