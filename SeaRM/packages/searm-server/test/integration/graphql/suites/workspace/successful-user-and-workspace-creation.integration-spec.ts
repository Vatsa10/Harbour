...
describe('Successful user and workspace creation', () => {
  ...
  it('should sign up a new user and create a new workspace successfully', async () => {
    ...
    const searmStandardApp = findManyApplicationsData.find(
      (application) =>
        application.universalIdentifier ===
        SEARM_STANDARD_APPLICATION.universalIdentifier,
    );

    jestExpectToBeDefined(searmStandardApp);
    const {
      sourcePath: _sourcePath,
      sourceType: _sourceType,
      ...expectedStandardSearmApplication
    } = SEARM_STANDARD_APPLICATION;

    expect(searmStandardApp).toMatchObject(expectedStandardSearmApplication);
