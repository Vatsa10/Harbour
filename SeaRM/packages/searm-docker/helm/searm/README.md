...
Deploy SeaRM CRM on Kubernetes with server, worker, PostgreSQL, and Redis components.
...
helm install my-searm ./packages/searm-docker/helm/searm \
  --namespace searmcrm --create-namespace
```
...
- Database `searm` and schema `core` are created automatically by server init container
...
```bash
helm lint ./packages/searm-docker/helm/searm
helm template my-searm ./packages/searm-docker/helm/searm
helm plugin install https://github.com/quintush/helm-unittest
helm unittest ./packages/searm-docker/helm/searm
```
...
helm install my-searm ./packages/searm-docker/helm/searm -f values-secrets.yaml
```
