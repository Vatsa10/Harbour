...
su-exec postgres psql -h localhost -tc \
  "SELECT 1 FROM pg_roles WHERE rolname='searm'" | grep -q 1 \
  || su-exec postgres psql -h localhost -c "CREATE ROLE searm WITH LOGIN PASSWORD 'searm' SUPERUSER"

# Create database if it doesn't exist
su-exec postgres psql -h localhost -tc \
  "SELECT 1 FROM pg_database WHERE datname='default'" | grep -q 1 \
  || su-exec postgres createdb -h localhost -O searm default

# Run SeaRM database setup and migrations
cd /app/packages/searm-server

has_schema=$(PGPASSWORD=searm psql -h localhost -U searm -d default -tAc \
...
has_workspace=$(PGPASSWORD=searm psql -h localhost -U searm -d default -tAc \
