...
e auto-hospedadas, esse caminho é servido pelo seu servidor SeaRM
(por exemplo, `http://localhost:2020/s/documents/generate`). No SeaRM Cloud, `RestApiClient`
...
sanitizado) e o coloca em um limpo, página imprimível que mostra apenas o conteúdo do modelo- a mesma aparência que o PDF e a visualização no aplicativo.
  [Veja o ajudante](https://github.com/Vatsa10/Harbour/blob/main/packages/searm-apps/examples/document-generator/src/utils/render-document.ts).
...
`yarn searm dev:function:logs`, ou invocá-los diretamente com
