...
As relações conectam dois objetos entre si. No SeaRM, as relações são sempre **bidirecionais** — cada relação tem dois lados, e cada lado é declarado como um campo que faz referência ao outro.
...
Para criar uma relação com um objeto integrado do SeaRM (Person, Company, etc.), use `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
