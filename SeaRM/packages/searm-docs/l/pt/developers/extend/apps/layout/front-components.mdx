...
description: Crie componentes React que renderizam dentro da UI do SeaRM com isolamento em sandbox.
icon: window-maximize
---

Componentes de front-end são componentes React que renderizam diretamente dentro da UI do SeaRM. Eles são executados em um **Web Worker isolado** usando Remote DOM — seu código é executado dentro de um iframe de origem opaca e em sandbox, mas sua interface ainda é renderizada de forma nativa na página em vez de ficar confinada a esse iframe.

## Onde os componentes de front-end podem ser usados

Os componentes de front-end podem ser renderizados em três locais dentro do SeaRM:
...
Após sincronizar com `yarn searm dev` (ou executando uma única vez o `yarn searm apply`), a ação rápida aparece no canto superior direito da página:
...
O componente é renderizado como uma seção **dentro** da aba Settings, e não como uma substituição de toda a aba. As seções gerenciadas pelo sistema do SeaRM — atualização automática, App URL e conexões — são sempre renderizadas acima dela e não podem ser substituídas pelo app.
...
Como o componente retorna `null`, o SeaRM ignora renderizar um contêiner para ele — nenhum espaço vazio aparece no layout. O componente ainda tem acesso a todos os hooks e à API de comunicação do host.

## Componentes Command do SDK

O pacote `searm-sdk` fornece quatro componentes auxiliares Command projetados para componentes de front-end headless. Cada componente executa uma ação ao montar, trata erros exibindo uma notificação de snackbar e desmonta automaticamente o componente de front-end ao concluir.

Importe-os de `searm-sdk/front-component`:
...
Uma função lógica declarada com `httpRouteTriggerSettings` é acessível por HTTP em seu caminho de rota. `RestApiClient` trata caminhos que começam com `/s/` como rotas de aplicativo, resolve-os para a URL a partir da qual suas funções são servidas e os autentica com `SEARM_APP_ACCESS_TOKEN`.

> **No SeaRM Cloud, funções lógicas acionadas por HTTP são servidas em um domínio dedicado por workspace** em `https://\<your-workspace-subdomain>.withsearm.com\<path>`. Para chamadores externos, copie a URL exata das configurações de **HTTP trigger** da função ou da guia **Settings** do aplicativo.
...
O caminho passado para o `RestApiClient` é o `httpRouteTriggerSettings.path` da função de lógica, prefixado com `/s`. Mantenha `isAuthRequired: true`; o `SEARM_APP_ACCESS_TOKEN` que a SeaRM gera para o seu componente autentica a solicitação:
...
<Note>
`SEARM_APP_ACCESS_TOKEN` é injetado automaticamente — consulte [Variáveis de aplicação](#application-variables). Como as variáveis de aplicação secretas nunca são expostas aos componentes de front, mantenha as chaves de API e outra lógica sensível na função lógica, não no componente de front.
</Note>

### Chamando a API REST da SeaRM

Para chamar rotas HTTP do aplicativo ou ler e gravar registros da SeaRM a partir de um front component, use `RestApiClient` de `searm-client-sdk/rest`. Ele envia caminhos `/s/...` para a URL base das funções do seu workspace e qualquer outro caminho, incluindo `/rest/...`, para `SEARM_API_URL`.
...
| Variável                  | Descrição                                                     |
| ------------------------- | ------------------------------------------------------------- |
| `SEARM_API_URL`          | URL base da API principal da SeaRM                           |
| `SEARM_APP_ACCESS_TOKEN` | Token de curta duração limitado ao escopo do papel do seu app |

### `SEARM_FUNCTIONS_URL`

A SeaRM também injeta `SEARM_FUNCTIONS_URL` em front components e funções de lógica: a URL base a partir da qual as funções de lógica acionadas por HTTP do seu aplicativo são servidas.

Ela existe porque essa URL nem sempre é o próprio servidor da SeaRM. No SeaRM Cloud, as rotas do aplicativo são servidas em um domínio dedicado por workspace (`https://\<your-workspace-subdomain>.withsearm.com`, ou o domínio público primário da aplicação quando um é configurado) para que respostas criadas pelo aplicativo sejam executadas em uma origem isolada, em vez de na origem do aplicativo SeaRM. Instâncias self-hosted e locais servem rotas do aplicativo sob o prefixo `/s` no próprio servidor e podem não definir a variável. Como a URL base varia por workspace e por instância, seu código não pode defini-la de forma fixa — o servidor injeta o valor correto em tempo de execução.

Você raramente precisa lê-la diretamente. Chame suas rotas por meio de `RestApiClient` com um caminho prefixado com `/s/` e o cliente resolverá a URL para você: ele remove o prefixo `/s` e direciona para `SEARM_FUNCTIONS_URL`, recorrendo a `\<SEARM_API_URL>/s` quando a variável não está definida. Use `resolveUrl('/s/\<path>')` para obter a URL absoluta sem enviar uma requisição, por exemplo, para um link. Leia a variável diretamente apenas ao construir uma URL manualmente:
...
Componentes de front-end podem acionar navegação, modais e notificações usando funções de `searm-sdk`:
...
* **Componentes de UI da SeaRM** — a própria biblioteca de componentes da SeaRM; consulte [Usando componentes de UI da SeaRM](#using-searm-ui-components) abaixo
...
## Usando componentes de UI da SeaRM

SeaRM distribui sua biblioteca de componentes como o pacote [`searm-ui`](https://www.npmjs.com/package/searm-ui/v/1.0.0-alpha.1). Os componentes de front-end podem usá-lo para botões, tags, pílulas de status, chips, avatares, ícones, tipografia e tokens de tema que correspondem automaticamente ao tema claro e escuro do espaço de trabalho.

### Instalação

Adicione o pacote ao seu app, fixado na versão fornecida pela sua instância do SeaRM:

```bash
yarn add searm-ui@1.0.0-alpha.1
```

`searm-ui` é empacotado no seu componente de front-end em tempo de build, então ele só precisa ser uma dependência do seu app — não há nada para configurar em tempo de execução.
...
| --------------------------- | ----------------------------------------------- |
| `searm-ui/input`           | `Button` e inputs de formulário                 |
| `searm-ui/data-display`    | `Tag`, `Status`, `Chip`, `Avatar` e mais        |
| `searm-ui/feedback`        | `Callout`, `Banner`, `Info` e mais              |
| `searm-ui/typography`      | `H1Title`, `H2Title`, `H3Title`, `Label` e mais |
| `searm-ui/icon`            | Componentes `Icon*` (por exemplo, `IconCheck`)  |
...
Importe ícones individuais de `searm-ui/icon`:
...
Os componentes do SeaRM UI correspondem automaticamente ao tema claro e escuro do espaço de trabalho — o renderizador aplica o esquema de cores ativo no host, e os componentes resolvem suas cores com base nele.

Para usar os mesmos tokens de design nos seus próprios estilos inline, chame o hook `useTheme()`. Ele retorna os tokens de tema do SeaRM (espaçamento, cores, raios, fontes) conectados ao tema ativo, sem necessidade de configurar `ThemeProvider` no seu componente:
...
Para diferenciar explicitamente com base no esquema ativo, leia-o com `useColorScheme()` de `searm-sdk/front-component`, que retorna 'light' ou 'dark'.
