...
* **Incluídos nas compilações** — `yarn searm dev:build` agrupa todos os recursos públicos na saída de distribuição.

## Acessando recursos públicos com `getPublicAssetUrl`

Use o helper `getPublicAssetUrl` de `searm-sdk` para obter a URL completa de um arquivo no seu diretório `public/`. Funciona tanto em **funções lógicas** quanto em **componentes de front-end**.
