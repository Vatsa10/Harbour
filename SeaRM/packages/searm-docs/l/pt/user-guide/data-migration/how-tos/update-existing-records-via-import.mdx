...
Quando importar um ficheiro que contenha um **identificador único** que corresponde a um registo existente, a SeaRM atualiza esse registo em vez de criar um duplicado.

| Se o identificador único...        | A SeaRM irá...                                           |
| ---------------------------------- | --------------------------------------------------------- |
...
4. **Não altere os valores do identificador único** — caso contrário a SeaRM criará novos registos
...
Se alterar `john@acme.com` para `john.smith@acme.com`, a SeaRM criará um novo registo em vez de atualizar o existente.
</Warning>
...
A SeaRM faz a correspondência dos registos pelo identificador único e atualiza-os com novos valores.
...
3. A SeaRM fará a correspondência com base nesse campo
...
| **Incompatibilidade de maiúsculas/minúsculas** | O ficheiro tem `John@acme.com`, mas a SeaRM tem `john@acme.com` | Cria novo registo                        | Exporte da SeaRM para obter os valores exatos |
...
<AccordionGroup>
  ...
  <Accordion title="Como sei quais os registos que serão atualizados vs. criados?">
    Durante a etapa de revisão da importação, a SeaRM mostra quantos registos serão atualizados vs. criados com base nas correspondências do identificador único.
  </Accordion>
