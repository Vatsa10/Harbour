...
description: قم بإحالة العمل الطويل الأمد أو المقيّد بالمعدل إلى عمّال SeaRM عن طريق وضع تشغيل دالة منطقية أخرى في قائمة الانتظار بدلًا من تنفيذ كل شيء ضمن نفس السياق.
icon: layer-group
---
...
تقوم `enqueueJob` بذلك بالضبط: فهي تطلب من عمّال SeaRM تشغيل واحدة من دوال المنطق في تطبيقك لاحقًا، في عملية مستقلة، وبمهلة زمنية خاصة بها. يعود المستدعي فورًا.
...
قم باستيراد `enqueueJob` من `searm-sdk/logic-function` ووجّهه إلى `universalIdentifier` الخاص بوظيفة المنطق التي تريد تشغيلها.
