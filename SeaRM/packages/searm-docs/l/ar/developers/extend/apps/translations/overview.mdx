...
تُكتب تطبيقات SeaRM باللغة **الإنجليزية**: السلاسل في شفرتك وملف البيان هي نص المصدر بالإنجليزية، و`en` هي الإعداد المحلي الذي تترجم منه، وأي إعداد محلي يُترك دون ترجمة يعود إليه افتراضيًا.
...
قم باستيراد أدوات الترجمة من `searm-sdk/front-component`:
...
يقوم `searm dev:build` بتجميع الكتالوجات وتقديم اللغة الصحيحة للمستخدم
...
إعادة تشغيل `searm dev:build` (وإعادة النشر)، تمامًا مثل أي تغيير آخر.

<Note>
يتم تجميع الترجمات بواسطة `searm dev:build` (و`searm apply`). تشغيل المراقبة المستمرة `searm dev`
