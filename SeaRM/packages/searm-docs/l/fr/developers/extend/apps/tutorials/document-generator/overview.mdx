...
description: Créez une véritable application SeaRM qui génère des documents personnalisés à partir des données de votre CRM.
---
...
un serveur SeaRM local en cours d’exécution sur le port `2020` et le CLI authentifié dessus.
...
Laissez `yarn searm dev` s’exécuter dans un terminal pendant tout le tutoriel. Chaque fois que
