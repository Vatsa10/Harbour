...
description: Confiez les travaux longs ou soumis à des limitations de débit aux workers de SeaRM en mettant en file d’attente une autre exécution de fonction logique au lieu de tout faire en ligne.
icon: layer-group
---
...
`enqueueJob` fait exactement cela : il demande aux workers de SeaRM d’exécuter plus tard l’une des fonctions logiques de votre application, dans son propre processus, avec son propre budget de délai d’expiration. La fonction appelante retourne immédiatement.
...
Importez `enqueueJob` depuis `searm-sdk/logic-function` et pointez-le vers le `universalIdentifier` de la fonction logique que vous voulez exécuter.
