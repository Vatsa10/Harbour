...
Les applications SeaRM sont rédigées en **anglais** : les chaînes de vos sources et de votre manifeste
...
Importez les helpers de traduction depuis `searm-sdk/front-component` :
...
`searm dev:build` compile les catalogues et sert la bonne langue pour
...
de relancer `searm dev:build` (et de redéployer), comme pour toute autre modification.

<Note>
Les traductions sont compilées par `searm dev:build` (et `searm apply`). La
surveillance continue `searm dev` affiche les chaînes source, donc testez la sortie localisée
