...
Le système d'autorisations de SeaRM vous permet de contrôler l'accès à trois domaines principaux :
...
<Note>
Les autorisations au niveau des lignes sont une **fonctionnalité Premium** disponible avec l’offre **Organization** (Cloud et Self-Hosted).
</Note>

Les autorisations au niveau des lignes vous permettent de restreindre les enregistrements individuels qu’un rôle peut voir ou modifier, en fonction de critères dynamiques. Contrairement aux autorisations sur les objets (qui s’appliquent à un type d’objet entier), les autorisations au niveau des lignes évaluent chaque enregistrement indépendamment.

**Exemples de cas d’utilisation :**

* Les représentants commerciaux ne peuvent voir que leurs propres opportunités
* Les responsables peuvent voir tous les enregistrements de leur région
* Les agents du support ne peuvent consulter que les tickets qui leur sont attribués

Pour configurer les autorisations au niveau des lignes, ouvrez un rôle, allez à l’onglet **Objects**, puis utilisez la section **Row-Level** pour définir des conditions de filtrage pour un objet spécifique.
