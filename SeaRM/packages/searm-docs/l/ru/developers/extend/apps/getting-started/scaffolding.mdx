...
description: Интерактивно генерируйте файлы сущностей с помощью yarn searm dev:add — объекты, поля, представления, логические функции и многое другое.
...
| -------------------- | ---------------------------------------- | ------------------------------------------------------- |
| Объект               | `yarn searm dev:add object`             | `src/objects/\<name>.ts`               |
| Поле                 | `yarn searm dev:add field`              | `src/fields/\<name>.ts`                |
| Логическая функция   | `yarn searm dev:add logicFunction`      | `src/logic-functions/\<name>.ts`       |
| Компонент фронтенда  | `yarn searm dev:add frontComponent`     | `src/front-components/\<name>.tsx`     |
| Роль                 | `yarn searm dev:add role`               | `src/roles/\<name>.ts`                 |
| Навык                | `yarn searm dev:add skill`              | `src/skills/\<name>.ts`                |
| Агент                | `yarn searm dev:add agent`              | `src/agents/\<name>.ts`                |
| Представление        | `yarn searm dev:add view`               | `src/views/\<name>.ts`                 |
| Пункт меню навигации | `yarn searm dev:add navigationMenuItem` | `src/navigation-menu-items/\<name>.ts` |
| Макет страницы       | `yarn searm dev:add pageLayout`         | `src/page-layouts/\<name>.ts`          |

## Что генерирует скэффолдер

У каждого типа сущности есть свой шаблон. Например, `yarn searm dev:add object` запрашивает:
