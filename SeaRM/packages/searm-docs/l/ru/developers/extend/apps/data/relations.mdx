...
Отношения связывают два объекта между собой. В SeaRM отношения всегда **двунаправленные** — у каждого отношения есть две стороны, и каждая сторона объявляется как поле, ссылающееся на другую.
...
Чтобы создать отношение со встроенным объектом SeaRM (Person, Company и т. д.), используйте `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
