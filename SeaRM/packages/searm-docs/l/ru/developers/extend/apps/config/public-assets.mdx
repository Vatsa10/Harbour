...
* **Включены в сборки** — `yarn searm dev:build` упаковывает все публичные ресурсы в выходной дистрибутив.

## Доступ к публичным ресурсам с помощью `getPublicAssetUrl`

Используйте хелпер `getPublicAssetUrl` из `searm-sdk`, чтобы получить полный URL файла в каталоге `public/` вашего приложения. Он работает как в **логических функциях**, так и в **компонентах фронтенда**.
