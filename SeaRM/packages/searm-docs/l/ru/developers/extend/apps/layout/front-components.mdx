...
description: Создавайте компоненты React, которые отображаются внутри интерфейса SeaRM в изолированной песочнице.
icon: window-maximize
---

Фронтенд-компоненты — это компоненты React, которые отображаются непосредственно внутри интерфейса SeaRM. Они выполняются в изолированном Web Worker с использованием Remote DOM — ваш код изолирован (sandboxed), но рендерится нативно на странице, а не в iframe.

## Где можно использовать фронт-компоненты

Фронт-компоненты могут отображаться в двух местах внутри SeaRM:
...
После синхронизации с помощью `yarn searm dev` (или однократного запуска `yarn searm dev --once`) быстрое действие появится в правом верхнем углу страницы:
...
import { defineFrontComponent } from 'searm-sdk/define';
import { useRecordId, enqueueSnackbar } from 'searm-sdk/front-component';
...
Поскольку компонент возвращает `null`, SeaRM пропускает рендеринг контейнера для него — в макете не появляется пустое место. Компонент по-прежнему имеет доступ ко всем хукам и API взаимодействия с хостом.

## Компоненты SDK Command

Пакет `searm-sdk` предоставляет четыре вспомогательных компонента Command, предназначенных для headless фронт-компонентов. Каждый компонент выполняет действие при монтировании, обрабатывает ошибки, показывая уведомление snackbar, и автоматически размонтирует фронт-компонент по завершении.

Импортируйте их из `searm-sdk/command`:
...
import { defineFrontComponent } from 'searm-sdk/define';
import { Command } from 'searm-sdk/command';
import { CoreApiClient } from 'searm-sdk/clients';
...
import { defineFrontComponent } from 'searm-sdk/define';
import { CommandModal } from 'searm-sdk/command';
...
Логическая функция, объявленная с `httpRouteTriggerSettings`, доступна по HTTP по своему пути маршрута. SeaRM внедряет в воркер базовый URL, с которого обслуживаются ваши функции, в виде `SEARM_FUNCTIONS_URL` вместе с `SEARM_APP_ACCESS_TOKEN`, который аутентифицирует вызов. Пока что нет отдельного клиентского SDK для вызова ваших собственных функций, поэтому вызывайте их с помощью обычного `fetch`:

> **В SeaRM Cloud логические функции с HTTP-триггером обслуживаются на выделенном домене для каждого рабочего пространства** по адресу `https://\<your-workspace-subdomain>.searm.com\<path>` — именно к этому и разрешается `SEARM_FUNCTIONS_URL`. Для внешних вызовов скопируйте точный URL из настроек **HTTP trigger** функции или на вкладке **Settings** приложения.

<Warning>
  Устаревший маршрут функции `/s/` **не рекомендуется к использованию** и будет **деактивирован 2026-07-24**. Вместо этого используйте `SEARM_FUNCTIONS_URL` (выше) и перенесите все жестко заданные URL вида `/s/` до этой даты. Маршрут `/s/` по-прежнему доступен при самостоятельном размещении (self-hosting).
</Warning>
...
const SyncPrs = () => {
  const execute = async () => {
    await fetch(`${process.env.SEARM_FUNCTIONS_URL}/github/fetch-prs`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${process.env.SEARM_APP_ACCESS_TOKEN}`,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ owner: 'Vatsa10', repo: 'searm' }),
    });
  };
...
Путь, добавляемый к `SEARM_FUNCTIONS_URL`, — это значение `httpRouteTriggerSettings.path` логической функции. Сохраните `isAuthRequired: true`; `SEARM_APP_ACCESS_TOKEN`, который SeaRM выпускает для вашего компонента, аутентифицирует запрос:
...
<Note>
`SEARM_FUNCTIONS_URL` и `SEARM_APP_ACCESS_TOKEN` внедряются автоматически — см. [переменные приложения](#application-variables). Поскольку секретные переменные приложения никогда не раскрываются front-компонентам, храните ключи API и другую конфиденциальную логику в логической функции, а не во front-компоненте.
</Note>

### Вызов REST API SeaRM

Чтобы читать или изменять записи SeaRM из фронт-компонента, используйте `RestApiClient` из `searm-client-sdk/rest`. Он принадлежит к тому же семейству клиентов, что и `CoreApiClient` и `MetadataApiClient`, но нацелен на REST API SeaRM (`/rest/...`) вместо GraphQL API, считывая базовый URL из `SEARM_API_URL`.
...
| Переменная                | Описание                                                                      |
| ------------------------- | ----------------------------------------------------------------------------- |
| `SEARM_FUNCTIONS_URL`    | Базовый URL, с которого обслуживаются функции HTTP-логики вашего приложения   |
| `SEARM_API_URL`          | Базовый URL основного API SeaRM                                              |
| `SEARM_APP_ACCESS_TOKEN` | Краткоживущий токен с областью действия, ограниченной ролью вашего приложения |

## API взаимодействия с хостом

Компоненты фронтенда могут вызывать навигацию, модальные окна и уведомления с помощью функций из `searm-sdk`:
...
import { defineFrontComponent, numberOfSelectedRecords } from 'searm-sdk/define';
...
* **SeaRM UI components** — собственная библиотека компонентов SeaRM; см. раздел [Using SeaRM UI components](#using-searm-ui-components) ниже
...
## Использование компонентов SeaRM UI

SeaRM поставляет свою библиотеку компонентов как пакет [`searm-ui`](https://www.npmjs.com/package/searm-ui/v/1.0.0-alpha.1). Компоненты фронтенда могут использовать его для кнопок, тегов, статусных плашек, чипов, аватаров, иконок, типографики и токенов темы, которые автоматически соответствуют светлой и тёмной теме рабочего пространства.

### Установка

Добавьте пакет в своё приложение, зафиксировав его на версии, с которой поставляется ваш экземпляр SeaRM:

```bash
yarn add searm-ui@1.0.0-alpha.1
```

`searm-ui` включается в ваш компонент фронтенда на этапе сборки, поэтому его достаточно иметь в зависимостях вашего приложения — во время выполнения ничего настраивать не нужно.
...
| --------------------------- | ------------------------------------------------- |
| `searm-ui/input`           | `Button` и элементы ввода формы                   |
| `searm-ui/data-display`    | `Tag`, `Status`, `Chip`, `Avatar` и другие        |
| `searm-ui/feedback`        | `Callout`, `Banner`, `Info` и другие              |
| `searm-ui/typography`      | `H1Title`, `H2Title`, `H3Title`, `Label` и другие |
| `searm-ui/icon`            | Компоненты `Icon*` (например, `IconCheck`)        |
...
Импортируйте отдельные иконки из `searm-ui/icon`:
...
Компоненты SeaRM UI автоматически подстраиваются под светлую и тёмную темы рабочего пространства — рендерер применяет активную цветовую схему на хосте, а компоненты вычисляют свои цвета относительно неё.

Чтобы использовать те же дизайн‑токены в собственных встроенных стилях, вызовите хук `useTheme()`. Он возвращает токены темы SeaRM (отступы, цвета, радиусы, шрифты), привязанные к активной теме, без необходимости настраивать `ThemeProvider` в вашем компоненте:
...
Чтобы явно разветвлять логику по активной цветовой схеме, считайте её с помощью `useColorScheme()` из `searm-sdk/front-component`, который возвращает `'light'` или `'dark'`.
