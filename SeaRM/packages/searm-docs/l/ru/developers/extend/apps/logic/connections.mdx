...
* Используйте `serverVariables` (не `applicationVariables`) — учетные данные OAuth являются общими для сервера, и на каждом сервере SeaRM используется одно приложение OAuth.
...
4. Откройте установленное приложение в SeaRM под учетной записью администратора сервера → задайте значения в соответствующих `serverVariables`.
