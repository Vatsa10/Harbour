...
icon: laptop-code
description: Руководство для участников (или любопытных разработчиков), которые хотят запускать SeaRM локально.
---
...
Прежде чем установить и использовать SeaRM, убедитесь, что у вас установлено следующее:
...
SeaRM требует кэша Redis для обеспечения наилучшей производительности.
...
## Шаг 8: Использовать SeaRM

**Frontend**

Фронтенд SeaRM будет работать на [http://localhost:3001](http://localhost:3001).
Вы можете войти, используя учетную запись демо по умолчанию: `tim@apple.dev` (пароль: `tim@apple.dev`)

**Backend**

* Сервер SeaRM будет работать на [http://localhost:3000](http://localhost:3000).
