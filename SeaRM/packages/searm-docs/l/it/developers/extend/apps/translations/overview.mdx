...
Le app di SeaRM sono scritte in **inglese**: le stringhe nel tuo sorgente e nel manifest
...
Importa gli helper di traduzione da `searm-sdk/front-component`:
...
`searm dev:build` compila i cataloghi e serve la lingua corretta per l’utente
...
eseguire nuovamente `searm dev:build` (e ridistribuire), come per qualsiasi altra modifica.

<Note>
Le traduzioni vengono compilate da `searm dev:build` (e `searm apply`). La modalità watch continua di `searm dev` mostra le stringhe sorgente, quindi testa l’output localizzato
