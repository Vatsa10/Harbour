...
<Note>
Per l’iterazione locale quotidiana vuoi quasi sempre `yarn searm dev`. Il deploy e la pubblicazione servono a distribuire le release, **non** per il ciclo locale.
</Note>

| Vuoi…                                                       | Comando                             | Note                                                                                                                                   |
| ----------------------------------------------------------- | ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| Iterare in locale con sincronizzazione in tempo reale       | `yarn searm dev`                   | Monitora i file e sincronizza a ogni modifica.                                                                                         |
| Sincronizzare una volta e uscire (CI, script, hook)         | `yarn searm apply`                 | Esegue una build + sincronizzazione, poi termina. Aggiungi `--force` per saltare la conferma delle modifiche distruttive.              |
| Visualizzare in anteprima le modifiche **senza applicarle** | `yarn searm plan`                  | Calcola e stampa il diff; non scrive nulla.                                                                                            |
| Rimuovere l'app dallo spazio di lavoro                      | `yarn searm app:uninstall`         | Aggiungi `--yes` per saltare il prompt.                                                                                                |
| Inviare un tarball a un server                              | `yarn searm app:publish --private` | Richiede una versione di `package.json` **strettamente superiore** — vedi [Publishing](/l/it/developers/extend/apps/operations/publishing). |
| Pubblicare nel marketplace (npm)                            | `yarn searm app:publish`           | —                                                                                                                                      |
| Installare / aggiornare una versione distribuita            | `yarn searm app:install`           | Installa la versione attualmente distribuita.                                                                                          |
| Pulire il server locale e ripartire da zero                 | `yarn searm docker:reset`          | Elimina **tutti** i dati locali — ultima risorsa.                                                                                      |

<Note>
`yarn searm dev --once` e `yarn searm dev --once --dry-run` funzionano ancora come alias deprecati per `yarn searm apply` e `yarn searm plan`.
</Note>

### La sincronizzazione locale non richiede un incremento di versione

La regola della `version` strettamente crescente (`VERSION_ALREADY_EXISTS` in fase di deploy, `APP_ALREADY_INSTALLED` / `CANNOT_DOWNGRADE_APPLICATION` in fase di installazione) si applica a **`app:publish` / `app:install`** — il percorso di release. `yarn searm dev` sincronizza il tuo manifest in-place e non richiede mai una modifica di versione, quindi non devi toccare `package.json` per iterare. Se ti ritrovi ad aumentare la versione per testare una modifica locale, stai usando il percorso di release quando invece vuoi il ciclo di sviluppo.
...
`yarn searm plan` crea il tuo manifest, chiede al server il piano di migrazione e lo stampa — **senza applicare nulla**. È il modo sicuro per rispondere a "cosa cambierebbe questa sincronizzazione?" prima di impegnarti ad applicarla.
...
<Note>
Un piano mostra in anteprima solo le modifiche ai **metadati** e richiede che l'app sia stata sincronizzata almeno una volta (così lo spazio di lavoro la conosce). Se lo esegui su un'app che non è mai stata sincronizzata, il server segnala che l'app non è installata — esegui prima `yarn searm dev` una volta.
</Note>
...
1. **Nuova sincronizzazione.** Esegui di nuovo `yarn searm apply`. Le sincronizzazioni sono idempotenti — rieseguire un manifest pulito è sicuro e spesso risolve un problema temporaneo.
2. **Visualizza in anteprima il piano.** Esegui `yarn searm plan` per vedere esattamente cosa intende cambiare la prossima sincronizzazione, senza applicarlo.
...
4. **Disinstalla e reinstalla.** `yarn searm app:uninstall`, poi sincronizza di nuovo (`yarn searm dev`). Questo ricostruisce i metadati dell'app partendo da zero, lasciando intatto il resto del tuo spazio di lavoro.
5. **Ripristino completo (ultima risorsa).** `yarn searm docker:reset`, poi esegui di nuovo il seeding e la sincronizzazione.

<Warning>
`yarn searm docker:reset` elimina **tutti** i dati nella tua istanza locale — ogni workspace, record e app. Usalo solo quando i passaggi precedenti hanno fallito.
</Warning>

<Note>
Hai riscontrato un errore di metadati? Per favore [apri una issue](https://github.com/Vatsa10/Harbour/issues/new/choose) e includi il messaggio di migrazione non riuscita (con il tipo di metadato e lo `universalIdentifier`), l'output `Metadata changes` della sincronizzazione e i comandi che hai eseguito.
</Note>
