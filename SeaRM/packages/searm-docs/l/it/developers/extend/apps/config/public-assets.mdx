...
* **Inclusi nelle build** — `yarn searm dev:build` raggruppa tutti gli asset pubblici nell'output di distribuzione.

## Accedere agli asset pubblici con `getPublicAssetUrl`

Usa l'helper `getPublicAssetUrl` da `searm-sdk` per ottenere l'URL completo di un file nella tua directory `public/`. Funziona sia nelle funzioni logiche che nei componenti front-end.
