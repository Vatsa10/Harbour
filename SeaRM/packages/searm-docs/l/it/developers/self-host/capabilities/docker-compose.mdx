...
Questa guida fornisce istruzioni passo passo per installare e configurare l'applicazione SeaRM usando Docker Compose. L'obiettivo è rendere il processo semplice ed evitare gli errori comuni che potrebbero compromettere il tuo setup.
...
Installa l'ultima versione stabile di SeaRM con un unico comando:
...
Se ospiti searmCRM sul tuo computer, apri il browser e naviga a [http://localhost:3000](http://localhost:3000).
...
### Esponi SeaRM per Accesso Esterno

Per impostazione predefinita, SeaRM gira su `localhost` alla porta `3000`. Per accedervi tramite un dominio esterno o indirizzo IP devi configurare il `SERVER_URL` nel tuo file `.env`.
...
Raccomandiamo fortemente di configurare SeaRM dietro un proxy inverso con terminazione SSL per una sicurezza e funzionalità ottimali.
...
<Tip>
  Preferisci non gestire e mantenere SeaRM in autonomia? [Trova un partner SeaRM certificato](https://searm.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) per distribuire, ospitare e gestire la tua istanza end-to-end. *(Oppure contatta il nostro team: [contact@searm.com](mailto:contact@searm.com).)*
</Tip>
