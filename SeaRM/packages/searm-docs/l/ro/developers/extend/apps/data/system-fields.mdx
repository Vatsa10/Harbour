...
description: Rezolvă identificatorii universali determinați ai metadatelor pe care SeaRM le provizionează automat pe fiecare obiect, astfel încât aplicația ta să le poată referenția fără hardcodare.
icon: gears
---

Fiecare obiect din SeaRM vine cu **metadate de sistem** pe care nu le declari niciodată tu însuți, cum ar fi un set de câmpuri și o vizualizare principală de listă cu coloanele sale. Serverul le creează pe toate atunci când obiectul este provizionat, iar setul crește pe măsură ce SeaRM evoluează.

Deoarece nu îl declari, nu există nicio constantă `universalIdentifier` pe care să o imporți. În schimb, serverul **derivează** fiecare identificator într-un mod determinist, iar `searm-sdk` expune aceeași derivare astfel încât manifestul tău să poată rezolva exact valoarea pe care o folosește serverul.
...
<Note>
  `getFieldUniversalIdentifier` este disponibil începând cu `searm-sdk` 2.21.
</Note>
...
<Note>
  `getSystemRelationFieldUniversalIdentifier` este disponibil începând cu versiunea 2.23 a `searm-sdk` și necesită un server SeaRM pe versiunea 2.23 sau o versiune ulterioară.
</Note>
...
<Note>
  ...
  sunt disponibile începând cu versiunea 2.26 a `searm-sdk` și necesită un server SeaRM pe
...
Observă primul argument: o coloană este plasată în spațiul de nume al aplicației care deține **câmpul pe care îl afișează**, nu al celei care deține vizualizarea. Un câmp pe care aplicația ta îl adaugă la un obiect standard își obține coloana derivată sub aplicația ta, pe o vizualizare deținută de SeaRM.
...
## Obiecte standard SeaRM

Pentru un obiect SeaRM **standard** (Person, Company, Opportunity, …), nu trebuie să derivezi nimic: identificatorii sunt constante pre-calculate pe care le poți importa direct, atât pentru câmpuri, cât și pentru vizualizări.
