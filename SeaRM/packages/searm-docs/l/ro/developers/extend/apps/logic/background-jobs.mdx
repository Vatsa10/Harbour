...
description: Predă munca de durată sau supusă limitării ratei către lucrătorii SeaRM, punând în coadă o altă execuție a unei funcții logice, în loc să rulezi totul inline.
icon: layer-group
---
...
`enqueueJob` face exact asta: le cere lucrătorilor SeaRM să ruleze una dintre funcțiile de logică ale aplicației tale mai târziu, în propriul său proces, cu propriul său buget de timeout. Apelantul returnează imediat.
...
Importă `enqueueJob` din `searm-sdk/logic-function` și indică-l către `universalIdentifier` al funcției logice pe care vrei să o rulezi.
...
| ------------ | -------- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `retryLimit` | `0`      | `0`–`10`                 | Încercări suplimentare dacă rularea aruncă o eroare. Crește această valoare numai pentru handleri care sunt siguri de rulat de două ori. |
