...
Componentele front-end pot fi afișate în trei locații în cadrul SeaRM:
...
După sincronizarea cu `yarn searm dev` (sau prin rularea o singură dată a comenzii `yarn searm apply`), acțiunea rapidă apare în colțul din dreapta sus al paginii:
...
Componenta este afișată ca o secțiune în interiorul filei Settings, nu ca un înlocuitor pentru întreaga filă. Secțiunile gestionate de sistem ale SeaRM — actualizare automată, URL aplicație și conexiuni — sunt întotdeauna afișate deasupra și nu pot fi suprascrise de aplicație.
...
Deoarece componenta returnează `null`, SeaRM omite redarea unui container pentru ea — nu apare spațiu gol în layout. Componenta are în continuare acces la toate hook-urile și la API-ul de comunicare cu gazda.

## Componentele Command din SDK

Pachetul `searm-sdk` oferă patru componente ajutătoare Command, concepute pentru componente front-end headless. Fiecare componentă execută o acțiune la montare, gestionează erorile afișând o notificare snackbar și demontează automat componenta front-end la final.

Importați-le din `searm-sdk/front-component`:
...
O funcție logică declarată cu `httpRouteTriggerSettings` este accesibilă prin HTTP la ruta sa. `RestApiClient` tratează căile care încep cu `/s/` ca rute ale aplicației, le rezolvă către URL-ul de la care sunt deservite funcțiile tale și le autentifică folosind `SEARM_APP_ACCESS_TOKEN`.

> **În SeaRM Cloud, funcțiile logice declanșate prin HTTP sunt deservite pe un domeniu dedicat pentru fiecare spațiu de lucru** la `https://\<your-workspace-subdomain>.withsearm.com\<path>`. Pentru apelanții externi, copiază URL-ul exact din setările **HTTP trigger** ale funcției sau din fila **Settings** a aplicației.
...
Calea transmisă către `RestApiClient` este proprietatea `httpRouteTriggerSettings.path` a funcției logice, cu prefixul `/s`. Păstrează `isAuthRequired: true`; `SEARM_APP_ACCESS_TOKEN` pe care SeaRM îl generează pentru componenta ta autentifică cererea:
...
<Note>
`SEARM_APP_ACCESS_TOKEN` este injectat automat — vezi [Application variables](#application-variables). Deoarece variabilele de aplicație secrete nu sunt niciodată expuse componentelor de front, păstrează cheile API și altă logică sensibilă în funcția logică, nu în componenta de front.
</Note>

### Apelarea API-ului REST SeaRM

Pentru a apela rute HTTP ale aplicației sau pentru a citi și scrie înregistrări SeaRM dintr-un front component, folosește `RestApiClient` din `searm-client-sdk/rest`. Trimite căile de forma `/s/...` către URL-ul de bază al funcțiilor spațiului tău de lucru, iar orice altă cale, inclusiv `/rest/...`, către `SEARM_API_URL`.
...
| Variabilă                 | Descriere                                                |
| ------------------------- | -------------------------------------------------------- |
| `SEARM_API_URL`          | URL-ul de bază al API-ului de bază SeaRM                |
| `SEARM_APP_ACCESS_TOKEN` | Token cu durată scurtă, limitat la rolul aplicației dvs. |

### `SEARM_FUNCTIONS_URL`

SeaRM injectează, de asemenea, `SEARM_FUNCTIONS_URL` în front components și în funcțiile logice: URL-ul de bază de la care sunt deservite funcțiile logice ale aplicației tale declanșate prin HTTP.

Există deoarece acel URL nu este întotdeauna chiar serverul SeaRM. În SeaRM Cloud, rutele aplicației sunt deservite pe un domeniu dedicat pentru fiecare spațiu de lucru (`https://\<your-workspace-subdomain>.withsearm.com` sau domeniul public principal al aplicației atunci când este configurat unul), astfel încât răspunsurile generate de aplicație să ruleze pe o origine izolată, nu pe originea aplicației SeaRM. Instanțele self-hosted și locale deservesc rutele aplicației sub prefixul `/s` chiar pe server și este posibil să nu seteze deloc variabila. Deoarece URL-ul de bază variază în funcție de spațiul de lucru și de instanță, codul tău nu îl poate hardcoda — serverul injectează valoarea corectă la runtime.

Rareori ai nevoie să o citești direct. Apelează-ți rutele prin `RestApiClient` folosind o cale prefixată cu `/s/`, iar clientul îți rezolvă URL-ul: elimină prefixul `/s` și țintește `SEARM_FUNCTIONS_URL`, folosind `\<SEARM_API_URL>/s` ca rezervă atunci când variabila nu este setată. Folosește `resolveUrl('/s/\<path>')` pentru a obține URL-ul absolut fără a trimite o cerere, de exemplu pentru un link. Citește variabila direct doar atunci când construiești manual un URL:
...
Componentele front-end pot declanșa navigare, ferestre modale și notificări folosind funcții din `searm-sdk`:
...
* **Componente UI SeaRM** — biblioteca proprie de componente a SeaRM; vezi [Folosirea componentelor UI SeaRM](#using-searm-ui-components) mai jos
...
## Folosirea componentelor UI SeaRM

SeaRM livrează biblioteca sa de componente ca pachetul [`searm-ui`](https://www.npmjs.com/package/searm-ui/v/1.0.0-alpha.1). Componentele frontend îl pot folosi pentru butoane, etichete, pastile de stare, chips, avataruri, pictograme, tipografie și tokeni de temă care se potrivesc automat cu tema luminoasă și întunecată a spațiului de lucru.

### Instalare

Adaugă pachetul în aplicația ta, fixat la versiunea cu care este livrată instanța ta de SeaRM:

```bash
yarn add searm-ui@1.0.0-alpha.1
```

`searm-ui` este inclus în componenta ta frontend la momentul build-ului, astfel încât trebuie să fie doar o dependență a aplicației tale — nu este nimic de configurat la runtime.
...
| Subpath                     | Ce exportă                                         |
| --------------------------- | -------------------------------------------------- |
| `searm-ui/input`           | `Button` și câmpuri de formular                    |
| `searm-ui/data-display`    | `Tag`, `Status`, `Chip`, `Avatar` și altele        |
| `searm-ui/feedback`        | `Callout`, `Banner`, `Info` și altele              |
| `searm-ui/typography`      | `H1Title`, `H2Title`, `H3Title`, `Label` și altele |
| `searm-ui/icon`            | Componente `Icon*` (de ex. `IconCheck`)            |
...
Importă pictograme individuale din `searm-ui/icon`:
...
Componentele SeaRM UI se potrivesc automat cu tema luminoasă și întunecată a spațiului de lucru — renderer-ul aplică schema de culori activă pe gazdă, iar componentele își determină culorile în funcție de aceasta.

Pentru a folosi aceiași tokeni de design în propriile tale stiluri inline, apelează hook-ul `useTheme()`. Acesta returnează tokenii de temă ai SeaRM (spațiere, culori, raze, fonturi) conectați la tema activă, fără a necesita vreo configurare `ThemeProvider` în componenta ta:
...
Pentru a ramifica explicit în funcție de schema activă, citește-o cu `useColorScheme()` din `searm-sdk/front-component`, care returnează `'light'` sau `'dark'`.
...
Dacă unul dintre aceste lucruri te blochează, [deschide un tichet](https://github.com/Vatsa10/Harbour/issues/new/choose) ca să fie prioritar.
...
CSS-ul componentei, fie din `import './styles.css'`, CSS-in-JS sau un element `\<style>`, este injectat în `\<head>` al paginii gazdă **fără scope**. Astfel numele de clase intră în coliziune cu cele ale SeaRM (prefixează-le și nu scrie niciodată un selector simplu `div { ... }`), iar `@media` se potrivește cu fereastra browserului, nu cu widgetul tău (folosește `@container` cu propriul tău `container-type`). Proprietățile `style` inline nu sunt afectate.
...
* Apelurile către SeaRM API și către rutele aplicației tale sunt proxate de gazdă, așa că preferă [`RestApiClient`](#calling-the-searm-rest-api). La apelurile proxate, `AbortSignal` și celelalte opțiuni `RequestInit` sunt eliminate, iar doar corpurile de tip `string` și `URLSearchParams` sunt acceptate.
...
* `fetch('/rest/people')` nu este niciodată asociat cu SeaRM API, deoarece sandbox-ul nu are un URL de pagină față de care să rezolve o cale relativă.
