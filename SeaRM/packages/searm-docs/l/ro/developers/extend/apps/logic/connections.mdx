...
* Folosește `serverVariables` (nu `applicationVariables`) — acreditările OAuth sunt la nivel de server și există o singură aplicație OAuth pentru fiecare server SeaRM.
...
4. Deschide aplicația instalată în SeaRM ca administrator de server → setează valorile pe `serverVariables` corespunzătoare.
