---
title: Concepte
description: Cum funcționează aplicațiile SeaRM — modelul de entități, izolarea (sandboxing) și ciclul de viață al instalării.
icon: sitemap
---

Aplicațiile SeaRM sunt pachete TypeScript care vă extind spațiul de lucru cu obiecte personalizate, logică, componente UI și capabilități AI. Acestea rulează pe platforma SeaRM, cu izolare completă (sandboxing) și controale de permisiuni.

## Cum funcționează aplicațiile

O aplicație este o colecție de **entități** declarate folosind funcțiile `defineEntity()` din pachetul `searm-sdk`. SDK-ul detectează aceste declarații prin analiză AST în timpul construirii și produce un **manifest** — o descriere completă a ceea ce aplicația dvs. adaugă unui spațiu de lucru. Aceste funcții validează configurația în timpul build-ului și oferă completare automată în IDE și siguranța tipurilor.
...
| **Componentă front-end**          | Interfață React izolată (sandboxed) în SeaRM              | [Componente front-end](/l/ro/developers/extend/apps/layout/front-components)       |
...
* **Componentele front-end** rulează în Web Workers folosind Remote DOM — izolate de pagina principală, dar randând elemente DOM native (nu iframes). Acestea comunică cu SeaRM printr-un API al gazdei bazat pe transmiterea de mesaje.
* **Permisiunile** sunt aplicate la nivelul API-ului. Tokenul de rulare (`SEARM_APP_ACCESS_TOKEN`) este derivat din rolul definit în `defineApplication()`.
...
* **`yarn searm dev`** — monitorizează fișierele sursă și sincronizează în timp real modificările către un server SeaRM conectat. Clientul API tipizat este regenerat automat atunci când schema se schimbă.
* **`yarn searm dev:build`** — compilează TypeScript, împachetează funcțiile logice și componentele front-end cu esbuild și produce un manifest.
