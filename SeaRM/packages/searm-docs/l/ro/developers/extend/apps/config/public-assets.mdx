...
* **Incluse în build-uri** — `yarn searm dev:build` împachetează toate resursele publice în outputul de distribuție.

## Accesarea resurselor publice cu `getPublicAssetUrl`

Utilizați helperul `getPublicAssetUrl` din `searm-sdk` pentru a obține URL-ul complet al unui fișier din directorul `public/`. Funcționează atât în funcții logice, cât și în componente frontend.
