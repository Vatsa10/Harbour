...
description: Ce conține o aplicație SeaRM generată — fișiere, foldere și ce face fiecare.
icon: folder-tree
---

O aplicație nouă generată de `npx create-searm-app` arată astfel:
...
Ambele pachete SeaRM SDK trebuie plasate sub `devDependencies`, nu sub `dependencies`:
...
Generatorul de schelete fixează versiunile `searm-sdk` și `searm-client-sdk` la propria sa versiune — păstrează-le sincronizate când faci upgrade.

* **`searm-sdk`** livrează CLI-ul `searm` și uneltele de build/scaffolding. Acesta rulează doar în timpul dezvoltării și al build-ului și nu este niciodată importat de runtime-ul aplicației tale publicate.
* **`searm-client-sdk`** este importat de codul aplicației tale (`CoreApiClient`, `MetadataApiClient`, `RestApiClient`), dar SeaRM îl furnizează la runtime — funcțiile de logică îl obțin dintr-un strat SDK generat, iar componentele de interfață îl rezolvă din module livrate de server. Copia instalată local este folosită doar pentru verificarea tipurilor și pentru build-ul la momentul de deploy, astfel că nu trebuie niciodată inclusă în bundle-ul livrat.

Păstrarea oricărui pachet sub `dependencies` îl include în bundle-ul de runtime al aplicației instalate, unde reprezintă o încărcătură inutilă. `searm dev:build` emite un avertisment atunci când oricare dintre ele este încă listat sub `dependencies`.
