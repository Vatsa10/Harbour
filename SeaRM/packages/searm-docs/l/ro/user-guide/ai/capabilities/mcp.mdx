---
title: Server MCP
description: Conectați asistenți AI la spațiul dvs. de lucru SeaRM folosind Model Context Protocol.
---

SeaRM expune un server [MCP](https://modelcontextprotocol.io/) astfel încât asistenții AI — Claude Desktop, Claude Code, Cursor, ChatGPT și alții — să poată citi și scrie datele tale din CRM prin limbaj natural.

Folosește **URL-ul spațiului de lucru** (URL-ul pe care îl folosești pentru a accesa SeaRM) drept punct final MCP. Pe SeaRM Cloud, URL-ul spațiului tău de lucru poate fi `https://{mycompany}.searm.com` sau un domeniu personalizat. Serverul este disponibil la:

| Mediu                | Punct final MCP                                                                |
| -------------------- | ------------------------------------------------------------------------------ |
| **Cloud**            | `https://{your-workspace-url}/mcp` (de ex. `https://mycompany.searm.com/mcp`) |
...
Adaugă asta în configurația clientului tău MCP, înlocuind `{your-workspace-url}` cu gazda spațiului tău de lucru (de ex. `mycompany.searm.com`):
...
1. Va descoperi metadatele OAuth ale SeaRM prin `/.well-known/oauth-protected-resource` și `/.well-known/oauth-authorization-server`
...
Mergi la **Settings → MCP & APIs → MCP** în SeaRM. Alege metoda de autentificare (OAuth sau Cheie API), copiază fragmentul JSON (va folosi deja URL-ul spațiului tău de lucru) și lipește-l în fișierul de configurare al clientului tău MCP.
...
Repornește clientul tău MCP (sau reîncarcă configurația). Dacă folosești OAuth, vei fi redirecționat către SeaRM pentru a autoriza accesul. Dacă folosești o cheie API, conexiunea este imediată.
...
După conectare, serverul MCP expune instrumente care oglindesc API-ul SeaRM. Fluxul de lucru recomandat este:
...
Pentru instanțele auto-găzduite, înlocuiește `{your-workspace-url}` cu URL-ul serverului tău. Asigură-te că `SERVER_URL` din mediul tău corespunde URL-ului public al instanței tale SeaRM — acesta este folosit pentru a genera metadatele de descoperire OAuth.
