...
title: Aplicaciones
icon: cube
description: "Amplía SeaRM con código: objetos personalizados, lógica del lado del servidor, componentes de IU y agentes de IA, todo como paquetes de TypeScript."
---

La mayoría de los CRM te dan un panel de configuración. SeaRM te da una plataforma. Las aplicaciones son la forma en que los desarrolladores amplían SeaRM más allá de lo que ofrece la IU: definen modelos de datos, lógica del lado del servidor, componentes de IU y capacidades de IA como código, y luego los implementan en uno o más espacios de trabajo.
...
Una aplicación es un paquete de TypeScript que declara **entidades** usando `searm-sdk`:
...
| **Componentes de frontend** | Componentes de React aislados (sandboxed) que se renderizan dentro de la interfaz de SeaRM (panel lateral, widgets, menú de comandos) |
...
Escribes tu app como un proyecto de TypeScript en tu máquina. La CLI observa tus archivos fuente y los sincroniza en tiempo real con un servidor de SeaRM en ejecución: edita un archivo y ve el cambio en la interfaz en cuestión de un segundo. El cliente de API tipado se regenera automáticamente cuando cambia el esquema. Cuando estés listo, `yarn searm app:publish --private` envía la app a un servidor de producción, o `yarn searm app:publish` publica tu app en npm y en el marketplace de SeaRM.
