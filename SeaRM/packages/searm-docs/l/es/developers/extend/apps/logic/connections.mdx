...
* Usa `serverVariables` (no `applicationVariables`) — las credenciales de OAuth son a nivel de servidor y hay una aplicación OAuth por servidor de SeaRM.
...
4. Abre la aplicación instalada en SeaRM como administrador del servidor → establece los valores en los `serverVariables` correspondientes.
