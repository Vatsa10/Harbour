...
Los webhooks envían datos a tus sistemas en tiempo real cuando ocurren eventos en SeaRM — no se requiere sondeo. Úsalos para mantener sincronizados los sistemas externos, activar automatizaciones o enviar alertas.
...
SeaRM envía webhooks para estos tipos de eventos:
...
SeaRM firma cada solicitud de webhook por seguridad. Valida las firmas para garantizar que las solicitudes sean auténticas.

### Encabezados

| Encabezado                   | Descripción                     |
| ---------------------------- | ------------------------------- |
| `X-SeaRM-Webhook-Signature` | Firma HMAC SHA256               |
| `X-SeaRM-Webhook-Timestamp` | Marca de tiempo de la solicitud |

### Pasos de validación

1. Obtén la marca de tiempo de `X-SeaRM-Webhook-Timestamp`
2. Crea la cadena: `{timestamp}:{JSON payload}`
3. Calcula HMAC SHA256 usando tu secreto de webhook
4. Compara con `X-SeaRM-Webhook-Signature`
...
| **Disparador de webhook de flujo de trabajo** | ENTRADA   | Recibir datos en SeaRM desde sistemas externos                                   |
