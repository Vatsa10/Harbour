...
El padre (lado "uno") debe existir en SeaRM antes de poder hacer referencia a él.
...
1. Exporte los registros padre desde SeaRM
...
Use `https://domain.com` (no solo `domain.com`). Esto coincide con cómo SeaRM almacena los dominios de las Empresas y evita errores de coincidencia.
</Warning>
...
SeaRM vinculará automáticamente cada registro hijo con el padre correspondiente.
...
| **Registros inexistentes**                | El registro padre no existe en SeaRM                | Importe primero los registros padre o verifique si hay errores tipográficos |
| **Diferencia de mayúsculas y minúsculas** | `Acme.com` en el archivo pero `acme.com` en SeaRM   | Asegure una coincidencia exacta de mayúsculas y minúsculas                  |
...
<AccordionGroup>
  <Accordion title="¿Qué pasa si mi registro padre no tiene dominio ni correo electrónico?">
    Tiene dos opciones:
    1. Use el `id` de SeaRM (exporte los registros padre para obtener sus IDs)
