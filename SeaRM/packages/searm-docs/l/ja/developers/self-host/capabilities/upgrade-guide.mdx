...
1. SeaRMを停止：`docker compose down`
2. `docker-compose.yml` と同じディレクトリにある `.env` ファイルの `TAG` 値を変更します
3. SeaRMを起動：`docker compose up -d`

サーバーは起動時に、必要なアップグレード用のマイグレーションを自動的に実行します。 手動のコマンドは不要です。

## クロスバージョンのアップグレード (v1.23+)

**v1.23** 以降、SeaRM はクロスバージョンのアップグレードをサポートします。 サポートされている任意のバージョンから、中間バージョンを段階的に経ることなく、最新リリースへ直接移行できます。

たとえば、v1.23 から v2.0 へ直接アップグレードすることが完全にサポートされています。

## v2.5 以降へのアップグレード — 保存データ暗号化エンベロープ

**v2.5** 以降、SeaRM は保存時のシークレット（OAuth トークン、アプリケーション変数、署名用の秘密鍵、機密な設定値、TOTP シークレット）を、バージョン付きの `enc:v2:` エンベロープ内に格納し、`ENCRYPTION_KEY`（`ENCRYPTION_KEY` が未設定の場合は `APP_SECRET`）で暗号化します。
