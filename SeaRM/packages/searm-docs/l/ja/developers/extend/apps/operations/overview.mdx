...
**オペレーションレイヤー** とは、アプリを使って *何かをする* のではなく、アプリに対して *行う* あらゆる操作を指します。具体的には、CLI コマンドの実行、実際の SeaRM サーバーに対する統合テストの実行、CI の設定、そしてリリースの提供です — 単一サーバーにデプロイする tarball として、またはマーケットプレイスに掲載される npm パッケージとして行われます。
...
<CardGroup cols={2}>
  <Card title="CLI" icon="terminal" href="/l/ja/developers/extend/apps/operations/cli">
    `yarn searm` リファレンス — exec、logs、uninstall、remotes。
  </Card>
