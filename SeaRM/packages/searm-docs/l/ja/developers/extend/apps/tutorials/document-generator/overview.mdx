...
description: 実際の SeaRM アプリを構築し、CRM データからパーソナライズされたドキュメントを生成しましょう。
---
...
ローカルの SeaRM サーバーがポート `2020` で動作しており、CLI がそのサーバーに対して認証済みになっている状態です。
...
[`packages/searm-apps/examples/document-generator`](https://github.com/Vatsa10/Harbour/tree/main/packages/searm-apps/examples/document-generator) にあります。
...
チュートリアル全体を通して、ターミナルで `yarn searm dev` を実行したままにしておいてください。 `src/` 配下のファイルを追加または編集するたびに、数秒以内にサーバーへ再同期されるため、ビルドしながら各機能が UI に現れていく様子を確認できます。
