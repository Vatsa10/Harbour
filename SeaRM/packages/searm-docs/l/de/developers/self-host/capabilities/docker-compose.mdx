...
Diese Anleitung enthält Schritt-für-Schritt-Anweisungen, um die SeaRM-Anwendung mit Docker Compose zu installieren und zu konfigurieren. Das Ziel ist, den Prozess übersichtlich zu gestalten und häufige Fallstricke zu vermeiden, die Ihre Einrichtung unterbrechen könnten.
...
Installieren Sie die neueste stabile Version von SeaRM mit einem einzigen Befehl:
...
Wenn Sie searmCRM auf Ihrem eigenen Computer hosten, öffnen Sie Ihren Browser und navigieren Sie zu [http://localhost:3000](http://localhost:3000).
...
### SeaRM für externen Zugriff freigeben

Standardmäßig läuft SeaRM auf `localhost` an Port `3000`. Um über eine externe Domain oder IP-Adresse darauf zuzugreifen, müssen Sie die `SERVER_URL` in Ihrer `.env`-Datei konfigurieren.
...
SSL (HTTPS) ist erforderlich, damit bestimmte Browserfunktionen ordnungsgemäß funktionieren. Obwohl diese Funktionen möglicherweise während der lokalen Entwicklung funktionieren (da Browser localhost anders behandeln), ist eine ordnungsgemäße SSL-Einrichtung erforderlich, wenn SeaRM auf einer regulären Domain gehostet wird.
...
Wir empfehlen dringend, SeaRM hinter einem Reverse-Proxy mit SSL-Beendigung für optimale Sicherheit und Funktionalität zu konfigurieren.
...
<Tip>
  Möchten Sie SeaRM lieber nicht selbst betreiben und warten? [Finden Sie einen zertifizierten SeaRM-Hosting-Partner](https://searm.com/partners/list?categories=HOSTING\&ref=docs-docker-compose), der Ihre Instanz Ende-zu-Ende bereitstellt, hostet und verwaltet. *(Oder wenden Sie sich an unser Team: [contact@searm.com](mailto:contact@searm.com).)*
</Tip>
