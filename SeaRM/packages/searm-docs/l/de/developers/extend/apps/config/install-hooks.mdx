...
* Die übliche Logikfunktions-Umgebung (`APPLICATION_ID`, `APP_ACCESS_TOKEN`, `API_URL`) wird injiziert, sodass du die SeaRM-API mit dem Token deiner App aufrufen kannst.
...
* **Wird im Dev-Modus nicht ausgeführt**: `yarn searm dev` überspringt den Installations-Flow und synchronisiert Dateien direkt, sodass Hooks dort nie ausgeführt werden. Führe sie stattdessen manuell aus:
