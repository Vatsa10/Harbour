...
* Verwenden Sie `serverVariables` (nicht `applicationVariables`) — OAuth-Anmeldedaten gelten serverweit und es gibt eine OAuth-App pro SeaRM-Server.
...
4. Öffnen Sie die installierte App in SeaRM als Serveradministrator → setzen Sie die Werte in den entsprechenden `serverVariables`.
