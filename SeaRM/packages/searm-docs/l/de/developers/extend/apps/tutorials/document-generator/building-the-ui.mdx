...
Komponente mit Sandkasten in SeaRM. Wir liest den ausgewählten Datensatz ein, lädt die
...
`searm-ui` importiert. Das SDK verspottet das Paket während des Builds, so dass Modul-Level-Importe von
Theme-Konstanten `undefiniert` wären. Siehe
[vollständige Komponente](https://github.com/Vatsa10/Harbour/blob/main/packages/searm-apps/examples/document-generator/src/front-components/generate-document-form.front-component.tsx).
</Warning>
...
Styles über einen kleinen [`Markdown`](https://github.com/Vatsa10/Harbour/blob/main/packages/searm-apps/examples/document-generator/src/utils/markdown-to-react.tsx)
Helfer machen.
...
`RICH_TEXT`-Feld ist, stellt SeaRM bereits einen vollständigen Rich-Text-Editor dafür bereit – denselben, den auch die Standardobjekte Note und Task verwenden. Wir Oberflächen es nur auf der
...
[`template-record. age-layout.ts`](https://github.com/Vatsa10/Harbour/blob/main/packages/searm-apps/examples/document-generator/src/page-layouts/template-record.page-layout.ts).
