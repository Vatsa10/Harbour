...
* **In Builds enthalten** — `yarn searm dev:build` bündelt alle öffentlichen Assets in der Distributionsausgabe.

## Zugriff auf öffentliche Assets mit `getPublicAssetUrl`

Verwenden Sie den Helper `getPublicAssetUrl` aus `searm-sdk`, um die vollständige URL einer Datei in Ihrem `public/`-Verzeichnis zu erhalten. Dies funktioniert sowohl in Logikfunktionen als auch in Frontend-Komponenten.
