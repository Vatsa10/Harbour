...
description: Podrobný průvodce krok za krokem pro migraci vašich dat z libovolného CRM do SeaRM.
---

## Přehled

Tento průvodce vás provede migrací vašich dat z libovolného CRM do SeaRM. Proces zahrnuje audit vašich dat, přípravu vašeho pracovního prostoru SeaRM, export z vašeho současného systému a import do SeaRM.
...
Vytvořte mapovací dokument mezi vaším současným CRM a SeaRM:
...
## Krok 3: Nastavte svůj pracovní prostor SeaRM

Před importem dat připravte svůj pracovní prostor SeaRM:
...
Pokud vaše data obsahují odkazy na uživatele (vlastník účtu, přiřazený uživatel apod.), tito uživatelé musí v SeaRM existovat před importem. Jinak tyto relace nelze namapovat.
</Warning>
...
Otevřete každý exportovaný soubor CSV v tabulkovém editoru a připravte jej pro SeaRM.

### Odstraňte duplicity

1. Seřaďte podle jedinečného pole (e-mail u lidí, doména u společností)
2. Odstraňte nebo sloučte duplicitní řádky
3. Ověřte, že v SeaRM již neexistují žádné duplicity
...
Použijte `https://domain.com` (ne `domain.com` ani `www.domain.com`). To odpovídá formátu SeaRM a předchází duplicitám, když připojíte synchronizaci e-mailu/kalendáře.
</Warning>
...
2. Použijte stejné e-mailové adresy, které uživatelé použili při připojení do vašeho pracovního prostoru SeaRM
...
* SeaRM začne synchronizovat e-maily k záznamům kontaktů
...
<AccordionGroup>
  <Accordion title="Mohu migrovat pracovní postupy automaticky?">
    V tuto chvíli ne. Pracovní postupy je nutné v SeaRM znovu vytvořit ručně.
  </Accordion>
...
<Tip>
  Chcete pomoc s migrací? [Najít certifikovaného dvaceti partnerů](https://searm.com/partners/list?categories=SOLUTIONING\&ref=docs-migrate-crm) pro přesun vašich dat, plynovodů a uživatelů od konce ke konci.
</Tip>
