...
Nadřazený („jedna“ strana) musí v SeaRM existovat, než na něj budete moci odkazovat.
...
1. Exportujte nadřazené záznamy z SeaRM
...
Použijte `https://domain.com` (ne pouze `domain.com`). To odpovídá způsobu, jak SeaRM ukládá domény společností, a předchází chybám při párování.
</Warning>
...
SeaRM automaticky propojí každý podřízený záznam s odpovídajícím nadřazeným.
...
| **Neexistující záznamy**     | Nadřazený záznam v SeaRM neexistuje            | Nejprve importujte nadřazené záznamy nebo zkontrolujte překlepy |
| **Rozdílná velikost písmen** | `Acme.com` v souboru, ale v SeaRM `acme.com`   | Zajistěte přesnou shodu velikosti písmen                        |
...
<AccordionGroup>
  <Accordion title="Co když můj nadřazený záznam nemá doménu ani e-mail?">
    Máte dvě možnosti:
    1. Použijte SeaRM `id` (exportujte nadřazené záznamy, abyste získali jejich ID)
