...
Webhooky posílají data do vašich systémů v reálném čase, když v SeaRM dojde k událostem — bez potřeby průběžného dotazování. Použijte je k udržování externích systémů v synchronizaci, spouštění automatizací nebo zasílání upozornění.
...
SeaRM odesílá webhooky pro tyto typy událostí:
...
SeaRM kvůli bezpečnosti podepisuje každý požadavek webhooku. Ověřujte podpisy, abyste zajistili, že požadavky jsou autentické.

### Záhlaví

| Hlavička                     | Popis                    |
| ---------------------------- | ------------------------ |
| `X-SeaRM-Webhook-Signature` | Podpis HMAC SHA256       |
| `X-SeaRM-Webhook-Timestamp` | Časové razítko požadavku |

### Kroky ověření

1. Získejte časové razítko z `X-SeaRM-Webhook-Timestamp`
2. Vytvořte řetězec: `{timestamp}:{JSON payload}`
3. Vypočítejte HMAC SHA256 pomocí vašeho tajného klíče webhooku
4. Porovnejte s `X-SeaRM-Webhook-Signature`
...
| **Spouštěč webhooku v pracovním postupu** | IN   | Přijímejte data do SeaRM z externích systémů                   |
