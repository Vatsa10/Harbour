...
Tato příručka poskytuje podrobný návod k instalaci a konfiguraci aplikace SeaRM pomocí Docker Compose. Cílem je zjednodušit proces a zabránit běžným úskalím, která by mohla narušit vaše nastavení.
...
Nainstalujte nejnovější stabilní verzi SeaRM jedním příkazem:
...
Pokud hostujete searmCRM na svém vlastním počítači, otevřete svůj prohlížeč a přejděte na stránku [http://localhost:3000](http://localhost:3000).
...
### Otevření SeaRM pro Externí Přístup

Ve výchozím nastavení běží SeaRM na `localhost` na portu `3000`. Pro přístup přes externí doménu nebo IP adresu musíte nakonfigurovat `SERVER_URL` ve vašem `.env` souboru.
...
SSL (HTTPS) je potřebné pro správné fungování některých funkcí prohlížeče. I když tyto funkce mohou fungovat během místního vývoje (protože prohlížeče zacházejí s localhost jinak), správná konfigurace SSL je nutná při hostování SeaRM na běžné doméně.
...
Důrazně doporučujeme nastavit SeaRM za reverzní proxy se SSL ukončením pro optimální bezpečnost a funkčnost.
...
<Tip>
  Nechcete SeaRM provozovat a spravovat sami? [Najděte si certifikovaného hostingového partnera SeaRM](https://searm.com/partners/list?categories=HOSTING\&ref=docs-docker-compose), který pro vás zajistí nasazení, hosting a kompletní správu vaší instance od začátku do konce. *(Nebo kontaktujte náš tým: [contact@searm.com](mailto:contact@searm.com).)*
</Tip>
