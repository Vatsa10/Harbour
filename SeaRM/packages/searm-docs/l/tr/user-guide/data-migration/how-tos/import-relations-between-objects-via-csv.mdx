...
Ana ("bir" taraf) kaydı, ona referans verebilmeden önce SeaRM'de mevcut olmalıdır.
...
1. Ana kayıtları SeaRM'den dışa aktarın
...
`https://domain.com` kullanın (yalnızca `domain.com` değil). Bu, SeaRM'nin Şirket alan adlarını saklama biçimiyle eşleşir ve eşleştirme hatalarını önler.
</Warning>
...
SeaRM, her alt kaydı otomatik olarak eşleşen ana kayda bağlar.
...
| **Mevcut olmayan kayıtlar**      | Ana kayıt SeaRM'de mevcut değil               | Önce ana kayıtları içe aktarın veya yazım hatalarını kontrol edin |
| **Büyük/küçük harf uyumsuzluğu** | Dosyada `Acme.com`, SeaRM'de ise `acme.com`   | Bire bir büyük/küçük harf eşleşmesini sağlayın                    |
...
<AccordionGroup>
  <Accordion title="Ana kaydımın alan adı veya e-postası yoksa ne olur?">
    İki seçeneğiniz var:
    1. SeaRM `id` değerini kullanın (kimlikleri almak için ana kayıtları dışa aktarın)
