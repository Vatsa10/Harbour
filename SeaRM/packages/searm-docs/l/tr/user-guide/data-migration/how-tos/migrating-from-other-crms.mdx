...
description: Verilerinizi herhangi bir CRM'den SeaRM'ye taşımak için adım adım kılavuz.
---

## Genel Bakış

Bu kılavuz, verilerinizi herhangi bir CRM'den SeaRM'ye taşıma sürecinde size yol gösterir. Süreç, verilerinizi denetlemeyi, SeaRM çalışma alanınızı hazırlamayı, mevcut sisteminizden dışa aktarmayı ve SeaRM'ye içe aktarmayı içerir.
...
Mevcut CRM'iniz ile SeaRM arasında bir eşleme belgesi oluşturun:
...
## Adım 3: SeaRM Çalışma Alanınızı Kurun

Verileri içe aktarmadan önce SeaRM çalışma alanınızı hazırlayın:
...
Verileriniz kullanıcı referansları içeriyorsa (Hesap Sahibi, Atanan vb.), bu kullanıcıların içe aktarmadan önce SeaRM'de mevcut olması gerekir. Aksi halde bu ilişkiler eşlenemez.
</Warning>
...
Dışa aktarılan her CSV'yi bir hesap tablosu uygulamasında açın ve SeaRM için hazırlayın.
...
3. SeaRM'de halihazırda yinelenen olmadığını doğrulayın
...
`https://domain.com` kullanın (`domain.com` veya `www.domain.com` değil). Bu, SeaRM'nin biçimiyle eşleşir ve e-posta/takvim eşzamanlamasını bağladığınızda yinelenmeleri önler.
</Warning>
...
2. Kullanıcıların SeaRM çalışma alanınıza katılmak için kullandıklarıyla aynı e-posta adreslerini kullanın
...
* SeaRM, e-postaları kişi kayıtlarıyla eşzamanlamaya başlayacaktır
...
<AccordionGroup>
  <Accordion title="İş akışlarını otomatik olarak taşıyabilir miyim?">
    Şu anda mümkün değil. İş akışları SeaRM'de manuel olarak yeniden oluşturulmalıdır.
  </Accordion>
...
<Tip>
  Geçişiniz için yardıma mı ihtiyacınız var? Verilerinizi, pipeline'larınızı ve kullanıcılarınızı uçtan uca taşımak için [sertifikalı bir SeaRM iş ortağı bulun](https://searm.com/partners/list?categories=SOLUTIONING\&ref=docs-migrate-crm).
</Tip>
