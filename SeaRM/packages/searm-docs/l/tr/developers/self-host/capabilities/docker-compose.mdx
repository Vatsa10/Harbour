...
Bu kılavuz, Docker Compose kullanarak SeaRM uygulamasını kurmak ve yapılandırmak için adım adım yönergeler sunar. Amaç, süreci sadeleştirmek ve kurulumunuzu bozabilecek yaygın sorunları önlemektir.
...
Tek bir komutla en son kararlı SeaRM sürümünü yükleyin:
...
Eğer searmCRM'i kendi bilgisayarınızda barındırıyorsanız, tarayıcınızı açın ve [http://localhost:3000](http://localhost:3000) adresine gidin.
...
### SeaRM'yi Dış Erişime Açma

Varsayılan olarak, SeaRM `localhost` üzerinde `3000` portunda çalışır. Harici bir alan adı veya IP adresi aracılığıyla erişim sağlamak için `.env` dosyanızda `SERVER_URL`'i yapılandırmanız gerekir.
...
Belirli tarayıcı özelliklerinin düzgün çalışabilmesi için SSL (HTTPS) gereklidir. Bu özellikler yerel geliştirme sırasında çalışabilirken (tarayıcılar localhost'u farklı şekilde ele alır), SeaRM'yi düzenli bir alanda barındırırken uygun bir SSL kurulumu gereklidir.
...
Optimum güvenlik ve işlevsellik için SeaRM'nin SSL sonlandırma ile bir ters proxy arkasında ayarlanmasını şiddetle öneririz.
...
<Tip>
  SeaRM'yi kendiniz çalıştırıp bakımını yapmak istemiyor musunuz? [Sertifikalı bir SeaRM barındırma iş ortağı bulun](https://searm.com/partners/list?categories=HOSTING\&ref=docs-docker-compose); kurulumunuzun dağıtımını yapması, barındırması ve uçtan uca yönetmesi için. *(Veya ekibimize ulaşın: [contact@searm.com](mailto:contact@searm.com).)*
</Tip>
