...
description: SeaRM meta verilerinin deterministik evrensel tanımlayıcılarını her nesnede otomatik olarak çözümleyin; böylece uygulamanız bunlara, değerleri kodun içine gömmek zorunda kalmadan başvurabilir.
icon: gears
---

SeaRM içindeki her nesne, kendi başınıza asla bildirmediğiniz **sistem meta verileriyle** birlikte gelir; bir alan kümesi ve sütunlarıyla birlikte ana liste görünümü gibi. Sunucu, nesne sağlandığında bunların hepsini oluşturur ve SeaRM büyüdükçe bu küme de büyür.

Siz bunu bildirmediğiniz için, içe aktarabileceğiniz bir `universalIdentifier` sabiti yoktur. Bunun yerine, sunucu her tanımlayıcıyı deterministik olarak **türetir** ve `searm-sdk`, bildirim dosyanızın sunucunun kullandığı kesin değeri çözümleyebilmesi için aynı türetmeyi dışa açar.
...
<Note>
  `getFieldUniversalIdentifier`, `searm-sdk` 2.21 sürümünden itibaren kullanılabilir.
</Note>
...
<Note>
  `getSystemRelationFieldUniversalIdentifier` 2.23 sürümünden itibaren `searm-sdk` içinde kullanılabilir ve SeaRM sunucusunun 2.23 veya daha yeni bir sürümünü gerektirir.
</Note>
...
<Note>
  `getSystemViewUniversalIdentifier` ve `getSystemViewFieldUniversalIdentifier`, `searm-sdk` 2.26 sürümünden itibaren kullanılabilir ve SeaRM sunucusunun 2.26 veya daha yeni bir sürümünü gerektirir.
</Note>
...
İlk argümana dikkat edin: bir sütun, **görüntülediği alana** sahip olan uygulamanın ad alanındadır, görünüme sahip olanın değil. Uygulamanızın standart bir nesneye eklediği bir alan, SeaRM'ye ait bir görünüm üzerinde, kendi uygulamanız altında türetilen bir sütun alır.
...
## Standart SeaRM nesneleri

**Standart** bir SeaRM nesnesi (Person, Company, Opportunity, …) için hiçbir şey türetmeniz gerekmez: hem alanlar hem de görünümler için, doğrudan içe aktarabileceğiniz önceden hesaplanmış sabit tanımlayıcılar vardır.
