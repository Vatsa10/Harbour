...
ve kendi sunucunuzda barındırılan örneklerde, bu yol SeaRM sunucunuz
tarafından sunulur (örneğin `http://localhost:2020/s/documents/generate`). SeaRM Cloud'da, `RestApiClient`
...
[Yardımcıyı inceleyin](https://github.com/Vatsa10/Harbour/blob/main/packages/searm-apps/examples/document-generator/src/utils/render-document.ts).
...
<Tip>
Ayrıca test ederken bir işlevin günlüklerini `yarn searm dev:function:logs` ile gerçek zamanlı izleyebilir veya `yarn searm dev:function:exec` ile doğrudan çağırabilirsiniz.
</Tip>
