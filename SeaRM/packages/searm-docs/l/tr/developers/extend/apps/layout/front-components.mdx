...
description: SeaRM'nin UI'si içinde, korumalı alan (sandbox) izolasyonu ile görüntülenen React bileşenleri oluşturun.
icon: window-maximize
---

Ön uç bileşenler, SeaRM'nin UI'si içinde doğrudan görüntülenen React bileşenleridir. Remote DOM kullanan **izole bir Web Worker** içinde çalışırlar — kodunuz, korumalı (sandbox) ve opak kökenli bir iframe içinde yürütülür; ancak UI, o iframe ile sınırlanmak yerine sayfada yerel olarak işlenir.
...
Ön uç bileşenler, SeaRM içinde üç konumda işlenebilir:
...
const HelloWorld = () => {
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <p>This component renders inside SeaRM.</p>
    </div>
  );
};
...
`yarn searm dev` ile senkronize ettikten sonra (veya tek seferlik bir `yarn searm apply` çalıştırdıktan sonra), hızlı işlem sayfanın sağ üst köşesinde görünür:
...
Bileşen, tüm sekmenin yerine geçmek yerine, Settings sekmesi içinde bir bölüm olarak görüntülenir. SeaRM'nin sistem tarafından yönetilen bölümleri — otomatik yükseltme, App URL ve bağlantılar — her zaman bunun üstünde görüntülenir ve uygulama tarafından geçersiz kılınamaz.

```tsx src/front-components/app-settings.tsx
import { defineSettingsFrontComponent } from 'searm-sdk/define';
...
import { defineFrontComponent } from 'searm-sdk/define';
import { useSelectedRecordIds, enqueueSnackbar } from 'searm-sdk/front-component';
...
Bileşen `null` döndürdüğü için, SeaRM bunun için bir kapsayıcı oluşturmayı atlar — düzende boş alan görünmez. Bileşen yine de tüm hook'lara ve host iletişim API'sine erişime sahiptir.

## SDK Command bileşenleri

`searm-sdk` paketi, headless ön uç bileşenler için tasarlanmış dört Command yardımcı bileşeni sağlar. Her bileşen bağlandığında bir eylem yürütür, hataları bir snackbar bildirimi göstererek ele alır ve tamamlandığında ön bileşeni otomatik olarak kaldırır.

Bunları `searm-sdk/front-component` içinden içe aktarın:
...
import { defineFrontComponent } from 'searm-sdk/define';
import { Command } from 'searm-sdk/front-component';
...
import { defineFrontComponent } from 'searm-sdk/define';
import { CommandModal } from 'searm-sdk/front-component';
...
`httpRouteTriggerSettings` ile bildirilen bir mantık işlevi, rota yolunda HTTP üzerinden erişilebilir durumdadır. `RestApiClient`, `/s/` ile başlayan yolları uygulama rotaları olarak ele alır, bunları fonksiyonlarınızın sunulduğu URL'ye çözümler ve `SEARM_APP_ACCESS_TOKEN` ile kimlik doğrulaması yapar.

> **SeaRM Cloud üzerinde, HTTP ile tetiklenen mantık işlevleri, çalışma alanı başına ayrılmış özel bir etki alanında** `https://\<your-workspace-subdomain>.withsearm.com\<path>` adresinde sunulur. Harici çağrıcılar için, tam URL’yi işlevin **HTTP trigger** ayarlarından veya uygulamanın **Settings** sekmesinden kopyalayın.
...
`RestApiClient`'e iletilen yol, başına `/s` eklenmiş mantık işlevinin `httpRouteTriggerSettings.path` değeridir. `isAuthRequired: true` ayarını koruyun; bileşeniniz için SeaRM'nin oluşturduğu `SEARM_APP_ACCESS_TOKEN` isteği kimlik doğrular:
...
<Note>
`SEARM_APP_ACCESS_TOKEN` otomatik olarak enjekte edilir — bkz. [Uygulama değişkenleri](#application-variables). Gizli uygulama değişkenleri asla ön bileşenlere açığa çıkarılmadığından, API anahtarlarını ve diğer hassas mantığı ön bileşende değil, mantık işlevinin içinde tutun.
</Note>

### SeaRM REST API'sini çağırma

Uygulama HTTP rotalarını çağırmak veya bir front bileşeninden SeaRM kayıtlarını okumak ve yazmak için `searm-client-sdk/rest` paketinden `RestApiClient` kullanın. `/s/...` yollarını çalışma alanınızdaki işlevlerin temel URL'sine, `/rest/...` dahil diğer tüm yolları ise `SEARM_API_URL`'ye gönderir.
...
const client = new RestApiClient({
  baseUrl: 'https://myworkspace.searm.com',
  token: 'my-token',
});
```
...
```tsx
import { RestApiClient, RestApiClientError } from 'searm-client-sdk/rest';
...
| Değişken                  | Açıklama                                                  |
| ------------------------- | --------------------------------------------------------- |
| `SEARM_API_URL`          | SeaRM çekirdek API'sinin temel URL'si                    |
| `SEARM_APP_ACCESS_TOKEN` | Uygulamanızın rolüyle sınırlanan kısa ömürlü bir belirteç |

### `SEARM_FUNCTIONS_URL`

SeaRM ayrıca `SEARM_FUNCTIONS_URL` değişkenini front bileşenlerine ve mantık işlevlerine enjekte eder: bu, uygulamanızın HTTP ile tetiklenen mantık işlevlerinin sunulduğu temel URL'dir.

Bu değişken, bu URL her zaman SeaRM sunucusunun kendisi olmadığı için mevcuttur. SeaRM Cloud üzerinde, uygulama rotaları, uygulama tarafından yazılmış yanıtların SeaRM uygulamasının kaynağı yerine yalıtılmış bir kaynak üzerinde çalışması için, çalışma alanı başına ayrılmış özel bir etki alanında (`https://\<your-workspace-subdomain>.withsearm.com` veya yapılandırıldığında uygulamanın birincil genel etki alanında) sunulur. Self-hosted ve yerel örnekler, uygulama rotalarını sunucunun kendisinde `/s` öneki altında sunar ve değişkeni hiç ayarlamayabilir. Temel URL, çalışma alanına ve örneğe göre değiştiğinden, kodunuz bunu sabit olarak yazamaz — sunucu, çalışma zamanında doğru değeri enjekte eder.

Bu değişkeni doğrudan okumanız nadiren gerekir. Rotalarınızı, yolu `/s/` önekiyle birlikte vererek `RestApiClient` üzerinden çağırın; istemci URL'yi sizin için çözümler: `/s` önekini kaldırır ve `SEARM_FUNCTIONS_URL`'yi hedefler, değişken ayarlı değilse `\<SEARM_API_URL>/s` adresine geri döner. Mutlak URL'yi bir istek göndermeden elde etmek için, örneğin bir bağlantı için, `resolveUrl('/s/\<path>')` kullanın. Değişkeni yalnızca bir URL'yi elle oluştururken doğrudan okuyun:

```ts
const routeUrl = `${process.env.SEARM_FUNCTIONS_URL || `${process.env.SEARM_API_URL}/s`}/documents/generate`;
```

## Host iletişim API'si

Ön uç bileşenleri, `searm-sdk`'deki işlevleri kullanarak gezinmeyi, modalları ve bildirimleri tetikleyebilir:
...
import { defineFrontComponent } from 'searm-sdk/define';
import { enqueueSnackbar, closeSidePanel, useSelectedRecordIds } from 'searm-sdk/front-component';
import { CoreApiClient } from 'searm-client-sdk/core';
...
import { enqueueSnackbar, closeSidePanel } from 'searm-sdk/front-component';
import { CoreApiClient } from 'searm-client-sdk/core';
...
```tsx
import { defineFrontComponent } from 'searm-sdk/define';
import { getPublicAssetUrl } from 'searm-sdk/utils';
...
* **SeaRM UI bileşenleri** — SeaRM'nin kendi bileşen kitaplığı; aşağıda [SeaRM UI bileşenlerini kullanma](#using-searm-ui-components) bölümüne bakın
...
## SeaRM UI bileşenlerini kullanma

SeaRM, bileşen kitaplığını [`searm-ui`](https://www.npmjs.com/package/searm-ui/v/1.0.0-alpha.1) paketi olarak sunar. Ön uç bileşenleri bunu, çalışma alanının açık ve koyu temasıyla otomatik olarak eşleşen düğmeler, etiketler, durum rozetleri, çipler, avatarlar, simgeler, tipografi ve tema belirteçleri için kullanabilir.

### Kurulum

Paketi, SeaRM örneğinizle birlikte gelen sürüme sabitleyerek uygulamanıza ekleyin:

```bash
yarn add searm-ui@1.0.0-alpha.1
```

`searm-ui`, derleme zamanında ön uç bileşeninize paketlenir, bu nedenle yalnızca uygulamanızın bir bağımlılığı olması gerekir — çalışma zamanında yapılandırmanız gereken hiçbir şey yoktur.
...
| --------------------------- | -------------------------------------------------------- |
| `searm-ui/input`           | `Button` ve form girdileri                               |
| `searm-ui/data-display`    | `Tag`, `Status`, `Chip`, `Avatar` ve daha fazlası        |
| `searm-ui/feedback`        | `Callout`, `Banner`, `Info` ve daha fazlası              |
| `searm-ui/typography`      | `H1Title`, `H2Title`, `H3Title`, `Label` ve daha fazlası |
| `searm-ui/icon`            | `Icon*` bileşenleri (ör. `IconCheck`)                    |
| `searm-ui/theme-constants` | `ThemeProvider`, `themeCssVariables`                     |
...
Tek tek simgeleri `searm-ui/icon` içinden içe aktarın:

```tsx
import { IconBox, IconCheck } from 'searm-ui/icon';
```
...
SeaRM UI bileşenleri, çalışma alanının açık ve koyu temasıyla otomatik olarak eşleşir — oluşturucu etkin renk şemasını barındırıcıda uygular ve bileşenler renklerini buna göre belirler.

Kendi satır içi stillerinizde aynı tasarım belirteçlerini kullanmak için `useTheme()` kancasını çağırın. Bu kanca, etkin temaya bağlı SeaRM tema belirteçlerini (boşluklandırma, renkler, köşe yarıçapları, yazı tipleri) döndürür; bileşeninizde `ThemeProvider` kurulumuna gerek yoktur:
...
Etkin şemaya açıkça dallanmak için, `searm-sdk/front-component` içindeki `useColorScheme()` ile okuyun; bu kanca `'light'` veya `'dark'` döndürür.
...
Bunlardan biri sizi engelliyorsa, önceliklendirilmesi için [bir issue açın](https://github.com/Vatsa10/Harbour/issues/new/choose).
...
Bileşen CSS’i, ister `import './styles.css'` ile, ister CSS-in-JS ile ya da bir `\<style>` öğesinden gelsin, host sayfanın `\<head>` kısmına **scope’suz** olarak enjekte edilir. Bu nedenle sınıf adları SeaRM’nin kendi sınıf adlarıyla çakışır (onlara önek ekleyin ve asla çıplak `div { ... }` seçicileri yazmayın) ve `@media`, sizin bileşeniniz yerine tarayıcı penceresini hedefler (kendi `container-type` değerinizle `@container` kullanın). Satır içi `style` prop’ları etkilenmez.
...
* SeaRM API’sine ve uygulamanızın rotalarına yapılan çağrılar host tarafından proxy’lenir, bu yüzden [`RestApiClient`](#calling-the-searm-rest-api) kullanmayı tercih edin. Proxy’lenen çağrılarda `AbortSignal` ve diğer `RequestInit` seçenekleri kaldırılır ve yalnızca `string` ve `URLSearchParams` gövdeleri desteklenir.
...
* `fetch('/rest/people')` hiçbir zaman SeaRM API’siyle eşleştirilmez, çünkü sandbox’un göreli yolu çözecek bir sayfa URL’si yoktur.
