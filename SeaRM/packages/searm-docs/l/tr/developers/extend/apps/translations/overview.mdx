...
SeaRM uygulamaları **İngilizce** olarak oluşturulur: kaynak ve manifestinizdeki dizeler İngilizce kaynak metindir, `en` çevirdiğiniz kaynak yerel ayarıdır ve çevrilmeden bırakılan herhangi bir yerel ayar buna geri döner.
...
Çeviri yardımcılarını `searm-sdk/front-component` içinden içe aktarın:
...
`searm dev:build`, katalogları derler ve geçerli kullanıcı için doğru dili sunar:
...
`searm dev:build`'i yeniden çalıştırmayı (ve yeniden dağıtmayı) gerektirir; bu, diğer herhangi bir değişiklikle aynıdır.

<Note>
Çeviriler `searm dev:build` (ve `searm apply`) tarafından derlenir. Sürekli
`searm dev` izlemesi kaynak dizeleri gösterir, bu yüzden yerelleştirilmiş çıktıyı tek seferlik bir build ile test edin.
</Note>
