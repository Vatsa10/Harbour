...
* Alışıldık mantık işlevi ortamı (`APPLICATION_ID`, `APP_ACCESS_TOKEN`, `API_URL`) enjekte edilir, böylece SeaRM API'sini uygulamanızın jetonuyla çağırabilirsiniz.
...
* **Geliştirme modunda yürütülmez**: `yarn searm dev` kurulum akışını atlar ve dosyaları doğrudan senkronize eder, bu nedenle kancalar burada asla çalışmaz. Bunları bunun yerine manuel olarak tetikleyin:
