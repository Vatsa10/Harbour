...
* **Derlemelere dahil edilir** — `yarn searm dev:build`, tüm genel varlıkları dağıtım çıktısına paketler.

## `getPublicAssetUrl` ile genel varlıklara erişme

`searm-sdk` içindeki `getPublicAssetUrl` yardımcı işlevini kullanarak `public/` dizininizdeki bir dosyanın tam URL'sini alın. Hem **mantık işlevlerinde** hem de **ön uç bileşenlerinde** çalışır.
