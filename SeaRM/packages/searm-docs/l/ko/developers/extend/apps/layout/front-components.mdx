---
title: 프런트 컴포넌트
description: SeaRM의 UI 내부에서 샌드박스 환경으로 격리된 채 렌더링되는 React 컴포넌트를 빌드하세요.
icon: window-maximize
---

프런트 컴포넌트는 SeaRM의 UI 내부에서 직접 렌더링되는 React 컴포넌트입니다. 이들은 Remote DOM을 사용하는 **격리된 Web Worker**에서 실행됩니다 — 코드는 샌드박스 처리된 opaque-origin iframe 내부에서 실행되지만, UI는 해당 iframe 안에 갇히지 않고 페이지 내에서 네이티브로 렌더링됩니다.
...
프런트 컴포넌트는 SeaRM 내에서 세 위치에 렌더링될 수 있습니다:
...
`yarn searm dev`로 동기화한 후(또는 일회성으로 `yarn searm apply`를 실행한 경우), 페이지 우측 상단에 빠른 작업이 표시됩니다:
...
이 컴포넌트는 전체 탭을 대체하는 것이 아니라, Settings 탭 내부의 섹션으로 렌더링됩니다. SeaRM의 시스템 관리 섹션(자동 업그레이드, App URL, 연결)은 항상 이 컴포넌트 위에 렌더링되며, 앱에서 이를 재정의할 수 없습니다.
...
컴포넌트가 `null`을 반환하기 때문에, SeaRM는 해당 컴포넌트에 대한 컨테이너 렌더링을 생략합니다 — 레이아웃에 빈 공간이 생기지 않습니다. 컴포넌트는 여전히 모든 훅과 호스트 통신 API에 접근할 수 있습니다.

## SDK Command 컴포넌트

`searm-sdk` 패키지는 헤드리스 프런트 컴포넌트를 위해 설계된 네 가지 Command 헬퍼 컴포넌트를 제공합니다. 각 컴포넌트는 마운트 시 동작을 실행하고, 스낵바 알림을 표시하여 오류를 처리하며, 완료되면 프런트 컴포넌트를 자동으로 언마운트합니다.

`searm-sdk/front-component`에서 임포트하세요:
...
`httpRouteTriggerSettings`로 선언된 로직 함수는 HTTP를 통해 해당 라우트 경로에서 액세스할 수 있습니다. `RestApiClient`는 `/s/`로 시작하는 경로를 앱 라우트로 처리하고, 해당 경로를 함수가 제공되는 URL로 해석한 뒤 `SEARM_APP_ACCESS_TOKEN`으로 인증합니다.

> **SeaRM Cloud에서 HTTP로 트리거되는 로직 함수는 작업공간별 전용 도메인에서 제공됩니다**: `https://\<your-workspace-subdomain>.withsearm.com\<path>`. 외부 호출자의 경우, 함수의 **HTTP trigger** 설정 또는 애플리케이션의 **Settings** 탭에서 정확한 URL을 복사하세요.
...
`RestApiClient`에 전달되는 경로는 로직 함수의 `httpRouteTriggerSettings.path` 앞에 `/s`가 접두사로 붙은 값입니다. `isAuthRequired: true`로 유지하세요. 컴포넌트를 위해 SeaRM가 발급하는 `SEARM_APP_ACCESS_TOKEN`이 요청을 인증합니다:
...
<Note>
`SEARM_APP_ACCESS_TOKEN`은 자동으로 주입됩니다. 자세한 내용은 [Application variables](#application-variables)을 참고하세요. 비밀 애플리케이션 변수는 Front 컴포넌트에 절대 노출되지 않으므로, API 키 및 기타 민감한 로직은 Front 컴포넌트가 아니라 로직 함수 안에 유지해야 합니다.
</Note>

### SeaRM REST API 호출하기

앱 HTTP 라우트를 호출하거나 프론트 컴포넌트에서 SeaRM 레코드를 읽고 쓰려면 `searm-client-sdk/rest`의 `RestApiClient`를 사용하세요. 이 클라이언트는 `/s/...` 경로를 작업공간의 함수 base URL로 전송하고, `/rest/...`를 포함한 그 외 모든 경로는 `SEARM_API_URL`로 전송합니다.
...
| 변수                        | 설명                       |
| ------------------------- | ------------------------ |
| `SEARM_API_URL`          | SeaRM 코어 API의 기본 URL    |
| `SEARM_APP_ACCESS_TOKEN` | 앱의 역할 범위로 제한된 단기간 유효한 토큰 |

### `SEARM_FUNCTIONS_URL`

SeaRM는 또한 프론트 컴포넌트와 로직 함수에 `SEARM_FUNCTIONS_URL`을 주입합니다. 이는 앱의 HTTP로 트리거되는 로직 함수가 제공되는 base URL입니다.

이 URL이 항상 SeaRM 서버 자체를 가리키는 것은 아니기 때문에 이 변수가 존재합니다. SeaRM Cloud에서는 앱 라우트가 작업공간별 전용 도메인(`https://\<your-workspace-subdomain>.withsearm.com`, 또는 구성된 경우 애플리케이션의 기본 public 도메인)에서 제공되므로, 앱이 작성한 응답이 SeaRM 앱 origin이 아니라 분리된 origin에서 실행됩니다. 셀프 호스팅 및 로컬 인스턴스에서는 앱 라우트가 서버 자체의 `/s` 접두사 아래에서 제공되며, 이 변수가 전혀 설정되지 않을 수 있습니다. base URL은 작업공간과 인스턴스마다 다르므로 코드에서 이를 하드코딩할 수 없으며, 서버가 런타임에 올바른 값을 주입합니다.

이 변수를 직접 읽어야 하는 경우는 드뭅니다. `RestApiClient`를 통해 `/s/` 접두사가 붙은 경로로 라우트를 호출하면, 클라이언트가 URL을 대신 해석합니다. `/s` 접두사를 제거하고 `SEARM_FUNCTIONS_URL`을 대상으로 하며, 변수가 설정되어 있지 않은 경우 `\<SEARM_API_URL>/s`를 사용합니다. 요청을 보내지 않고 절대 URL이 필요할 때(예: 링크용) `resolveUrl('/s/\<path>')`를 사용하세요. 직접 URL을 구성해야 할 때만 이 변수를 직접 읽으세요:
...
프런트 컴포넌트는 `searm-sdk`의 함수를 사용해 내비게이션, 모달, 알림을 트리거할 수 있습니다:
...
* **SeaRM UI 컴포넌트** — SeaRM의 자체 컴포넌트 라이브러리입니다. 아래의 [SeaRM UI 컴포넌트 사용하기](#using-searm-ui-components)를 참조하세요.
...
SeaRM는 컴포넌트 라이브러리를 [`searm-ui`](https://www.npmjs.com/package/searm-ui/v/1.0.0-alpha.1) 패키지로 제공합니다. 프론트 컴포넌트에서는 버튼, 태그, 상태 배지, 칩, 아바타, 아이콘, 타이포그래피, 그리고 워크스페이스의 라이트 및 다크 테마에 자동으로 맞춰지는 테마 토큰 등에 사용할 수 있습니다.

### 설치

앱에 패키지를 추가하되, SeaRM 인스턴스에서 제공되는 버전에 고정(pinned)하여 사용하세요:

```bash
yarn add searm-ui@1.0.0-alpha.1
```

`searm-ui` 는 빌드 시점에 프론트 컴포넌트에 번들되므로, 앱의 의존성으로만 추가하면 됩니다. 런타임에 따로 설정할 내용은 없습니다.
...
| 서브패스                        | 내보내는 항목                                    |
| --------------------------- | ------------------------------------------ |
| `searm-ui/input`           | `Button` 및 폼 입력 컴포넌트                       |
| `searm-ui/data-display`    | `Tag`, `Status`, `Chip`, `Avatar` 등        |
| `searm-ui/feedback`        | `Callout`, `Banner`, `Info` 등              |
| `searm-ui/typography`      | `H1Title`, `H2Title`, `H3Title`, `Label` 등 |
| `searm-ui/icon`            | `Icon*` 컴포넌트(예: `IconCheck`)               |
...
`searm-ui/icon` 에서 개별 아이콘을 import 하세요:
...
SeaRM UI 컴포넌트는 워크스페이스의 라이트 및 다크 테마와 자동으로 일치합니다. 렌더러가 호스트에서 활성 색 구성표를 적용하고, 컴포넌트는 그에 맞춰 자신의 색상을 결정합니다.

자신의 인라인 스타일에서 동일한 디자인 토큰을 사용하려면 `useTheme()` 훅을 호출하세요. 이 훅은 활성 테마에 연결된 SeaRM의 테마 토큰(여백, 색상, 모서리 반경, 폰트)을 반환하며, 컴포넌트에서 별도의 `ThemeProvider` 설정이 필요하지 않습니다:
...
활성 스킴에 따라 분기해야 할 경우, `searm-sdk/front-component` 의 `useColorScheme()` 으로 값을 읽으세요. 이 훅은 `'light'` 또는 `'dark'` 를 반환합니다.
...
이 중 하나가 개발을 가로막는다면, 우선순위를 높일 수 있도록 [issue를 열어](https://github.com/Vatsa10/Harbour/issues/new/choose) 주세요.
...
컴포넌트 CSS는 `import './styles.css'`, CSS-in-JS 또는 `\<style>` 요소에서 오든, 호스트 페이지의 `\<head>`에 **범위 없이(unscoped)** 주입됩니다. 따라서 클래스 이름이 SeaRM 자체 것과 충돌합니다(접두사를 붙이고, `div { ... }` 선택자를 직접 작성하지 마세요). 그리고 `@media`는 위젯이 아니라 브라우저 창을 기준으로 매칭됩니다(직접 지정한 `container-type`과 함께 `@container`를 사용하세요). 인라인 `style` prop은 영향을 받지 않습니다.
...
* SeaRM API와 앱의 라우트로 가는 호출은 호스트에서 프록시되므로, [`RestApiClient`](#calling-the-searm-rest-api)를 사용하는 것이 좋습니다. 프록시되는 호출에서는 `AbortSignal` 및 기타 `RequestInit` 옵션이 제거되며, `string`과 `URLSearchParams` 타입의 body만 지원됩니다.
...
* 샌드박스에는 상대 경로를 해석할 페이지 URL이 없으므로, `fetch('/rest/people')`은 SeaRM API와 절대 매칭되지 않습니다.
