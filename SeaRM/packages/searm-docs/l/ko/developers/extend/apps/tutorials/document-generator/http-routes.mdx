...
둘 다 `httpRouteTriggerSettings`를 사용합니다. 앱 라우트는 `/s/` 경로로 접근합니다. 로컬 및 셀프 호스팅 인스턴스에서는 해당 경로가 SeaRM 서버에서 제공됩니다(예: `http://localhost:2020/s/documents/generate`). SeaRM Cloud에서는 `RestApiClient`가 워크스페이스의 functions 도메인을 통해 경로를 해석합니다.
...
[헬퍼를 확인하세요](https://github.com/Vatsa10/Harbour/blob/main/packages/searm-apps/examples/document-generator/src/utils/render-document.ts).
...
<Tip>
테스트하는 동안 `yarn searm dev:function:logs`로 함수의 로그를 스트리밍하거나, `yarn searm dev:function:exec`로 직접 호출할 수도 있습니다.
</Tip>
