---
title: 데이터 모델 FAQ
description: SeaRM의 데이터 모델에 대한 자주 묻는 질문입니다.
---
...
표준 객체(사람, 회사, 기회)의 레이블은 변경할 수 있지만, API 이름은 변경할 수 없습니다. 모든 SeaRM 워크스페이스에서의 일관성을 위해 API 이름은 고정되어 있습니다.
</Accordion>
...
하나의 객체가 가질 수 있는 필드 수의 최대값은 Postgres 제한에 따라 기존 필드와 삭제된 필드를 모두 포함해 1600개입니다. 1600개 필드 한도에 도달하면 이는 당사의 공정 사용(Fair Usage) 정책에 해당하므로 SeaRM에서는 워크스페이스를 수정해 주지 않는다는 점에 유의하세요.
</Accordion>
</AccordionGroup>
