---
title: CSV로 연락처 가져오기
description: SeaRM로 사람/연락처를 가져오는 포괄적인 단계별 가이드.
---

## 개요

이 가이드는 연락처(People)를 SeaRM로 가져오는 과정을 단계별로 안내합니다.
...
* **고유해야 함** — 파일 또는 SeaRM에 중복이 없어야 합니다
...
<Note>
**도메인 URL 형식을 사용하세요**(`https://acme.com`), 라벨은 사용하지 마세요. 이는 SeaRM에서 회사가 저장되는 방식과 일치합니다.
</Note>
...
<Note>
**전문가 팁:** 가져오기 중에 **Download sample file**을 클릭하거나 기존 People 몇 명을 내보내 SeaRM가 기대하는 정확한 열 이름을 확인하세요.
</Note>
...
3. SeaRM가 파일을 분석할 때까지 기다리세요

## 4단계: 열 매핑

SeaRM가 열을 필드에 자동으로 매칭하려고 시도합니다. 검토 및 조정:
...
1. SeaRM에서 사용자의 값을 기존 옵션과 나란히 표시합니다
2. 파일의 각 값을 SeaRM 옵션과 매칭하세요
3. 필요하면 새 옵션을 만드세요
...
가져오기를 완료하기 전에 SeaRM가 데이터를 검증합니다:
...
| 오류              | 원인                        | 해결책                       |
| --------------- | ------------------------- | ------------------------- |
| **중복 이메일**      | 이메일이 SeaRM 또는 파일에 이미 존재함 | 중복을 제거하거나 기존 레코드를 업데이트하세요 |
...
<AccordionGroup>
  <Accordion title="왜 두 연락처가 동일한 이메일을 가질 수 없나요?">
    이메일은 SeaRM에서 고유 식별자입니다. 이는 중복 연락처를 방지하고 이메일 동기화가 올바른 연락처에 이메일을 정확히 연결되도록 보장합니다.
  </Accordion>
  ...
  <Accordion title="이전 CRM의 ID를 가져올 수 있나요?">
    예! 외부 ID를 저장하려면 데이터 모델에 "unique"로 표시된 사용자 지정 필드를 만드세요. 참고: 필드 이름 `id`는 SeaRM의 내부 ID로 예약되어 있습니다.
  </Accordion>
