...
title: Local Setup
icon: "laptop-code"
description: "The guide for contributors (or curious developers) who want to run SeaRM locally."
---
...
Before you can install and use SeaRM, make sure you install the following on your computer:
...
SeaRM requires a Redis cache to provide the best performance.
...
<Info>
**Multi-Workspace Mode:** By default, SeaRM runs in single-workspace mode where only one workspace can be created. To enable multi-workspace support (useful for testing subdomain-based features), set `IS_MULTIWORKSPACE_ENABLED=true` in your server `.env` file. See [Multi-Workspace Mode](/developers/self-host/capabilities/setup#multi-workspace-mode) for details.
</Info>

## Step 6: Installing dependencies
To build SeaRM server and seed some data into your database, run the following command:
```bash
yarn
```
...
SeaRM's frontend will be running at [http://localhost:3001](http://localhost:3001).
You can log in using the default demo account: `tim@apple.dev` (password: `tim@apple.dev`)

**Backend**

- SeaRM's server will be up and running at [http://localhost:3000](http://localhost:3000)
