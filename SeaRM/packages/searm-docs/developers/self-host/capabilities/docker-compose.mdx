...
This guide provides step-by-step instructions to install and configure the SeaRM application using Docker Compose. The aim is to make the process straightforward and prevent common pitfalls that could break your setup.
...
Install the latest stable version of SeaRM with a single command:
```bash
...
If you host searmCRM on your own computer, open your browser and navigate to [http://localhost:3000](http://localhost:3000).
...
### Expose SeaRM to External Access

By default, SeaRM runs on `localhost` at port `3000`. To access it via an external domain or IP address, you need to configure the `SERVER_URL` in your `.env` file.
...
SSL (HTTPS) is required for certain browser features to work properly. While these features might work during local development (as browsers treat localhost differently), a proper SSL setup is needed when hosting SeaRM on a regular domain.
...
We strongly recommend setting up SeaRM behind a reverse proxy with SSL termination for optimal security and functionality.
...
<Tip>
  Prefer not to run and maintain SeaRM yourself? [Find a certified SeaRM hosting partner](https://searm.com/partners/list?categories=HOSTING&ref=docs-docker-compose) to deploy, host, and manage your instance end-to-end. *(Or reach our team: [contact@searm.com](mailto:contact@searm.com).)*
</Tip>
