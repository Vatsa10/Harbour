...
If you have an existing SeaRM instance with production data, **DO NOT** follow these steps as they will permanently delete your database!

While installing SeaRM for the first time, you might want to change the default database password.
...
During SeaRM installation, you need to provision your postgres database with the right schemas, extensions, and users.
...
#### Cannot find module 'searm-emails' or its corresponding type declarations.

You have to build the `searm-emails` package (`npx nx run searm-emails:build`) before running the database initialization.

#### Missing searm-x package

Make sure to run yarn in the root directory and then run `npx nx server:dev searm-server`. If this still doesn't work try building the missing package manually.
...
#### While running `npx nx start` or `npx nx start searm-front`, an out-of-memory error is thrown

Run only the services you need, instead of `npx nx start`. For instance, if you work on the server, run only `npx nx worker searm-server`

**If it does not work:**
If you tried to run only `npx nx run searm-server:start` on WSL and it's failing with the below memory error:
...
**If it does not work:**
Investigate which processes are taking up most of your machine's RAM. At SeaRM, we noticed some VS Code extensions were taking up a lot of RAM, so we temporarily disabled them.
...
#### Cannot connect my Microsoft 365 account to SeaRM
...
- Open your SeaRM app, navigate to `/settings`, and enable the **Advanced** toggle at the bottom left of the screen.
- Create a new webhook.
- Paste **Your Unique Webhook URL** in the **Endpoint Url** field in SeaRM. Set the **Filters** to `Companies` and `Created`.
<div style={{textAlign: 'center'}}>
...
Comment out the checker plugin in `packages/searm-ui/vite-config.ts`, as shown in the example below:
```
plugins: [
...
If you're running SeaRM behind a reverse proxy and experiencing connection issues:
...
2. **Check Reverse Proxy Settings:**
    ...
    - Confirm that your reverse proxy is correctly forwarding requests to the SeaRM server.
    - Ensure headers like `X-Forwarded-For` and `X-Forwarded-Proto` are properly set.

3. **Restart Services:**

    After making changes, restart both the reverse proxy and SeaRM containers.
...
- Community Support:
    ...
    Reach out to the [SeaRM community](https://github.com/Vatsa10/Harbour/issues) or [support channels](https://discord.gg/cx5n4Jzs57) for assistance.
