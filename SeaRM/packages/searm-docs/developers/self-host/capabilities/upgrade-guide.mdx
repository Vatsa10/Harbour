...
Starting from **v1.23**, SeaRM supports cross-version upgrades. You can jump directly from any supported version to the latest release without stepping through each intermediate version.

For example, upgrading from v1.23 straight to v2.0 is fully supported.

## Upgrading to v2.5+ — at-rest encryption envelope

Starting in **v2.5**, SeaRM stores at-rest secrets (OAuth tokens, application variables, signing-key private keys, sensitive config values, TOTP secrets) inside a versioned `enc:v2:` envelope encrypted with `ENCRYPTION_KEY` (or `APP_SECRET` if `ENCRYPTION_KEY` is unset).
