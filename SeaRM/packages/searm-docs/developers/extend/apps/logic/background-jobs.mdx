...
description: Hand long or rate-limited work to the SeaRM workers by enqueuing another logic function run instead of doing everything inline.
icon: "layer-group"
---
...
`enqueueJob` does exactly that: it asks the SeaRM workers to run one of your app's logic functions later, in its own process, with its own timeout budget. The caller returns immediately.
...
Import `enqueueJob` from `searm-sdk/logic-function` and point it at the `universalIdentifier` of the logic function you want to run.
