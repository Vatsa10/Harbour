---
title: Concepts
description: How SeaRM apps work — entity model, sandboxing, and the install lifecycle.
icon: "sitemap"
---

SeaRM apps are TypeScript packages that extend your workspace with custom objects, logic, UI components, and AI capabilities. They run on the SeaRM platform with full sandboxing and permission controls.

## How apps work

An app is a collection of **entities** declared using `defineEntity()` functions from the `searm-sdk` package. The SDK detects these declarations via AST analysis at build time and produces a **manifest** — a complete description of what your app adds to a workspace. These functions validate your configuration at build time and provide IDE autocompletion and type safety.
...
| **Front Component** | Sandboxed React UI inside SeaRM | [Front Components](/developers/extend/apps/layout/front-components) |
...
- **Front components** run in Web Workers using Remote DOM — sandboxed from the main page but rendering native DOM elements (not iframes). They communicate with SeaRM via a message-passing host API.
- **Permissions** are enforced at the API level. The runtime token (`SEARM_APP_ACCESS_TOKEN`) is derived from the role defined in `defineApplication()`.
...
- **`yarn searm dev`** — watches your source files and live-syncs changes to a connected SeaRM server. The typed API client is regenerated automatically when the schema changes.
- **`yarn searm dev:build`** — compiles TypeScript, bundles logic functions and front components with esbuild, and produces a manifest.
