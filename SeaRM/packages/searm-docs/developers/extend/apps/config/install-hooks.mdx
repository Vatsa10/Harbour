...
- The usual logic-function environment (`APPLICATION_ID`, `APP_ACCESS_TOKEN`, `API_URL`) is injected, so you can call the SeaRM API with your app's token.
...
- **Not executed in dev mode**: `yarn searm dev` skips the install flow and syncs files directly, so hooks never run there. Trigger them manually instead:
