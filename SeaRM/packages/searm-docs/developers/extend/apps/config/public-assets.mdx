...
- **Included in builds** — `yarn searm dev:build` bundles all public assets into the distribution output.

## Accessing public assets with `getPublicAssetUrl`

Use the `getPublicAssetUrl` helper from `searm-sdk` to get the full URL of a file in your `public/` directory. It works in both **logic functions** and **front components**.
