...
Webhooks push data to your systems in real-time when events occur in SeaRM — no polling required. Use them to keep external systems in sync, trigger automations, or send alerts.
...
SeaRM sends webhooks for these event types:
...
SeaRM signs each webhook request for security. Validate signatures to ensure requests are authentic.

### Headers

| Header | Description |
|--------|-------------|
| `X-SeaRM-Webhook-Signature` | HMAC SHA256 signature |
| `X-SeaRM-Webhook-Timestamp` | Request timestamp |

### Validation Steps

1. Get the timestamp from `X-SeaRM-Webhook-Timestamp`
2. Create the string: `{timestamp}:{JSON payload}`
3. Compute HMAC SHA256 using your webhook secret
4. Compare with `X-SeaRM-Webhook-Signature`
...
| **Workflow Webhook Trigger** | IN | Receive data into SeaRM from external systems |
