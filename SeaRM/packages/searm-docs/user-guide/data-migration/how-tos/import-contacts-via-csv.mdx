...
description: Complete step-by-step guide to importing people/contacts into SeaRM.
---

## Overview

This guide walks you through importing your contacts (People) into SeaRM.
...
- **Must be unique** — no duplicates in your file or in SeaRM
...
<Note>
**Use Domain URL format** (`https://acme.com`), not the label. This matches how Companies are stored in SeaRM.
</Note>
...
<Note>
**Pro tip:** Click **Download sample file** during import or export a few existing People to see the exact column names SeaRM expects.
</Note>
...
3. Wait for SeaRM to analyze your file

## Step 4: Map Your Columns

SeaRM automatically tries to match your columns to fields. Review and adjust:
...
3. SeaRM will link each Person to the matching Company
...
1. SeaRM shows your values alongside existing options
2. Match each value in your file to a SeaRM option
...
Before completing the import, SeaRM validates your data:
...
|-------|-------|----------|
| **Duplicate email** | Email already exists in SeaRM or file | Remove duplicate or update existing record |
...
Your contacts are now in SeaRM! Next steps:
...
2. SeaRM matches records by this unique identifier
...
<AccordionGroup>
  <Accordion title="Why can't two People have the same email?">
    Email is a unique identifier in SeaRM. This prevents duplicate contacts and ensures email sync correctly links emails to the right person.
  </Accordion>
  ...
  <Accordion title="Can I import the ID from my previous CRM?">
    Yes! Create a custom field marked as "unique" in your data model to store the external ID. Note: the field name `id` is reserved for SeaRM's internal ID.
  </Accordion>
