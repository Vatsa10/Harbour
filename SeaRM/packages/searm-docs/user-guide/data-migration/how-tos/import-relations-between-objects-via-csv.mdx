...
The parent ("one" side) must exist in SeaRM before you can reference it.
...
If you need the `id`:
1. Export the parent records from SeaRM
...
Use `https://domain.com` (not just `domain.com`). This matches how SeaRM stores Company domains and prevents matching errors.
</Warning>
...
SeaRM will automatically link each child record to the matching parent.
...
| **Non-existent records** | The parent record doesn't exist in SeaRM | Import parent records first, or check for typos |
| **Case mismatch** | `Acme.com` in file but `acme.com` in SeaRM | Ensure exact case matching |
...
<AccordionGroup>
  <Accordion title="What if my parent record doesn't have a domain or email?">
    ...
    1. Use the SeaRM `id` (export parent records to get their IDs)
