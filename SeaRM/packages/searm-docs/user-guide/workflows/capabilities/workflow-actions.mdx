...
description: Learn about the actions available in SeaRM workflows.
---
...
<Note>
You can only send emails from mailboxes synced to your own SeaRM account. Sending from other team members' mailboxes (e.g., the account owner's email) is on the roadmap.
</Note>
...
Behind the scenes, each Code action is backed by its own **logic function** — a server-side TypeScript function that runs on the SeaRM platform. When you add a Code action, SeaRM creates a dedicated logic function for that step and exposes its editor inline, so the code you write lives with the workflow.
