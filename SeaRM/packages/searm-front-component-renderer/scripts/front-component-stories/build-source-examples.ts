...
import { getFrontComponentBuildPlugins } from 'searm-sdk/front-component-renderer/build';
...
const searmUiIndividualIndex = path.resolve(
  dirname,
  '../../../searm-ui/dist/individual/individual-entry.js',
);

const sdkDefineIndex = path.resolve(
  dirname,
  '../../../searm-sdk/dist/define/index.mjs',
);

const sdkFrontComponentIndex = path.resolve(
  dirname,
  '../../../searm-sdk/dist/front-component/index.mjs',
);

const searmSharedIndividualDir = path.resolve(
  dirname,
  '../../../searm-shared/dist/individual',
);

const SEARM_SHARED_SUBMODULES = [
  'ai',
  'application',
  'constants',
  'database-events',
...
const searmSharedAliases = Object.fromEntries(
  SEARM_SHARED_SUBMODULES.map((submodule) => [
    `searm-shared/${submodule}`,
    path.join(searmSharedIndividualDir, submodule, 'index.js'),
  ]),
);

const SEARM_UI_SUBMODULES = [
  'accessibility',
  'data-display',
  'feedback',
  'icon',
...
const searmUiAliases = {
  'searm-ui': searmUiIndividualIndex,
  ...Object.fromEntries(
    SEARM_UI_SUBMODULES.map((submodule) => [
      `searm-ui/${submodule}`,
      searmUiIndividualIndex,
    ]),
  ),
};
...
const storyAlias = {
  ...
  'searm-sdk/define': sdkDefineIndex,
  'searm-sdk/front-component': sdkFrontComponentIndex,
  ...searmUiAliases,
  ...searmSharedAliases,
};
