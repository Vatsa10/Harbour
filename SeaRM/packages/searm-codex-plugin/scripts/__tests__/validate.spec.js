...
test('assertSearmMcpFormattingContract passes on current state', () => {
  assert.deepStrictEqual(collectFailures(crossDocContracts.assertSearmMcpFormattingContract), []);
});
...
test('assertJsonMetadata catches non-canonical MCP server', () => {
  withJsonMutation(MCP_JSON_PATH, (mcp) => {
    ...
    const failures = collectFailures(metadata.assertJsonMetadata);
    assert.ok(failures.some((f) => f.includes('searm-docs')));
  });
});
...
test('assertAssets catches a non-PNG logo', () => {
  withJsonMutation(PLUGIN_JSON_PATH, (j) => { j.interface.logo = './assets/searm-logo.svg'; }, () => {
    const failures = collectFailures(assets.assertAssets);
...
test('assertTestingGuidance catches missing manage-app test target instructions', () => {
  ...
  withFileMutation(skillPath, (original) => original.replace('SEARM_API_URL=http://localhost:2021 yarn test', 'yarn test'), () => {
    const failures = collectFailures(crossDocContracts.assertTestingGuidance);
    assert.ok(
      failures.some((f) => f.includes('manage-app/SKILL.md') && f.includes('SEARM_API_URL')),
