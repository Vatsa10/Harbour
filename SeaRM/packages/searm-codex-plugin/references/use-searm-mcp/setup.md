...
Use this reference to configure a workspace-specific SeaRM MCP endpoint. The plugin must not assume a fixed workspace domain; collect the user's workspace URL first, normalize it to an MCP URL, then configure and authenticate the MCP client.
...
- MCP server name. Default to a workspace-derived name in the form `searm-<host-without-tld>`, for example `acme.example.com` becomes `searm-acme-example`.
...
Use the bundled helper from the SeaRM repo or plugin checkout:

```bash
bash packages/searm-codex-plugin/scripts/setup-mcp.sh <workspace-url-or-mcp-url>
```

Use `--name <server-name>` when configuring multiple SeaRM workspaces:

```bash
bash packages/searm-codex-plugin/scripts/setup-mcp.sh --name searm-prod acme.searm.com
bash packages/searm-codex-plugin/scripts/setup-mcp.sh --name searm-local acme.localhost:3001
```

Use `--force-login` only when running from a normal terminal and Codex did not open OAuth automatically:

```bash
bash packages/searm-codex-plugin/scripts/setup-mcp.sh --force-login acme.searm.com
```

Use `--print-url` to preview normalization without changing client config:

```bash
bash packages/searm-codex-plugin/scripts/setup-mcp.sh --print-url myworkspace.localhost:3001
```
...
```bash
codex mcp add searm-local --url http://myworkspace.localhost:3001/mcp
codex mcp login searm-local
```
...
When connected, use the SeaRM MCP discovery flow:
...
Use this workflow when the user reports missing tools, unexpected workspace data, authentication failures, or suspects Codex queried the wrong SeaRM workspace.
...
```bash
bash packages/searm-codex-plugin/scripts/setup-mcp.sh --print-url <workspace-url-or-mcp-url>
```

3. Inspect configured SeaRM MCP servers and compare their URLs to the intended normalized URL:
...
```bash
bash packages/searm-codex-plugin/scripts/setup-mcp.sh --name <server-name> <workspace-url-or-mcp-url>
```
...
7. Before querying workspace data, confirm the callable SeaRM MCP namespace or tool name corresponds to the intended server. If only a different SeaRM server is callable, do not use it as a fallback unless the user explicitly asks.
