...
Use this reference when creating or modifying files inside an existing SeaRM app.
...
Before changing app entities, confirm the current directory is a SeaRM app:
...
Use official SeaRM docs or local SDK source when exact imports, entity fields, or configuration shapes matter.
...
- `src/<service>-client/` — wrappers around external SDKs or HTTP clients. One folder per service, matching SeaRM's `*-client` convention (`redis-client/`, `sdk-client/`, ...).
- Tests as `*.spec.ts` in sibling `__tests__/` folders next to the code they cover.

Filenames are kebab-case; conventional suffixes are `.util.ts`, `.dto.ts`, `.service.ts`, `.spec.ts` — matching the SeaRM backend.
...
`yarn searm dev:typecheck` checks generated app types and `yarn lint` checks local lint rules. `yarn searm dev --once` then builds the app and pushes entity definitions to the active remote; if any definition is invalid, the sync reports the error. Run all three a single time once every edit is done, not repeatedly after each step.

When the user explicitly asks to run tests, follow `tests.md`: unit tests may use the package's unit-test script, and the full suite must run with `SEARM_API_URL=http://localhost:2021` against the isolated test instance.
