...
Use this reference for SeaRM app front component source files, registration, data access, SeaRM UI imports, runtime imports, and browser verification.
...
- Use `searm-sdk/front-component` for front component hooks and host APIs.
- Use `searm-client-sdk/core` or `searm-client-sdk/metadata` for data access.
- Install `searm-ui@1.0.0-alpha.1` from npm and import SeaRM UI components, icons, and theme tokens from its subpaths (`searm-ui/input`, `searm-ui/data-display`, `searm-ui/icon`, `searm-ui/typography`, `searm-ui/theme-constants`, and others) before adding external UI libraries.

The front component renderer provides a SeaRM `ThemeProvider` around the remote root. For isolated examples or local story-style verification, wrapping the component in `ThemeProvider` from `searm-ui/theme-constants` is also acceptable.

The canonical SeaRM UI front component example is `packages/searm-front-component-renderer/src/__stories__/showcase/searm-ui-example.front-component.tsx`. It imports `Button` from `searm-ui/input`; `Chip`, `Status`, and `Tag` from `searm-ui/data-display`; `H2Title` from `searm-ui/typography`; and `ThemeProvider` from `searm-ui/theme-constants`.

Read theme tokens with the `useTheme()` hook from `searm-ui/theme-constants` inside the component body. The same tokens are also exported as the `themeCssVariables` constant, but the SDK mocks `searm-ui` during manifest extraction, so a module-level constant that dereferences `themeCssVariables` can fail before the component renders — prefer `useTheme()`.

Prefer SeaRM UI icons from `searm-ui/icon` when one exists. Use inline SVG only for app-specific marks or icons that are not available from `searm-ui/icon`.
...
Use the generated or core SeaRM client for reads and writes. Keep loading, empty, error, disabled, and saving states explicit so runtime failures are visible and recoverable.
...
Inside `records`, prefer `id` for the SeaRM record ID because the array name already establishes record context. Do not add flat `recordId` payloads unless the user explicitly asks for a single-record action.
...
A clean typecheck and sync is not runtime verification. After the standard validation in `app-structure.md`, open the relevant SeaRM surface and confirm:
