...
Use this reference for the publish-facing documentation and visuals of a SeaRM app. It complements `create-app`; read this when the app is ready for packaging, marketplace listing, npm publishing, private sharing, or user-facing review.

## When To Use

Use this when the user asks to:

- Write or improve a SeaRM app `README.md`.
- Prepare npm package content or SeaRM marketplace copy.
...
SeaRM marketplace metadata comes from `defineApplication()`:
...
One-sentence summary of what the app adds to SeaRM.
...
- SeaRM server/version requirement, if any.
...
4. Open the relevant SeaRM view or app settings page.
...
- Name the SeaRM objects and views users will actually interact with.
...
- Do not use the SeaRM logo as the app logo unless the app is officially owned by SeaRM and the user intends that.

Screenshot guidance:

- Capture real app surfaces after `yarn searm dev --once` or watch-mode sync.
...
If the app requires a specific SeaRM server version, set `engines.searm` in `package.json`. If publishing to npm, add the `searm-app` keyword.
...
- `package.json` has `keywords: ["searm-app"]` when the app is intended for npm marketplace publishing.
