import { type CommandMenuContextApi } from 'searm-shared/types';
...
const createExpressionOnlyPlaceholder = (name: string): unknown => {
  const fail = (): never => {
    throw new Error(
      `"${name}" from searm-sdk can only be used inside a command's conditionalAvailabilityExpression, not at runtime.`,
    );
  };
