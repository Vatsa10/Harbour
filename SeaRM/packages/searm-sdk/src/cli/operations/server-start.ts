...
const innerServerStart = async (
  ...
  if (existingUrl) {
    ...
    onProgress?.(`SeaRM server detected on ${existingUrl}`);
    ...
    if (!healthy) {
      return {
        ...
        error: {
          ...
          message:
            'SeaRM server did not become healthy in time.\n' +
            "Check: 'yarn searm docker:logs'",
        },
      };
    }
  ...
  if (containerExists(containerName)) {
    ...
    if (existingPort !== port) {
      onProgress?.(
        `Existing container uses port ${existingPort}. Run 'yarn searm docker:reset${isTest ? ' --test' : ''}' first to change ports.`,
      );
    }
    ...
    onProgress?.(
      `Pulling Docker image (${image}) and starting SeaRM container...`,
    );
    ...
    const runResult = spawnSync(
      ...
      [
        ...
        '-v',
        `${volumeStorage}:/app/packages/searm-server/.local-storage`,
        image,
      ],
      { stdio: 'inherit' },
    );
    ...
    if (runResult.status !== 0) {
      return {
        ...
        error: {
          ...
          message: 'Failed to start SeaRM container.',
        },
      };
    }
  }

  onProgress?.('Waiting for SeaRM to be ready...');
