...
import { SyncableEntity } from 'searm-shared/application';
...
export const registerDevCommands = (program: Command): void => {
  ...
  const devAction = async (
    ...
    if (options.dryRun && !options.once) {
      console.warn(
        chalk.yellow(
          '--dry-run only applies with --once. Ignoring it; run `yarn searm plan` to preview changes.',
        ),
      );
    }
    ...
    if (options.once) {
      console.warn(
        chalk.yellow(
          options.dryRun
            ? '⚠ `searm dev --once --dry-run` is deprecated. Use `searm plan` instead.'
            : '⚠ `searm dev --once` is deprecated. Use `searm apply` instead.',
        ),
      );
  ...
  program
    ...
    .option(
      '-o, --once',
      'Build and sync once, then exit (deprecated: use `searm apply`)',
    )
    .option(
      '--dry-run',
      'Preview the metadata changes without applying them (deprecated: use `searm plan`)',
    )
    .option(
      '-f, --force',
