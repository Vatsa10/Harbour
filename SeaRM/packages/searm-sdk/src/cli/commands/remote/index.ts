...
import { normalizeUrl } from 'searm-shared/utils';
  ...
  if (serverUrl) {
    ...
    if (options.local) {
      if (!detectedUrl) {
        console.error(
          chalk.red(
            'No local SeaRM server found.\n' +
              'Start one with: yarn searm docker:start',
          ),
        );
        process.exit(1);
      }
...
const listAction = async () => {
  ...
  if (remotes.length === 0) {
    ...
    console.log("Use 'searm remote:add' to add one.");
  ...
  for (const remoteName of remotes) {
    ...
    const authMethod = config.searmCLIAccessToken
      ? 'oauth'
      : config.apiKey
        ? 'api-key'
        : 'none';
  ...
  console.log(
    '\n',
    chalk.gray("Use 'searm remote:use <name>' to change default"),
  );
};
...
export const registerRemoteCommands = (program: Command): void => {
  program
    ...
    .option('--local', 'Connect to a local SeaRM server (auto-detect)')
  ...
  const remote = program
    ...
    .description('Manage remote SeaRM servers');

  const deprecate = (oldCmd: string, newCmd: string) =>
    console.warn(
      chalk.yellow(
        `⚠ \`searm remote ${oldCmd}\` is deprecated. Use \`searm ${newCmd}\` instead.`,
      ),
    );
