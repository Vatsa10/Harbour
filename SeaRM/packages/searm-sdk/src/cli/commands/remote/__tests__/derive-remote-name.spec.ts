...
describe('deriveRemoteName', () => {
  it('should use only the subdomain when the host has one', () => {
    expect(deriveRemoteName('https://martin-s-workspace.searm.com')).toBe(
      'martin-s-workspace',
    );
  });

  it('should join multiple subdomain labels with dashes', () => {
    expect(deriveRemoteName('https://app.staging.searm.com')).toBe(
      'app-staging',
    );
  });

  it('should drop the tld when there is no subdomain', () => {
    expect(deriveRemoteName('https://searm.com')).toBe('searm');
  });
