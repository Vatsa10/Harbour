...
const stopAction = (options: { test?: boolean }) => {
  ...
  if (!containerExists(containerName)) {
    console.log(chalk.yellow('No SeaRM server container found.'));

    return;
  }

  execSync(`docker stop ${containerName}`, { stdio: 'ignore' });
  console.log(chalk.green('SeaRM server stopped.'));
};
...
const statusAction = async (options: { test?: boolean }) => {
  ...
  if (!containerExists(containerName)) {
    ...
    console.log(
      chalk.gray(
        `  Run 'yarn searm docker:start${options.test ? ' --test' : ''}' to create one.`,
      ),
    );
...
const resetAction = (options: { test?: boolean }) => {
  ...
  const volumeData = options.test
    ? 'searm-app-dev-test-data'
    : 'searm-app-dev-data';
  const volumeStorage = options.test
    ? 'searm-app-dev-test-storage'
    : 'searm-app-dev-storage';
  ...
  console.log(chalk.green('SeaRM server data reset.'));
  console.log(
    chalk.gray(
      `Run 'yarn searm docker:start${options.test ? ' --test' : ''}' to start a fresh instance.`,
    ),
  );
};
...
const upgradeAction = async (
  ...
  if (data.containerRecreated) {
    console.log(
      chalk.gray(
        `  Run 'yarn searm docker:start${options.test ? ' --test' : ''}' to wait for the server to be ready.`,
      ),
    );
  }
};
...
export const registerServerCommands = (program: Command): void => {
  program
    ...
    .description(
      'Start the local SeaRM container (version defaults to the app `engines.searm` range, then `latest`)',
    )
  ...
  program
    .command('docker:stop')
    .description('Stop the local SeaRM container')
  ...
  const server = program
    ...
    .description(
      'Manage a SeaRM server (local instance and server-side actions)',
    );

  const deprecate = (oldCmd: string, newCmd: string) =>
    console.warn(
      chalk.yellow(
        `⚠ \`searm server ${oldCmd}\` is deprecated. Use \`searm ${newCmd}\` instead.`,
      ),
    );
