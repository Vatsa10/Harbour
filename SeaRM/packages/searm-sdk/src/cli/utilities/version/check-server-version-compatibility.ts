...
export const checkServerVersionCompatibility = async (
  ...
  console.warn(
    chalk.yellow(
      `⚠ Local SeaRM server is v${info.localServerVersion} (${info.daysBehind} days behind v${info.latestServerVersion}).`,
    ),
  );
  console.warn(chalk.dim('  Update with: yarn searm docker:upgrade'));
  console.warn('');
};
