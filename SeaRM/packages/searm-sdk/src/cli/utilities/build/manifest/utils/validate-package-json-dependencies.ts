...
// Neither SeaRM SDK package needs to be resolved by the published app at
// runtime, so both must live in "devDependencies":
// - searm-sdk ships the CLI and build/scaffolding tooling used only at dev and
//   build time, and is never imported by the published app's runtime.
// - searm-client-sdk is imported by app code but is provided at runtime by
//   SeaRM's injected SDK (Lambda SDK layer / server-served modules), so the
...
const BUILD_TIME_DEPENDENCY_WARNINGS: Record<string, string> = {
  'searm-sdk':
    '"searm-sdk" is listed under "dependencies" in package.json. It is a build-time only tool and should be moved to "devDependencies".',
  'searm-client-sdk':
    '"searm-client-sdk" is listed under "dependencies" in package.json. It is provided at runtime by SeaRM\'s injected SDK and should be moved to "devDependencies".',
};
