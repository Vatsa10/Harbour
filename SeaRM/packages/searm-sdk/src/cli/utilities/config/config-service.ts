...
export type RemoteConfig = {
  apiUrl: string;
  apiKey?: string;
  // CLI OAuth app credentials (from `yarn searm remote:add`)
  searmCLIRegistrationId?: string;
  searmCLIRegistrationClientId?: string;
  searmCLIAccessToken?: string;
  searmCLIRefreshToken?: string;
...
export class ConfigService {
  ...
  // to the current format (remotes, searmCLIAccessToken/searmCLIRefreshToken)
  private async migrateConfigIfNeeded(
    ...
    const migrateRemoteFields = (
      ...
      searmCLIRegistrationClientId:
        str(source.searmCLIRegistrationClientId) ?? str(source.oauthClientId),
      searmCLIAccessToken:
        str(source.searmCLIAccessToken) ??
        str(source.accessToken) ??
        str(source.applicationAccessToken),
      searmCLIRefreshToken:
        str(source.searmCLIRefreshToken) ??
  ...
  async getConfigForRemote(remoteName: string): Promise<RemoteConfig> {
    ...
    try {
      ...
      return {
        ...
        searmCLIRegistrationId: remoteConfig.searmCLIRegistrationId,
        searmCLIRegistrationClientId:
          remoteConfig.searmCLIRegistrationClientId,
        searmCLIAccessToken: remoteConfig.searmCLIAccessToken,
        searmCLIRefreshToken: remoteConfig.searmCLIRefreshToken,
