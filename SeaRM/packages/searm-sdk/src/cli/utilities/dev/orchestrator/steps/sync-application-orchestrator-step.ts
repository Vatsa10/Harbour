...
import { type MetadataValidationErrorResponse } from 'searm-shared/metadata';
  ...
  private async gateDestructiveChange(
    ...
    if (!this.interactive) {
      stop(
        ...
        `Stopping: ${deleteCount} destructive change(s) need confirmation. Re-run with \`yarn searm dev --force\` to apply deletions.`,
      );
    ...
    if (!approved) {
      stop(
        ...
        `Stopping: declined ${deleteCount} destructive change(s). Re-run with \`yarn searm dev --force\` to apply deletions or \`yarn searm plan\` to preview changes.`,
      );
