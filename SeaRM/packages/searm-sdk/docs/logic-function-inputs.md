...
- `SearmRecord<'objectUniversalIdentifier'>` maps to a record input (see below).
...
To bind an input to a workspace object, type it with `SearmRecord`, passing the
object's universal identifier as a string literal:

```ts
import { defineLogicFunction, type SearmRecord } from 'searm-sdk/define';

const handler = async (params: {
  companyId: SearmRecord<'20202020-b374-4779-a561-80086cb2e17f'>;
  postCardIds: SearmRecord<'54b589ca-eeed-4950-a176-358418b85c05'>[];
}) => {
  return {
...
- **Standard objects**: get the identifier from `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`
  (exported from `searm-sdk/define`), e.g.
  `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS.company.universalIdentifier`.
- **App objects**: use the `universalIdentifier` you set on the object's
  `defineObject(...)`.

Only a string-literal argument resolves. `SearmRecord` with no argument, or with
a non-literal argument, is treated as an unknown input.

## What the handler receives

`SearmRecord<TUid>` is a branded `string`: `companyId` is a record id, and
