...
const fetchContributorStats = async (username: string) => {
  const apiUrl = `https://searm.com/api/contributors/contributorStats/${username}`;
...
const fetchContributorImage = async (username: string) => {
  const apiUrl = `https://searm.com/api/contributors/${username}/og.png`;
...
const runCongratulate = async () => {
  ...
  const { data: pullRequests } =
    await danger.github.api.rest.search.issuesAndPullRequests({
      q: `is:pr author:${userName} is:closed repo:Vatsa10/Harbour`,
      per_page: 2,
      page: 1,
    });
  ...
  const contributorUrl = `https://searm.com/contributors/${userName}`;
  ...
  const message =
    ...
    `![Contributions](https://searm.com/api/contributors/${userName}/og.png)`;
