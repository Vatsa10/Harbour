#!/bin/bash
# =============================================================================
# SeaRM CRM — Development Environment Setup
# =============================================================================
...
#   bash packages/searm-utils/setup-dev-env.sh          # start + configure
#   bash packages/searm-utils/setup-dev-env.sh --down    # stop services
#   bash packages/searm-utils/setup-dev-env.sh --reset   # wipe data + restart
#   bash packages/searm-utils/setup-dev-env.sh --docker  # force Docker mode
# =============================================================================
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
COMPOSE_FILE="$REPO_ROOT/packages/searm-docker/docker-compose.dev.yml"
...
if command -v npx &>/dev/null && [ -d node_modules ]; then
  npx nx reset:env searm-front
  npx nx reset:env searm-server
else
  for pkg in searm-front searm-server; do
...
if command -v npx &>/dev/null && [ -d node_modules ]; then
  if schema_exists; then
    ...
    if npx nx database:init searm-server; then
  ...
  info "Run 'npx nx database:init searm-server' to initialize the schema"
fi
...
echo "  npx nx start searm-front          # frontend  -> http://localhost:3001"
echo "  npx nx start searm-server         # backend   -> http://localhost:3000"
echo ""
