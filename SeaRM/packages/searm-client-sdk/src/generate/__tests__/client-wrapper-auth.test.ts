...
const searmClientTemplateSource = readFileSync(
  join(__dirname, '..', 'searm-client-template.ts'),
  'utf-8',
);

type SearmClassType = new (options?: {
  url?: string;
...
describe('Generated client wrapper auth behavior', () => {
  ...
  let SearmClass: SearmClassType;

  beforeAll(async () => {
    temporaryDir = await mkdtemp(join(tmpdir(), 'searm-generated-client-'));

    const wrapperSource = buildClientWrapperSource(searmClientTemplateSource, {
      apiClientName: 'MetadataApiClient',
      defaultUrl: '`${process.env.SEARM_API_URL}/metadata`',
    ...
    SearmClass = generatedModule.MetadataApiClient as SearmClassType;
  });

  beforeEach(() => {
    delete process.env.SEARM_APP_ACCESS_TOKEN;
    delete process.env.SEARM_API_KEY;
  ...
  it('uses SEARM_APP_ACCESS_TOKEN before SEARM_API_KEY', async () => {
    process.env.SEARM_APP_ACCESS_TOKEN = 'application-token';
    process.env.SEARM_API_KEY = 'api-key-token';
    ...
    const searmClient = new SearmClass({
      url: 'https://example.com/graphql',
      fetch: fetchMock as unknown as typeof globalThis.fetch,
    });

    await searmClient.query({ record: { id: true } });

    expect(capturedAuthorizationHeaders).toEqual(['Bearer application-token']);
  });

  it('falls back to SEARM_API_KEY when SEARM_APP_ACCESS_TOKEN is absent', async () => {
    process.env.SEARM_API_KEY = 'legacy-api-key-token';
  ...
  it('refreshes and retries once after auth error when refresh callback is available', async () => {
    process.env.SEARM_APP_ACCESS_TOKEN = 'stale-token';
    process.env.SEARM_API_KEY = 'legacy-api-key-token';
    ...
    const queryResult = await searmClient.query({ record: { id: true } });

    expect(queryResult).toEqual({ data: { record: { id: 'record-id' } } });
    expect(requestAccessTokenRefresh).toHaveBeenCalledTimes(1);
    expect(fetchMock).toHaveBeenCalledTimes(2);
    expect(process.env.SEARM_APP_ACCESS_TOKEN).toBe('fresh-token');
    expect(process.env.SEARM_API_KEY).toBe('legacy-api-key-token');
  });
  ...
  it('deduplicates concurrent token refresh requests', async () => {
    ...
    await Promise.all([
      searmClient.query({ record: { id: true } }),
      searmClient.query({ record: { id: true } }),
    ]);
  ...
  it('retries uploadFile once after 401 when refresh callback is available', async () => {
    ...
    const uploadResult = await searmClient.uploadFile(
      Buffer.from('content'),
      'test.txt',
      'text/plain',
      'field-uuid',
  ...
  it('bubbles auth error when refresh callback throws', async () => {
    ...
    await expect(searmClient.query({ record: { id: true } })).rejects.toThrow(
      'Unauthorized',
    );
    expect(requestAccessTokenRefresh).toHaveBeenCalledTimes(1);
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      'SeaRM client: token refresh failed',
      expect.any(Error),
    );
