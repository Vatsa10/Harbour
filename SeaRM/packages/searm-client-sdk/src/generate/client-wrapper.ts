...
export const buildClientWrapperSource = (
  ...
  source = source.replace("'__SEARM_DEFAULT_URL__'", options.defaultUrl);

  source = source.replace(/SearmGeneratedClient/g, options.apiClientName);
  ...
  return `\n// ${options.apiClientName} (auto-injected by searm-client-sdk)\n${source}`;
};
