...
`listr`, etc.) that SeaRM never executed.
...
- **Dropped the live-endpoint introspection path** (`schema/fetchSchema.ts`),
  which was the only consumer of `undici` / `native-fetch` / `qs`. SeaRM always
...
- **Replaced `@graphql-tools/load`** with graphql's own `buildSchema` — SeaRM
...
- **`Config` was narrowed** to the schema-string inputs SeaRM actually passes
  ...
  generated client still defaults its url/fetch to `undefined` (SeaRM's wrapper
