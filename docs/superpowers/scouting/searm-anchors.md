# SeaRM Codebase Anchors — Scouting Report

Base commit context: this scout was run against the `searm` checkout at
`d:/Files/Vatsa/Projects/AI-CRM/searm` (no git metadata present in this
...
`d:/Files/Vatsa/Projects/AI-CRM/searm/packages/` unless stated otherwise.
...
**Headline finding: SeaRM is dramatically further along the charter's
...
`searm-server/src/engine/metadata-modules/ai/`:
...
**What this means for the charter's `AgentRun`/`AgentTask`:** SeaRM has
...
- `core-modules/tool-provider/` — the **registry and dynamic tool
  ...
  **This is the extension point for a new tool category** (e.g. a
  ...
  — i.e. SeaRM already implements a **progressive tool-disclosure
...
- `mcp/constants/mcp-excluded-tool-names.const.ts`,
  ...
  `mcp-execute-tool-annotations.const.ts` — SeaRM already tags tools with
...
1. **Cron fan-out job** —
   ...
   lease/idempotency mechanism SeaRM already uses for scheduling durable
...
**This is SeaRM's existing identity-matching primitive** — the charter's
...
`searm-front/src/modules/object-record/spreadsheet-import/` (the
generic, object-aware import UI: field mapping, validation, preview) and
`searm-front/src/modules/spreadsheet-import/` (lower-level spreadsheet
...
`searm-front/src/modules/onboarding/components/import-contacts/`, is the
...
finalizing the import plan, grep `searm-server/src` for `spreadsheet` and
...
`searm-front/src/modules/spreadsheet-import` and
...
From `packages/searm-apps/fixtures/rich-app/src/` (a real, working test
...
- **Front-end**: `components/*.front-component.tsx`,
  ...
  and rendered by `searm-sdk`'s `front-component-renderer`.
- **Agents & skills**: confirmed in
  `packages/searm-apps/examples/hello-world/src/agents/example-agent.ts`
...
- **Connections**: `connection-provider` converter exists — apps can
  ...
  mechanism the public apps under `searm-apps/public/` — `slack`,
...
- **Upgrade**: `application-upgrade/application-upgrade.service.ts` +
  ...
  **a scheduled version-check cron**,
  ...
  i.e. SeaRM already periodically checks installed apps for newer
  versions.
...
`searm-sdk` (`packages/searm-sdk/src/`) is the authoring SDK apps import
...
etc.) actually import types from. `searm-cli` is a thin package
(`deprecate.js` + `package.json`/`README.md` only — appears to be a
deprecated/stub package now superseded by `searm-sdk`'s own `cli/`
subfolder; **verify this before citing `searm-cli` as the live CLI** —
the evidence points to `searm-sdk`'s bundled CLI, not a separate
`searm-cli` package, being the actual tool).
...
- **Standard object** (e.g. adding a new built-in object like `Person`):
  requires touching `searm-shared/src/metadata/constants/standard-object.constant.ts`
  ...
  `searm-server/src/engine/workspace-manager/searm-standard-application/`
  ...
  This is multi-file, cross-package (touches `searm-shared`, which is
  also consumed by `searm-front`), and carries a **migration obligation
...
- **Core-schema TypeORM entity** (e.g. `ProposalEntity`,
  ...
  for the `core` schema. No `searm-shared` metadata registry entry, no
...
| Charter entity | What SeaRM already has | Gap to close |
| --- | --- | --- |
...
1. **Record contract** (every action uses SeaRM objects/fields/relations/
...
5. **Principal contract** (audit distinguishes user/represented user/
   ...
   ActorMetadata` (from `searm-shared/types`) suggests a shared
...
- `searm-cli` vs `searm-sdk`'s bundled CLI — `searm-cli` looks
  vestigial (`deprecate.js` only); confirm which one is live before
  citing either in a plan.
- `ActorMetadata` type definition (`searm-shared/types`) — not opened;
