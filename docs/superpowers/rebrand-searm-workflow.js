export const meta = {
  name: 'rebrand-searm',
  description: 'Rename the fork from Twenty to SeaRM end to end, on an isolated branch, with a restore point and verification between every wave',
...
1. **Universal identifier UUIDs.** Every value in \`twenty-shared/src/metadata/constants/*\` is a stable key written into workspace metadata rows. A changed UUID orphans real data. Do not touch a single one.
2. **Postgres schema names** — \`core\`, \`metadata\`, and every per-workspace schema. They exist in the running database.
3. **Database table and column names.** \`proposal\`, \`proposalItem\`, \`evidence\`, \`fact\`, \`agentTask\`, \`agentRun\`, and every Twenty table. The entities map to real tables.
...
7. **The standard application identity.** \`twenty-standard-application\` and \`TWENTY_STANDARD_ALL_METADATA_NAME\` — VERIFY whether the application is keyed by this name in the database before touching it. If it is persisted, it stays, however much it looks like branding.
8. **LICENSE and copyright notices.** This is AGPL-3.0 code. Twenty's copyright headers and the LICENSE file stay exactly as they are. You may ADD SeaRM attribution alongside; you may never remove or alter Twenty's. Removing them is a licence violation, not a style choice.
...
\`twenty-cli\` is a strict prefix of \`twenty-client-sdk\`: a naive match counts 1201 hits where only 14 are real. \`twenty-front\` is a prefix of \`twenty-front-component-renderer\`: 780 vs 731. Rename in the wrong order and **1,229 references corrupt silently**. Match on exact boundaries and rename \`twenty-client-sdk\` before \`twenty-cli\`, \`twenty-front-component-renderer\` before \`twenty-front\`.

**L4 — the app packages pin REGISTRY versions, not \`workspace:*\`.**
The 19 packages under \`packages/twenty-apps\` depend on \`twenty-sdk\` / \`twenty-client-sdk\` at published versions (2.13.0 … 2.27.0, "latest") with their own \`yarn.lock\`, plus \`.yarnrc.yml\` enforcing \`npmMinimalAgeGate: 3d\` and an \`npmPreapprovedPackages\` allowlist. **Renaming those two packages before \`searm-sdk\` exists on the registry makes every app un-installable and reds 6 CI workflows.** Either keep the SDK package names until they are published under the new scope, or convert the apps to \`workspace:*\` first. Decide this explicitly and record it; do not discover it mid-wave.

**L2/L5 — live third-party coordinates that must NOT be rewritten.**
\`twenty.com\` (2330 hits), \`twentyhq\` (1745, including the upstream git remote and every GitHub Actions \`uses:\` reference), \`twentycrm/*\` Docker Hub images (305), \`LICENSE\` line 7 and 735, and 312 \`@license Enterprise\` markers. Also frozen on-disk identities: \`.twenty/output\`, \`~/.twenty\`, and the env names \`TWENTY_API_KEY\`, \`TWENTY_API_URL\`, \`TWENTY_APP_ACCESS_TOKEN\`, \`TWENTY_FUNCTIONS_URL\`. These name another project or a persisted location — renaming them breaks CI, pulls, or existing installs.

**Standard application — resolved.** The lookup is by UUID, not by any string containing "twenty": \`application.service.ts:394-397\` → \`TWENTY_STANDARD_APPLICATION.universalIdentifier\` → \`'20202020-64aa-4b6f-b003-9c74b97cee20'\`, and an instance command embeds that UUID in raw SQL. Frozen: the UUID, the persisted name value \`'Standard'\`, and the 18 values of \`TWENTY_STANDARD_ALL_METADATA_NAME\`. The directory and \`TWENTY_STANDARD_*\` symbols are source-only — leave them anyway, since renaming gains nothing and risks the constants they wrap.
...
Directory names \`packages/twenty-*\` → \`packages/searm-*\`; package.json \`name\` fields; the \`@twenty\` npm scope; tsconfig path aliases; Nx project names and project.json; import specifiers (\`twenty-shared/...\` → \`searm-shared/...\`); jest/vite/storybook config references; Docker image names and compose service names; Helm and k8s manifests; CI workflow references; the CLI binary name and every script that invokes it; README, docs, and all user-visible product strings.
...
A grep-and-replace across the repo is NOT acceptable on its own. \`twenty\` appears inside words, inside UUIDs, inside URLs to twenty.com, and inside the do-not-touch list above. Match on precise boundaries and review every hunk.
...
const survey = await agent(`Produce the complete rename map for turning this Twenty fork into **SeaRM**, before anything is edited.
...
2. **Dependency order.** Which package must be renamed before which, based on who imports whom. \`twenty-shared\` is imported by nearly everything, so it likely goes first and everything else follows — verify rather than assume.
3. **Every reference site class**, with a representative count from a real grep: tsconfig path aliases, nx project names, project.json, import specifiers, jest configs, vite configs, docker files, helm/k8s, CI workflows, package.json scripts, and the app packages under \`packages/twenty-apps\` that invoke the CLI.
4. **The CLI binary.** Where \`twenty\` is registered as a bin, and every script anywhere in the repo that calls \`npx twenty ...\` — including inside \`packages/twenty-apps/public/customer-support\`.
...
6. **Landmines.** Every place the literal string \`twenty\` appears where renaming would be WRONG: URLs to twenty.com, the LICENSE, copyright headers, UUID fragments, words containing "twenty", upstream remote references, and any user data.
...
Two hard limits: do NOT alter the LICENSE or any copyright header, and do NOT rewrite links to twenty.com that point at genuine upstream Twenty documentation — those are references to another project, not our branding.`, {
...
6. **Licence audit.** LICENSE unchanged, every Twenty copyright header intact, SeaRM attribution added rather than substituted. State plainly whether the fork still complies with AGPL-3.0.
7. **No dangling references.** Grep for surviving \`twenty-\` package references, \`@twenty\` scope imports, and \`npx twenty\` invocations. Distinguish genuine leftovers from legitimate references to upstream Twenty.
