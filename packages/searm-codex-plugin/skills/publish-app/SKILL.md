---
name: publish-app
description: Use when the user wants to prepare or verify a SeaRM app for npm or marketplace publication, including README/About copy, package metadata, defineApplication marketplace metadata, logos, screenshots, and public assets.
---

# When To Use

Pick this skill when the user wants to make an existing SeaRM app *presentable* — listing copy, branding, public assets, and publication. Representative triggers:

- "prepare my SeaRM app for the marketplace"
- "write the README for my SeaRM app"
...
Do not use this skill to scaffold (use `create-app`), to change app entities (use `develop-app`), to deploy a private build to a specific SeaRM server (use `manage-app`), or to query workspace records (use `use-searm-mcp`).
...
- For npm marketplace publication, ensure `package.json` has a valid bumped `version` and the `searm-app` keyword.
...
- Validate with `yarn searm dev:build`; publish with `yarn searm app:publish` or `yarn searm app:publish --tag <tag>`.
...
If the needed publishing detail is unclear, search the official SeaRM docs through the bundled `searm-docs` MCP server before guessing.
