...
Use this reference when a SeaRM app needs a full-page custom UI: an operational console, map, canvas, planner, status wall, or other page-sized tool.

For general page layout and navigation entities, use `layout.md`. For front component source, runtime imports, hooks, data access, and browser verification, use `front-components.md`. For visual polish and SeaRM UI component choices, use `../design/front-component-ui.md`. For CLI and deployment command details, use `../manage-app/cli-and-sync.md`.
...
6. SeaRM resolves the sidebar item to the `/page/:pageLayoutId` route and renders the front component widget inside the page layout.
...
- Use `searm-sdk/front-component` for runtime hooks, host navigation, snackbars, application variables, and side panel actions.
- Use `searm-client-sdk/core` for workspace records when the page is data driven.
- Use `getPublicAssetUrl` from `searm-sdk/define` for app-bundled images or static files.
...
```tsx
import { defineFrontComponent, getPublicAssetUrl } from 'searm-sdk/define';
import { getApplicationVariable } from 'searm-sdk/front-component';
...
Assume the available area changes with SeaRM chrome, the left sidebar, side panel state, tab list, and smaller screens. Use responsive CSS inside the front component:
...
Use `front-components.md` for general client/runtime rules. In standalone pages, the key additions are visible full-page loading, empty, and error states, plus navigation from the standalone surface back into SeaRM records.

```tsx
import { useEffect, useState } from 'react';
import { CoreApiClient, type CoreSchema } from 'searm-client-sdk/core';
import { defineFrontComponent } from 'searm-sdk/define';
import {
  AppPath,
  enqueueSnackbar,
  navigate,
} from 'searm-sdk/front-component';
...
- Confirm the app appears in SeaRM settings or the application developer view.
