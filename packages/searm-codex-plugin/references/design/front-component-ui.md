...
Use this when the user wants to design or polish a SeaRM front component UI.
...
Design front components as compact CRM product UI, not marketing pages. The goal is a surface that scans quickly, feels native inside SeaRM, and keeps actions predictable.

Use Figma measurements as evidence for the current SeaRM rhythm, not as implementation literals. Source code should use SeaRM primitives, component props, and `themeCssVariables` instead of hardcoded visual values.

## SeaRM UI Defaults

Prefer SeaRM UI primitives for CRM-native front component UI:
...
Use local inline styles for layout containers and custom data displays, but keep them aligned with SeaRM tokens. Use `front-components.md` for exact imports and runtime rules.

Use SeaRM UI icons before custom SVG. Use `themeCssVariables.icon.size.md` for default row and action icons, `themeCssVariables.icon.size.sm` for quiet chevrons, and `themeCssVariables.spacing[6]` for icon-only action targets.
...
- Icons use `themeCssVariables.icon.size.*` or native SeaRM UI icon sizing.
- Borders use `themeCssVariables.border.color.*`; avoid raw border colors.
- Shadows use `themeCssVariables.boxShadow.*`; avoid custom shadow stacks unless mirroring an existing SeaRM overlay exactly.
...
- Compact controls: use SeaRM `Button` sizing or `themeCssVariables.spacing[6]` for custom control height.
...
- Checkbox shape and target sizing should come from the native SeaRM checkbox pattern when available.
- Do not use pill or rounded radius for cards, rows, or toolbar buttons unless the existing SeaRM primitive already does.
- Do not mix several custom radii in one component. Pick the matching SeaRM radius token for each element role.
...
SeaRM UI looks precise because repeated edges line up:
...
- Use SeaRM tokens for color and spacing. Use raw values only for semantic data visualization that cannot be represented by existing tokens.

State backgrounds should follow the existing SeaRM pattern:
...
- Selected: same shape and height as the default row, with selected color treatment from the nearest SeaRM primitive.
...
Use actions that feel like SeaRM toolbar controls:

- Use `Button` for actual commands.
- Use SeaRM button `size` and `variant` props before custom button styling.
...
Make text fit in real SeaRM widths:
...
- The component uses SeaRM typography tokens, not custom text sizes.
- Cards use the medium radius token or a smaller existing primitive radius.
- Buttons and action controls use SeaRM `Button` props or compact spacing tokens.
- Table/list rows use a consistent row-height token.
- Icons use `themeCssVariables.icon.size.*` or native SeaRM icon sizing.
- Spacing uses `themeCssVariables.spacing[...]` and `themeCssVariables.betweenSiblingsGap`.
...
- The UI uses SeaRM primitives and `themeCssVariables` before custom styling.
...
2. Lock the structural dimensions with SeaRM primitives and `themeCssVariables`.
3. Align repeated edges and action positions.
4. Apply SeaRM primitives and tokens.
