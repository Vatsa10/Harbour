...
const assertSearmMcpFormattingContract = (fail) => {
  const skillPath = path.join(PLUGIN_ROOT, 'skills/use-searm-mcp/SKILL.md');
  const resultFormattingPath = path.join(PLUGIN_ROOT, 'references/use-searm-mcp/result-formatting.md');
  ...
  const requiredSkillFragments = [
    ...
    'MUST link each display name back to SeaRM',
  ...
  for (const fragment of requiredSkillFragments) {
    if (!skill.includes(fragment)) {
      fail(`use-searm-mcp/SKILL.md is missing formatting contract fragment: ${fragment}`);
    }
  }
...
const assertFrontComponentGuidance = (fail) => {
  ...
  const requiredDevelopSkillFragments = [
    ...
    'SeaRM UI imports',
    'Use `layout.md` for placement, `standalone-pages.md` for full-page custom UI, and `front-component-ui.md` for visual design and SeaRM UI component selection',
  ];
  ...
  const requiredFrontComponentFragments = [
    ...
    'Use `searm-sdk/front-component`',
    'Use `searm-client-sdk/core` or `searm-client-sdk/metadata`',
    'searm-ui@1.0.0-alpha.1',
    'import SeaRM UI components, icons, and theme tokens from its subpaths',
    'ThemeProvider',
    'showcase/searm-ui-example.front-component.tsx',
    'themeCssVariables',
    'mocks `searm-ui` during manifest extraction',
...
const assertCliGuidanceSplit = (fail) => {
  ...
  const forbiddenAppStructureFragments = [
    ...
    'Do not run `yarn searm dev:typecheck`',
  ...
  const requiredCliFragments = [
    ...
    'Do not use bare `yarn searm dev` (watch mode)',
    'yarn searm dev --once --verbose',
    'yarn searm remote:list',
...
module.exports = {
  assertSearmMcpFormattingContract,
