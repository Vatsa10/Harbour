...
const assertJsonMetadata = (fail) => {
  ...
  const packageJson = readJson('packages/searm-codex-plugin/package.json');
  const pluginJson = readJson('packages/searm-codex-plugin/.codex-plugin/plugin.json');
  const mcpJson = readJson('packages/searm-codex-plugin/.mcp.json');
  ...
  const marketplaceEntry = marketplaceJson?.plugins?.find((entry) => entry.name === 'searm');

  if (marketplaceJson && !marketplaceEntry) {
    fail('.agents/plugins/marketplace.json includes plugins but not the searm plugin entry');
  } else if (marketplaceEntry && marketplaceEntry.source?.path !== './packages/searm-codex-plugin') {
    fail('marketplace searm source path must be ./packages/searm-codex-plugin');
  }

  if (fs.existsSync(path.join(REPO_ROOT, 'plugins', 'searm'))) {
    fail('legacy plugins/searm path must not exist; use packages/searm-codex-plugin directly');
  }
};
...
const assertNoBundledMcpConfig = (fail) => {
  ...
  if (fs.existsSync(gitignorePath) && readText(gitignorePath).split(/\r?\n/).includes('.mcp.json')) {
    fail('packages/searm-codex-plugin/.gitignore must not ignore the public .mcp.json');
  }
...
const assertMarketplaceTemplate = (fail) => {
  const { readJson, readOptionalJson } = createJsonReaders(fail);
  const templatePath = 'packages/searm-codex-plugin/templates/marketplace.example.json';
  ...
  const searmEntry = entries.find((entry) => entry?.name === 'searm');

  if (!searmEntry) {
    fail(`${templatePath} must include a plugin entry named "searm"`);
    return;
  }

  if (searmEntry.version !== pluginJson?.version) {
    fail(`${templatePath} searm.version must match plugin.json version`);
  }

  if (searmEntry.source?.path !== './packages/searm-codex-plugin') {
    fail(`${templatePath} searm.source.path must be ./packages/searm-codex-plugin`);
  }

  if (!searmEntry.policy?.installation) {
    fail(`${templatePath} searm.policy.installation is required`);
  }

  if (!searmEntry.policy?.authentication) {
    fail(`${templatePath} searm.policy.authentication is required`);
  }

  if (searmEntry.category !== pluginJson?.interface?.category) {
    fail(`${templatePath} searm.category must match plugin.json interface.category`);
  }
};
