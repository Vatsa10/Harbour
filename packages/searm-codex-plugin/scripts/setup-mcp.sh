...
Examples:
  setup-mcp.sh acme.searm.com
  setup-mcp.sh --force-login https://crm.example.com
  setup-mcp.sh --name searm-local acme.localhost:3001
  setup-mcp.sh --name searm-prod https://crm.example.com/mcp
...
Environment:
  SEARM_MCP_URL   MCP URL to use when no URL argument is provided.
EOF
}
...
if [[ -z "$url" ]]; then
  echo "A SeaRM workspace URL is required, for example: https://crm.example.com" >&2
  echo
  usage
  exit 1
fi
