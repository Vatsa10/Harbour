...
All notable changes to the SeaRM Codex plugin are documented in this file.
...
Entries reference the canonical skills (`create-app`, `develop-app`, `manage-app`, `publish-app`, `use-searm-mcp`) and the validation script (`scripts/validate.js`).

## [0.1.0] - Unreleased

The first version of the SeaRM Codex plugin.
...
- `references/develop-app/tests.md` and `references/manage-app/cli-and-sync.md`: documented that integration tests must run against the isolated test instance (`yarn searm docker:start --test`, port `2021`) instead of the dev instance, since the test harness installs and uninstalls the app on its target server.
- `skills/manage-app/SKILL.md`: added direct test-run routing so "run tests" requests load the tests reference and run full suites with `SEARM_API_URL=http://localhost:2021`.
...
- `.mcp.json` declaring only the public `searm-docs` MCP server at `https://docs.searm.com/mcp`.
- `assets/searm-logo.png` and `assets/searm-logo.svg` for marketplace branding.

### Skills (`skills/`)
- `create-app` — scaffold a new SeaRM app via `create-searm-app`, with prompts for the SeaRM instance URL and Docker vs existing-instance choice.
- `develop-app` — add or modify SeaRM app entities (objects, fields, logic functions, roles, views, layouts, skills, agents, connection providers, front components); enforces `yarn searm dev:add` for entity generation and one-shot `yarn searm dev --once` sync.
...
- `use-searm-mcp` — workspace MCP connection, record retrieval, and readable Markdown output with linked record names.
...
- `design/front-component-ui.md` — visual design rules, SeaRM UI defaults, design tokens, component selection.
- `manage-app/cli-and-sync.md` — CLI command semantics, sync modes, build, deploy, logs, CI/CD; explicit ban on watch mode (`yarn searm dev` without `--once`) for agent use.
...
- `use-searm-mcp/result-formatting.md` — record link building, date formatting, readable Markdown contract.
