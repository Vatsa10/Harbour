...
<AccordionGroup>
<Accordion title="What roles come with SeaRM by default?">
SeaRM comes with **Admin** and **Member** roles by default. You can create additional custom roles based on your team's needs (e.g., Sales Rep, Manager, Read-Only User).
</Accordion>
...
You can control:
- **Send Email**: Ability to send emails from SeaRM
...
With JIT provisioning, user accounts are automatically created in SeaRM when someone logs in via SSO for the first time. They're assigned the default role automatically.
</Accordion>
