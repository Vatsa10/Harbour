---
title: Mailbox
description: Understanding email integration features in SeaRM.
---
...
SeaRM automatically links emails from your connected mailboxes to the relevant CRM records, keeping all communication history in one place.
...
### Sync Rate
SeaRM imports emails at approximately **400 messages per minute**. This rate is currently limited by Gmail API rate limiting (and applies similarly to other providers).
...
### Internal Emails
Choose if internal emails are synchronized in SeaRM:
...
- **Missing Emails**: Check if:
  ...
  - Contact auto-creation is disabled (emails need existing SeaRM records)
