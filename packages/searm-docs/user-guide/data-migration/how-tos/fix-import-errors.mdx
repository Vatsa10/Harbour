...
After uploading your file and mapping columns, SeaRM validates your data:

1. **Validation runs** — SeaRM checks each row for errors
...
|---------------|---------------|
| Duplicate values highlighted in yellow | Value already exists in SeaRM or appears twice in your file |
...
**Option 3: Let SeaRM update the existing record**
1. Ensure your file includes a unique identifier (`email`, `domain`, or `id`)
2. Map the unique identifier field
3. SeaRM will update the existing record instead of creating a duplicate
...
As long as one unique identifier matches, SeaRM updates the record.
</Note>
...
3. Check if records already exist in SeaRM
...
This means there is no Company in SeaRM with that specific identifier.
...
### What It Means
You're trying to link to a record that doesn't exist in SeaRM.
...
### What It Means
A column in your file can't be mapped because the field doesn't exist in SeaRM.
...
1. **User hasn't accepted their invitation** — the user doesn't exist in SeaRM yet
2. **Using user ID from old system** — SeaRM can't match IDs from another system
3. **Wrong email format** — the email doesn't match the user's SeaRM account

### How to Fix

1. Ensure all users have **accepted their invitation** to your SeaRM workspace
2. Use the user's **email address** (not their name or old system ID)
3. Use the same email they used to join SeaRM
...
<Check>Domain format matches SeaRM's format exactly</Check>
...
1. **Download the sample file** — see the exact format SeaRM expects
