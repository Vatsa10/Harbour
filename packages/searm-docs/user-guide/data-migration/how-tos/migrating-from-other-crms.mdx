...
description: Step-by-step guide to migrate your data from any CRM to SeaRM.
---

## Overview

This guide walks you through migrating your data from any CRM to SeaRM. The process involves auditing your data, preparing your SeaRM workspace, exporting from your current system, and importing into SeaRM.
...
Create a mapping document between your current CRM and SeaRM:
...
## Step 3: Set Up Your SeaRM Workspace

Before importing data, prepare your SeaRM workspace:
...
If your data includes user references (Account Owner, Assignee, etc.), those users must exist in SeaRM before import. Otherwise, those relations cannot be mapped.
</Warning>
...
Open each exported CSV in a spreadsheet application and prepare it for SeaRM.

### Remove Duplicates

1. Sort by the unique field (email for People, domain for Companies)
2. Remove or merge duplicate rows
3. Verify no duplicates exist in SeaRM already
...
Use `https://domain.com` (not `domain.com` or `www.domain.com`). This matches SeaRM's format and prevents duplicates when you connect email/calendar sync.
</Warning>
...
1. Navigate to the object in SeaRM
...
- SeaRM will start syncing emails to contact records
...
<AccordionGroup>
  <Accordion title="Can I migrate workflows automatically?">
    Not currently. Workflows must be recreated manually in SeaRM.
  </Accordion>
...
<Tip>
  Want help with your migration? [Find a certified SeaRM partner](https://searm.com/partners/list?categories=SOLUTIONING&ref=docs-migrate-crm) to move your data, pipelines, and users end-to-end.
</Tip>
