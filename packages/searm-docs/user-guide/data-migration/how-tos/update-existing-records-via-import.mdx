...
When you import a file containing a **unique identifier** that matches an existing record, SeaRM updates that record instead of creating a duplicate.

| If unique identifier... | SeaRM will... |
|-------------------------|----------------|
...
4. **Don't change unique identifier values** — or SeaRM will create new records
...
If you change `john@acme.com` to `john.smith@acme.com`, SeaRM will create a new record instead of updating the existing one.
</Warning>
...
SeaRM matches records by the unique identifier and updates them with new values.
...
3. SeaRM will match on that field
...
| **Case mismatch** | File has `John@acme.com` but SeaRM has `john@acme.com` | Creates new record | Export from SeaRM to get exact values |
...
<AccordionGroup>
  ...
  <Accordion title="How do I know which records will be updated vs. created?">
    During the import review step, SeaRM shows you how many records will be updated vs. created based on unique identifier matches.
  </Accordion>
