...
When you upload a file, SeaRM analyzes your columns and attempts to match them to existing fields.

### Automatic Mapping
SeaRM tries to match columns based on:
- Column header names (exact or similar matches)
- Data type detection (dates, numbers, emails)
- Common field patterns

<Note>**Quick tip:** Export a few rows from the object you want to import. The exported file will have the exact column names SeaRM expects, making automatic mapping seamless during import.</Note>

### Manual Mapping Options
For each column, you can:
- **Map to a field**: Select the matching SeaRM field from a dropdown
...
### ID Fields
Specifying an `id` during import is optional. SeaRM auto-generates one if not provided.
