...
a self-hostovaných instancích tuto cestu obsluhuje váš server SeaRM
(např. `http://localhost:2020/s/documents/generate`). Ve SeaRM Cloudu `RestApiClient`
...
[Viz pomocník] (https://github.com/Vatsa10/Harbour/blob/main/packages/searm-apps/examples/document-generator/src/utils/render-document.ts).
...
<Tip>
Během testování s
`yarn searm dev:function:logs`, nebo vyvolat přímo s
`yarn searm dev:exec`.
</Tip>
