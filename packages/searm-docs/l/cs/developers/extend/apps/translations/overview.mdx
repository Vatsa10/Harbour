...
Aplikace SeaRM jsou psané v **angličtině**: řetězce ve vašem zdrojovém kódu a manifestu tvoří anglický zdrojový text, `en` je zdrojová locale, ze které překládáte, a jakákoli locale ponechaná bez překladu se na ni automaticky vrátí.
...
Importujte pomocné překladové funkce z `searm-sdk/front-component`:
...
`searm dev:build` zkompiluje katalogy a doručí správný jazyk pro
...
znovu spustit `searm dev:build` (a znovu nasadit) stejně jako jakoukoli jinou změnu.

<Note>
Překlady jsou kompilovány příkazy `searm dev:build` (a `searm apply`). Nepřetržité sledování `searm dev` zobrazuje zdrojové řetězce, takže lokalizovaný výstup
