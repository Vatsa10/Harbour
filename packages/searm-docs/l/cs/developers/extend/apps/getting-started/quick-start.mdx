...
title: Rychlý start
icon: rocket
description: Vytvořte svou první aplikaci SeaRM během několika minut.
---
...
* **Docker** — [Stáhnout](https://www.docker.com/products/docker-desktop/). Nutné pro spuštění lokálního serveru SeaRM. Přeskočte, pokud už máte SeaRM spuštěné jinde.

Vytváření aplikace SeaRM má tři fáze. Generátor kostry je spojuje do jediného příkazu pro ideální scénář, ale každá fáze je samostatný koncept — když se něco nepovede, znalost aktuální fáze napoví, co opravit.

| Fáze                   | Co děláte                                                | Nástroj                             | Výsledek                                     |
| ---------------------- | -------------------------------------------------------- | ----------------------------------- | -------------------------------------------- |
| **1. Vytvořit kostru** | Vygenerovat zdrojový kód aplikace                        | `npx create-searm-app`             | Projekt v TypeScriptu na disku               |
| **2. Spustit server**  | Spustit server SeaRM, do kterého se bude synchronizovat | Docker + `yarn searm docker:start` | Běžící instance SeaRM                       |
| **3. Synchronizovat**  | Živě synchronizovat kód na server                        | `yarn searm dev`                   | Vaše změny se objeví v uživatelském rozhraní |
...
Generátor kostry je neinteraktivní: název adresáře se stane názvem aplikace. Předejte `--display-name` a `--description` pro přizpůsobení vygenerovaných metadat (později je můžete také upravit v `src/constants/universal-identifiers.ts`). Tím se v `my-searm-app/` vytvoří projekt v TypeScriptu se startovacím `application-config.ts`, výchozí rolí, pracovními postupy CI/CD a integračním testem.

**Po této fázi:** máte na svém počítači zdrojový kód aplikace. Zatím neběží — to je předmětem Fáze 2.

---

## Fáze 2 — Spusťte lokální server SeaRM

Vaše aplikace potřebuje server SeaRM, do kterého se bude synchronizovat. Server je plnohodnotná instance SeaRM — UI, GraphQL API, PostgreSQL — běžící lokálně v Dockeru. Váš lokální kód nahrává své definice na tento server, díky čemuž se objeví v UI.

Nástroj pro vytváření projektů to spustí za vás: při spuštěném Dockeru stáhne image `searmcrm/searm-app-dev`, spustí ji na portu `2020` a autentizuje CLI vůči předpřipravenému demo workspace (`tim@apple.dev`) — není vyžadováno žádné přihlášení.

Chcete-li se místo toho připojit k existujícímu serveru SeaRM, předejte `--url \<your-server-url>`. Vzdálené servery se ověřují pomocí OAuth: otevře se prohlížeč, abyste se mohli přihlásit a kliknout na **Authorize**, čímž udělíte nástroji CLI přístup k vašemu pracovnímu prostoru. (Můžete se také rozhodnout pro OAuth lokálně pomocí `--authentication-method oauth` — přihlaste se pomocí `tim@apple.dev` / `tim@apple.dev`.)

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="Přihlašovací obrazovka SeaRM" />
</div>

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="Autorizační obrazovka SeaRM CLI" />
</div>
...
**Po této fázi:** máte spuštěný server SeaRM na [http://localhost:2020](http://localhost:2020) a vaše CLI má oprávnění se k němu synchronizovat.

<Note>
Pokud Docker není nainstalovaný nebo neběží, generátor kostry vám sdělí správný příkaz pro spuštění ve vašem operačním systému. Jakmile Docker poběží, můžete pokračovat pomocí `yarn searm docker:start` — není potřeba znovu vytvářet kostru.
</Note>
...
<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/app-in-ui-1.png" alt="Seznam Your Apps se zobrazenou aplikací My searm app" />
</div>

Klikněte na **My searm app** a zobrazí se jeho **registrace aplikace** — záznam na úrovni serveru, který popisuje vaši aplikaci (název, identifikátor, přihlašovací údaje OAuth, zdroj). Jedna registrace může být nainstalována ve více pracovních prostorech na stejném serveru.
...
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| `yarn searm dev`   | Sleduje změny a při každé změně znovu synchronizuje. Běží, dokud jej nezastavíte.                                                                                      | Interaktivní lokální vývoj.                                              |
| `yarn searm apply` | Jedno sestavení + synchronizace, ukončí se s kódem `0` při úspěchu, `1` při chybě. Vyžádá si potvrzení destruktivních změn (předejte `--force`, abyste to přeskočili). | CI, pre-commit hooky, AI agenti, skriptované pracovní postupy.           |
| `yarn searm plan`  | Sestaví a vypíše změny metadat **bez jejich použití**.                                                                                                                 | Kontrola toho, co by synchronizace změnila, ještě před jejím potvrzením. |
...
<Note>
`yarn searm dev --once` a `yarn searm dev --once --dry-run` jsou zastaralé aliasy pro `yarn searm apply` a `yarn searm plan`.
</Note>
...
| **Frontendové komponenty** | Komponenty Reactu, které se vykreslují v uživatelském rozhraní SeaRM (postranní panel, widgety, příkazová nabídka) |
