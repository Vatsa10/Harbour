...
* Použijte `serverVariables` (nikoli `applicationVariables`) — pověření OAuth jsou celoserverová a na jeden server SeaRM je jedna aplikace OAuth.
...
4. Otevřete nainstalovanou aplikaci v SeaRM jako správce serveru → nastavte hodnoty na odpovídajících `serverVariables`.
