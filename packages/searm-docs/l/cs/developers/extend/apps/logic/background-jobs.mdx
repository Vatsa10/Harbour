...
description: Předejte dlouhotrvající nebo rychlostně omezenou práci SeaRM workerům tak, že místo provádění všeho přímo vkládáte do fronty další spuštění logické funkce.
icon: layer-group
---
...
`enqueueJob` dělá přesně to: požádá SeaRM workery, aby jednu z logických funkcí vaší aplikace spustili později, ve vlastním procesu a s vlastním časovým limitem. Volající se vrátí okamžitě.
...
Importujte `enqueueJob` z `searm-sdk/logic-function` a nasměrujte ho na `universalIdentifier` logické funkce, kterou chcete spustit.
