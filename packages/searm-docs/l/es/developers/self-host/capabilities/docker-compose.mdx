...
<Warning>
Los contenedores de Docker son para alojamiento en producción o autoalojamiento. Para contribuir, consulta la [Configuración local](/l/es/developers/contribute/capabilities/local-setup).
</Warning>

## Resumen

Esta guía proporciona instrucciones paso a paso para instalar y configurar la aplicación SeaRM utilizando Docker Compose. El objetivo es simplificar el proceso y prevenir errores comunes que podrían arruinar tu configuración.
...
Instala la última versión estable de SeaRM con un solo comando:

```bash
bash <(curl -sL https://raw.githubusercontent.com/Vatsa10/Harbour/main/packages/searm-docker/scripts/install.sh)
```

Para instalar una versión o rama específica:

```bash
VERSION=vx.y.z BRANCH=branch-name bash <(curl -sL https://raw.githubusercontent.com/Vatsa10/Harbour/main/packages/searm-docker/scripts/install.sh)
```
...
1. **Crea el archivo .env**
   ...
   ```bash
   curl -o .env https://raw.githubusercontent.com/Vatsa10/Harbour/refs/heads/main/packages/searm-docker/.env.example
   ```
...
3. **Actualiza el `.env`**
   ...
   Consulta la [guía de rotación de claves](/l/es/developers/self-host/capabilities/key-rotation) para obtener instrucciones sobre cómo rotarla sin tiempo de inactividad.
...
```bash
curl -o docker-compose.yml https://raw.githubusercontent.com/Vatsa10/Harbour/refs/heads/main/packages/searm-docker/docker-compose.yml
```
...
Si alojas searmCRM en tu propia computadora, abre tu navegador y navega a [http://localhost:3000](http://localhost:3000).
...
### Exponer SeaRM para acceso externo

Por defecto, SeaRM se ejecuta en `localhost` en el puerto `3000`. Para acceder a él mediante un dominio externo o dirección IP, necesitas configurar `SERVER_URL` en tu archivo `.env`.
...
Recomendamos encarecidamente configurar SeaRM detrás de un proxy inverso con terminación SSL para una seguridad y funcionalidad óptimas.
...
2. **Actualiza el archivo `.env`**
   ...
   * Acceso vía dominio con SSL:
     ```ini
     SERVER_URL=https://mysearmapp.com
     ```
...
```bash
docker exec searm-postgres pg_dump -U postgres searm > backup_$(date +%Y%m%d).sql
```
...
```bash
0 2 * * * docker exec searm-postgres pg_dump -U postgres searm > /backups/searm_$(date +\%Y\%m\%d).sql
```
...
```bash
docker compose stop searm-server searm-front
```

2. Restaura la base de datos:

```bash
docker exec -i searm-postgres psql -U postgres searm < backup_20240115.sql
```
...
<Tip>
  ¿Prefieres no ejecutar y mantener SeaRM por tu cuenta? [Encuentra un partner de alojamiento certificado de SeaRM](https://searm.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) para implementar, alojar y gestionar tu instancia de principio a fin. *(O contacta con nuestro equipo: [contact@searm.com](mailto:contact@searm.com).)*
</Tip>
