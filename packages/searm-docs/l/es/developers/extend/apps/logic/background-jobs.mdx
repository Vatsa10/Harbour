...
description: Puede delegar el trabajo prolongado o sujeto a limitaciones de tasa a los workers de SeaRM encolando otra ejecución de una función lógica en lugar de hacerlo todo en el mismo flujo.
icon: layer-group
---
...
`enqueueJob` hace exactamente eso: solicita a los workers de SeaRM que ejecuten más tarde una de las funciones lógicas de su aplicación, en su propio proceso, con su propio presupuesto de tiempo de espera. La llamada devuelve de inmediato.
...
Importa `enqueueJob` desde `searm-sdk/logic-function` y apúntalo al `universalIdentifier` de la función lógica que quieres ejecutar.
