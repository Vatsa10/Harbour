...
Las funciones lógicas son funciones de TypeScript del lado del servidor que se ejecutan en la plataforma SeaRM. Pueden activarse mediante solicitudes HTTP, programaciones de cron o eventos de base de datos — y también pueden exponerse como herramientas para agentes de IA.
...
* **httpRoute**: Expone tu función en una ruta y método HTTP. En el código de la aplicación, añade el prefijo `/s/` a la ruta cuando uses `RestApiClient`; la URL desplegada utiliza la base inyectada `SEARM_FUNCTIONS_URL` (o `\<server-url>/s` cuando no está configurada).
...
yarn searm dev:function:exec -n create-new-post-card -p '{"key": "value"}'
```
...
Importa el tipo `RoutePayload` desde `searm-sdk/logic-function`:
...
De forma predeterminada, devolver un valor sencillo desde tu controlador lo envía de vuelta como una respuesta `200` (JSON para objetos, `text/plain` para cadenas). Para controlar el código de estado y los encabezados de la respuesta, devuelve un `Response` desde `searm-sdk/logic-function`:
...
1. Una función de lógica de **resolver** — declarada con `serverRouteTriggerSettings` — se ejecuta en tu **espacio de trabajo propietario** (el espacio de trabajo que es propietario del registro de la aplicación). Inspecciona la solicitud entrante y devuelve una de las siguientes opciones:
   ...
   * un `Response` de `searm-sdk/logic-function` — la plataforma replica esa respuesta HTTP de forma **sincrónica** y **no** pone en cola un destino (usa esto para desafíos de verificación como `url_verification` de Slack).
...
import { defineLogicFunction } from 'searm-sdk/define';
import { Response, type RoutePayload } from 'searm-sdk/logic-function';
...
const handler = async (event: RoutePayload) => {
  ...
  const body = (event.body ?? {}) as {
    challenge?: string;
    metadata?: { searmWorkspaceId?: string };
    type?: string;
  };
  ...
  const workspaceId = body.metadata?.searmWorkspaceId;
...
```
POST https://your-searm-server.com/webhooks/server/:resolverLogicFunctionUniversalIdentifier
```
...
* **`toolTriggerSettings`** — hace que la función sea descubrible por las funciones de IA de SeaRM (chat, MCP, llamadas a funciones). Usa el JSON Schema estándar, el formato que los LLM entienden de forma nativa.
* **`workflowActionTriggerSettings`** — hace que la función aparezca como un paso en el constructor visual de flujos de trabajo. Usa el `InputSchema` completo de SeaRM para que el constructor pueda renderizar editores de campos adecuados, selectores de variables y etiquetas.
...
<Note>
**Relación con la acción Code del flujo de trabajo.** La acción integrada **Code** en el generador de flujos de trabajo es en sí misma una función lógica: SeaRM crea una por cada paso de Code y expone su editor en línea. `workflowActionTriggerSettings` es la forma de convertir ese código puntual en línea en una acción **reutilizable**: defines la función una vez en tu aplicación y se vuelve seleccionable en cualquier flujo de trabajo, en lugar de copiarla y pegarla en cada paso de Code. Consulta la [acción Code](/l/es/user-guide/workflows/capabilities/workflow-actions#code) en la guía del usuario para ver la perspectiva del usuario final.
</Note>
...
Para declarar tus parámetros **una sola vez** y atender ambas superficies, define un único JSON Schema (`InputJsonSchema`) y conviértelo para la acción de flujo de trabajo con `jsonSchemaToInputSchema` de `searm-sdk/logic-function`. `toolTriggerSettings.inputSchema` usa directamente el JSON Schema, mientras que `workflowActionTriggerSettings.inputSchema` espera el `InputSchema` de SeaRM:

```ts
import { defineLogicFunction } from 'searm-sdk/define';
import { jsonSchemaToInputSchema, type InputJsonSchema } from 'searm-sdk/logic-function';
...
| `inputSchema`  | El completo `InputSchema` de SeaRM: lo que el generador representa como campos configurables (con selectores de variables). Opcional; se infiere a partir del controlador cuando se omite. |
...
## Clientes de API tipados (searm-client-sdk)

El paquete `searm-client-sdk` proporciona dos clientes GraphQL tipados para interactuar con la API de SeaRM desde tus funciones de lógica y componentes de frontend.

| Cliente             | Importar                     | Endpoint                                                               | ¿Generado?                              |
| ------------------- | ---------------------------- | ---------------------------------------------------------------------- | --------------------------------------- |
| `CoreApiClient`     | `searm-client-sdk/core`     | `/graphql` — datos del espacio de trabajo (registros, objetos)         | Sí, en tiempo de desarrollo/compilación |
| `MetadataApiClient` | `searm-client-sdk/metadata` | `/metadata` — configuración del espacio de trabajo, cargas de archivos | No, viene preconstruido                 |

<AccordionGroup>
<Accordion title="CoreApiClient" description="Consultar y modificar datos del espacio de trabajo (registros, objetos)">

`CoreApiClient` es el cliente principal para consultar y mutar datos del espacio de trabajo. Se **genera a partir del esquema de tu espacio de trabajo** durante `yarn searm dev` o `yarn searm dev:build`, por lo que está completamente tipado para coincidir con tus objetos y campos.
...
<Note>
**CoreApiClient se genera en tiempo de desarrollo/compilación.** Si intentas usarlo sin ejecutar primero `yarn searm dev` o `yarn searm dev:build`, lanzará un error. La generación ocurre automáticamente: la CLI inspecciona el esquema GraphQL de tu espacio de trabajo y genera un cliente tipado usando `@genql/cli`.
</Note>
...
<Note>
  Cuando tu código se ejecuta en SeaRM (funciones de lógica o componentes de frontend), la plataforma inyecta credenciales como variables de entorno:

  * `SEARM_API_URL` — URL base de la API de SeaRM
  * `SEARM_APP_ACCESS_TOKEN` — Token de corta duración con alcance al rol de función predeterminado de tu aplicación
