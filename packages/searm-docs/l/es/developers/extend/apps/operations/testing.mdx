...
Ambos entornos tienen `searm-client-sdk/core` y `searm-client-sdk/metadata` disponibles como módulos preproporcionados — estos no se incluyen en el bundle sino que se resuelven en tiempo de ejecución por el servidor.
...
Crea un archivo de configuración global que verifique que el servidor es accesible, escriba una configuración de prueba para el SDK (`~/.searm/config.test.json`) y sincronice la aplicación antes de que se ejecuten las pruebas:
...
La subruta `searm-sdk/cli` exporta funciones que puedes invocar directamente desde el código de pruebas:

| Función        | Descripción                                                                |
| -------------- | -------------------------------------------------------------------------- |
| `appBuild`     | Compilar la aplicación y opcionalmente empaquetar un tarball               |
| `appDeploy`    | Subir un tarball al servidor                                               |
| `appDevOnce`   | Compila y sincroniza la aplicación una vez (igual que `yarn searm apply`) |
...
Asegúrate de que tu servidor local de SeaRM esté en ejecución y luego:
...
El generador crea un flujo de trabajo listo para usar en `.github/workflows/ci.yml`. En cada push a `main` y en cada pull request, inicia un servidor efímero de SeaRM en el runner (mediante la acción `Vatsa10/Harbour/.github/actions/spawn-searm-app-dev-test`), luego ejecuta `yarn lint`, `yarn typecheck`, `yarn test:unit` y `yarn test` con `SEARM_API_URL` / `SEARM_API_KEY` apuntando a ese servidor. No se requieren secretos y puedes fijar la versión del servidor mediante la variable de entorno `SEARM_VERSION` en la parte superior del flujo de trabajo.
