---
title: Conceptos
description: "Cómo funcionan las aplicaciones de SeaRM: modelo de entidad, sandboxing y ciclo de vida de la instalación."
icon: sitemap
---

Las aplicaciones de SeaRM son paquetes de TypeScript que amplían tu espacio de trabajo con objetos personalizados, lógica, componentes de UI y capacidades de IA. Se ejecutan en la plataforma SeaRM con sandboxing completo y controles de permisos.

## Cómo funcionan las aplicaciones

Una aplicación es un conjunto de **entidades** declaradas usando funciones `defineEntity()` del paquete `searm-sdk`. El SDK detecta estas declaraciones mediante análisis de AST en tiempo de compilación y produce un **manifiesto** — una descripción completa de lo que tu aplicación agrega a un espacio de trabajo. Estas funciones validan tu configuración en tiempo de compilación y proporcionan autocompletado en el IDE y seguridad de tipos.
...
| **Componente de frontend**          | Interfaz de usuario de React en entorno aislado dentro de SeaRM | [Componentes de frontend](/l/es/developers/extend/apps/layout/front-components)               |
...
* **Los componentes de frontend** se ejecutan en Web Workers usando Remote DOM — aislados de la página principal pero renderizando elementos DOM nativos (no iframes). Se comunican con SeaRM a través de una API de host de paso de mensajes.
* **Los permisos** se aplican a nivel de API. El token de tiempo de ejecución (`SEARM_APP_ACCESS_TOKEN`) se deriva del rol definido en `defineApplication()`.
...
┌─────────────────────────────────────────────────────────┐
│ Development                                             │
│   npx create-searm-app → yarn searm dev (live sync)   │
├─────────────────────────────────────────────────────────┤
│ Build & Deploy                                          │
│   yarn searm dev:build → yarn searm app:publish        │
├─────────────────────────────────────────────────────────┤
│ Install flow                                            │
...
├─────────────────────────────────────────────────────────┤
│ Publish                                                 │
│   npm publish → appears in SeaRM marketplace           │
└─────────────────────────────────────────────────────────┘
```

* **`yarn searm dev`** — observa tus archivos fuente y sincroniza en tiempo real los cambios con un servidor de SeaRM conectado. El cliente de API tipado se regenera automáticamente cuando cambia el esquema.
* **`yarn searm dev:build`** — compila TypeScript, agrupa las funciones de lógica y los componentes de frontend con esbuild, y produce un manifiesto.
