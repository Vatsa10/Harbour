...
description: Conoce las acciones disponibles en los flujos de trabajo de SeaRM.
---
...
<Note>
Solo puede enviar correos desde buzones sincronizados con su propia cuenta de SeaRM. El envío desde los buzones de otros miembros del equipo (p. ej., el correo del propietario de la cuenta) está en la hoja de ruta.
</Note>
...
Detrás de escena, cada acción de código está respaldada por su propia **función lógica** —una función TypeScript del lado del servidor que se ejecuta en la plataforma SeaRM—. Cuando agregas una acción de código, SeaRM crea una función lógica dedicada para ese paso y muestra su editor en línea, de modo que el código que escribes queda asociado al flujo de trabajo.
...
#### Reutilizar una función lógica en distintos flujos de trabajo

La acción de código en línea es ideal para lógica puntual, pero el código que contiene pertenece a ese único paso. Cuando quieras compartir la misma lógica en varios flujos de trabajo —o mantenerla como código fuente versionado en una app—, define una **función lógica reutilizable** en lugar de copiar y pegar código en cada acción de código.

Una función lógica se define una sola vez en una app (usando el `defineLogicFunction` del SDK) y se expone al generador de flujos de trabajo agregando `workflowActionTriggerSettings`. Una vez que tu app está instalada, esa función aparece como su propia acción en el generador de flujos de trabajo, junto con las acciones integradas. Al seleccionarla se muestran los campos de entrada que declaraste (con selectores de variables, como en las demás acciones) y se ejecuta tu código compartido; no se requiere JavaScript en línea.

|                          | Acción de código                                         | Función lógica reutilizable                                    |
| ------------------------ | -------------------------------------------------------- | -------------------------------------------------------------- |
| **Dónde vive el código** | Editor en línea, vinculado a un paso de flujo de trabajo | Definida en una app, versionada en el control de código fuente |
| **Reutilización**        | Copiada y pegada por paso                                | Agregada como una acción en cualquier flujo de trabajo         |
| **Entradas**             | Variables conectadas en el editor                        | Campos mostrados a partir de tu esquema de entrada declarado   |
| **Ideal para**           | Lógica rápida y específica de un flujo de trabajo        | Lógica compartida usada en varios flujos de trabajo            |

Ambas se ejecutan en el mismo entorno de ejecución de funciones lógicas, por lo que una acción de código que hayas prototipado en línea puede luego promocionarse a una función lógica reutilizable con cambios mínimos.

<Tip>
Para la referencia orientada a desarrolladores sobre cómo definir funciones lógicas y exponerlas en el generador de flujos de trabajo, consulta [Logic Functions](/l/es/developers/extend/apps/logic/logic-functions), en particular la sección **Exposing a function as an AI tool or workflow action** y `workflowActionTriggerSettings`.
</Tip>
...
<Note>
Los agentes de IA respetan los permisos basados en roles. Puede asignar roles específicos a los agentes en **Configuración → Miembros → Roles** para controlar a qué datos pueden acceder. Consulte [Permisos](/l/es/user-guide/permissions-access/capabilities/permissions) para obtener detalles.
</Note>
