---
title: 定位系统元数据
description: 解析 SeaRM 在每个对象上自动预配置的元数据的确定性通用标识符，这样你的应用无需硬编码就能引用它。
icon: gears
---

SeaRM 中的每个对象都带有你无需自行声明的**系统元数据**，例如一组字段以及带有列的主列表视图。 当对象被预配置时，服务器会创建所有这些内容，并且随着 SeaRM 的发展，这个集合也会随之增长。

由于你不会声明它，因此也就没有可供你导入的 `universalIdentifier` 常量。 相反，服务器会以确定性方式**派生**每个标识符，而 `searm-sdk` 暴露了相同的派生逻辑，这样你的 manifest 就能解析出服务器实际使用的精确值。
...
从 SeaRM 2.19 起，系统字段的通用标识符由服务器根据三个输入**确定性派生**：应用通用标识符、对象通用标识符以及字段名称。 自行构造一个 id 并将其硬编码是行不通的：它在服务器上不会匹配任何内容，同步时会拒绝这个悬空引用：
...
<Note>
  `getFieldUniversalIdentifier` 从 `searm-sdk` 2.21 版本开始可用。
</Note>
...
<Note>
  `getSystemRelationFieldUniversalIdentifier` 可在 `searm-sdk`
  2.23 及更高版本中使用，并且需要 SeaRM 服务器版本为 2.23 或更高。
</Note>
...
<Note>
  `getSystemViewUniversalIdentifier` 和 `getSystemViewFieldUniversalIdentifier` 可在 `searm-sdk` 2.26 及更高版本中使用，并且需要 SeaRM 服务器版本为 2.26 或更高。
</Note>
...
注意第一个参数：列是由**其显示的字段**所属的应用程序进行命名空间划分的，而不是由拥有该视图的应用程序进行划分。 你的应用向标准对象添加的字段，会在 SeaRM 拥有的视图上，以你的应用为命名空间来派生其列。
...
对于**标准** SeaRM 对象（Person、Company、Opportunity 等），你不需要进行任何派生：字段和视图的标识符都是预先计算好的常量，你可以直接导入。
