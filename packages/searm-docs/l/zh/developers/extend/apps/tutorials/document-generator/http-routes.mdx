...
两者都使用 `httpRouteTriggerSettings` 。 应用路由使用 `/s/` 路径；在本地和自托管实例上，该路径由你的 SeaRM 服务器提供服务（例如：`http://localhost:2020/s/documents/generate`）。 在 SeaRM Cloud 上，`RestApiClient` 通过工作区的 functions 域名来解析该路径。
...
[参见辅助函数](https://github.com/Vatsa10/Harbour/blob/main/packages/searm-apps/examples/document-generator/src/utils/render-document.ts)。
