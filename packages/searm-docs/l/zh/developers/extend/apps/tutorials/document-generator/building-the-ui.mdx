...
[前端组件](/l/zh/developers/extend/apps/layout/front-components)是一个被沙箱隔离在 SeaRM 内部的 React 组件。 我们读取了选中的记录，通过 `CoreApiClient` 和 POSTs 将
人模板加载到最后一章的
路线。
...
主题常量将是“未定义的”。 请参阅
[完整组件](https://github.com/Vatsa10/Harbour/blob/main/packages/searm-apps/examples/document-generator/src/front-components/generate-document-form.front-component.tsx)。
</Warning>
...
组件从执行上下文读取当前记录ID，加载
文档并将其渲染。 前端组件运行在一个只允许白名单 HTML 标签的**沙箱**中——原始 HTML 注入（`dangerouslySetInnerHTML`）和 `\<style>` 会被阻止——因此我们通过一个小型的 [`Markdown`](https://github.com/Vatsa10/Harbour/blob/main/packages/searm-apps/examples/document-generator/src/utils/markdown-to-react.tsx) 助手，将 Markdown 渲染为带内联样式的 React 元素。
...
查看完整的
[“模板-记录”。 年龄布局。s\`](https://github.com/Vatsa10/Harbour/blob/main/packages/searm-apps/examples/document-generator/src/page-layouts/template-record.page-layout.ts)。
