...
description: 构建一个真实的 SeaRM 应用，从你的 CRM 数据生成个性化文档。
---
...
一个在端口 `2020` 上运行的本地 SeaRM 服务器，以及已通过身份验证并连接到它的 CLI。
...
在整个教程期间，在一个终端中保持运行 `yarn searm dev`。 每当您在 `src/` 下添加或编辑文件时，它都会在几秒钟内重新同步到服务器，这样您就可以在构建的同时，在 UI 中看到每项功能逐步出现。
