...
SeaRM 应用均以**英文**编写：你的源码和 manifest 中的字符串
...
从 `searm-sdk/front-component` 中导入翻译辅助函数：
...
`searm dev:build` 会编译目录并为当前用户提供正确的语言：manifest 标签在服务端解析，前端组件目录被打包进每个组件 bundle 中。 在运行时，组件会从其执行上下文（宿主的当前语言）中读取语言环境，并将每个字符串在其目录中解析，当翻译缺失时回退到源文本。 在宿主中切换语言会实时重新渲染 `\<Trans>` 和 `useTranslate().t` 的字符串。

由于目录在构建时被编译，更新翻译意味着需要重新运行 `searm dev:build`（并重新部署），这和其他任何更改一样。

<Note>
翻译由 `searm dev:build`（以及 `searm apply`）编译。 持续运行的 `searm dev` 监视会显示源字符串，因此请使用一次性构建来测试本地化输出。
</Note>
