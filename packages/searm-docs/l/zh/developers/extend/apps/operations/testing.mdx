---
title: 测试
description: Vitest 设置、针对真实 SeaRM 服务器的集成测试、类型检查，以及使用 GitHub Actions 的 CI。
icon: flask
---

该 SDK 提供可编程的 API，使你可以在测试代码中构建、部署、安装和卸载你的应用。 结合 [Vitest](https://vitest.dev/) 和类型化 API 客户端，你可以编写集成测试，在真实的 SeaRM 服务器上验证你的应用端到端运行是否正常。
...
两个环境都将 `searm-client-sdk/core` 和 `searm-client-sdk/metadata` 作为预置模块提供 — 这些模块不会被打包，而是在运行时由服务器解析。
...
创建一个全局设置文件，用于验证服务器是否可访问，写入 SDK 的测试配置文件（`~/.searm/config.test.json`），并在测试运行前同步应用：
...
子路径 `searm-sdk/cli` 导出了可直接在测试代码中调用的函数：

| 函数             | 描述                                  |
| -------------- | ----------------------------------- |
| `appBuild`     | 构建应用，并可选地打包为 tar 包                  |
| `appDeploy`    | 将 tar 包上传到服务器                       |
| `appDevOnce`   | 构建并同步应用一次（与 `yarn searm apply` 相同） |
...
脚手架工具会在 `.github/workflows/ci.yml` 生成一个开箱即用的工作流。 在每次向 `main` 推送代码以及每个拉取请求上，它都会在 runner 中启动一个临时的 SeaRM 服务器（通过 `Vatsa10/Harbour/.github/actions/spawn-searm-app-dev-test` action），然后运行 `yarn lint`、`yarn typecheck`、`yarn test:unit` 和 `yarn test`，并将 `SEARM_API_URL` / `SEARM_API_KEY` 指向该服务器。 无需任何机密信息，你可以在工作流顶部通过 `SEARM_VERSION` 环境变量固定服务器版本。
