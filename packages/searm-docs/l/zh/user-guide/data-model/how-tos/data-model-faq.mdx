---
title: 数据模型常见问题解答
description: 关于 SeaRM 数据模型的常见问题。
---
...
您可以重命名标准对象的标签（People、Companies、Opportunities），但不能更改其 API 名称。 为了在所有 SeaRM 工作区中保持一致性，API 名称是固定的。
</Accordion>
...
公式字段在 **H2 2026** 中。 同时，您可以使用工作流程来自动计算和更新字段值。
</Accordion>

<Accordion title="我可以在我的对象中使用嵌套字段吗？">
嵌套字段将于 **H2 2026**。 目前，您可以使用工作流程从相关对象中获取字段值。 例如，要在人员记录上显示公司的行业信息，可以在人员自定义字段中创建一个自定义字段，并使用工作流程来同步该值。
</Accordion>

<Accordion title="我可以在对象中重新排序字段吗？">
是的，前往设置 -> 数据模型 → 您选择的对象-> 布局，并点击 **自定义记录页面**。 如果此按钮被禁用，您必须创建记录。
</Accordion>
</AccordionGroup>
...
多对多的关系出现在 **H2 2026**。 目前，可以创建一个中间对象，建立两个一对多关系作为替代方案。
