...
どちらも `httpRouteTriggerSettings` を使用します。 アプリのルートは `/s/` パスで指定されます。ローカル環境やセルフホスト環境では、そのパスは SeaRM サーバーによって提供されます（例: `http://localhost:2020/s/documents/generate`）。 SeaRM Cloud では、`RestApiClient` はワークスペースの functions ドメインを通じてパスを解決します。
...
[ヘルパーを参照](https://github.com/Vatsa10/Harbour/blob/main/packages/searm-apps/examples/document-generator/src/utils/render-document.ts)。
