...
* **ビルドに含まれる** — `yarn searm dev:build` はすべての公開アセットを配布物にバンドルします。

## `getPublicAssetUrl` で公開アセットにアクセスする

`searm-sdk` の `getPublicAssetUrl` ヘルパーを使用して、`public/` ディレクトリ内のファイルのフル URL を取得します。 これはロジック関数とフロントコンポーネントの両方で機能します。
