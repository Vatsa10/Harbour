...
Acest ghid oferă instrucțiuni pas cu pas pentru a instala și configura aplicația SeaRM folosind Docker Compose. Scopul este de a face procesul direct și de a preveni capcanele comune care ar putea deteriora configurarea dvs.
...
Instalați cea mai recentă versiune stabilă a SeaRM cu o singură comandă:
...
Dacă găzduiți searmCRM pe propriul computer, deschideți browserul și navigați la [http://localhost:3000](http://localhost:3000).
...
### Expuneți SeaRM pentru Acces Extern

Implicit, SeaRM rulează pe `localhost` la portul `3000`. Pentru a-l accesa printr-un domeniu extern sau adresă IP, trebuie să configurați `SERVER_URL` în fișierul dvs. `.env`.
...
SSL (HTTPS) este necesar pentru funcționarea corespunzătoare a anumitor funcții ale browserului. Deși aceste funcții pot funcționa în timpul dezvoltării locale (deoarece browserele tratează localhost diferit), o configurare SSL corespunzătoare este necesară când găzduiți SeaRM pe un domeniu obișnuit.
...
Recomandăm cu tărie configurarea SeaRM din spatele unui proxy revers cu terminare SSL pentru securitate și funcționalitate optimă.
...
<Tip>
  Preferiți să nu rulați și să nu întrețineți SeaRM chiar dumneavoastră? [Găsiți un partener SeaRM certificat pentru găzduire](https://searm.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) care să vă implementeze, găzduiască și gestioneze instanța de la un capăt la altul. *(Sau contactați echipa noastră: [contact@searm.com](mailto:contact@searm.com).)*
</Tip>
