...
Ambele medii au `searm-client-sdk/core` și `searm-client-sdk/metadata` disponibile ca module pre-furnizate — acestea nu sunt incluse în bundle, ci sunt rezolvate la rulare de către server.
...
Creează un fișier global de configurare inițială care verifică faptul că serverul este accesibil, scrie un fișier de configurare de test pentru SDK (`~/.searm/config.test.json`) și sincronizează aplicația înainte ca testele să ruleze:
...
Subcalea `searm-sdk/cli` exportă funcții pe care le puteți apela direct din codul de test:

| Funcție        | Descriere                                                                              |
| -------------- | -------------------------------------------------------------------------------------- |
| `appBuild`     | Construiți aplicația și, opțional, împachetați un tarball                              |
| `appDeploy`    | Încărcați un tarball pe server                                                         |
| `appDevOnce`   | Construiește și sincronizează aplicația o singură dată (la fel ca `yarn searm apply`) |
...
Asigurați-vă că serverul SeaRM local rulează, apoi:
...
Scaffolderul generează un workflow gata de utilizare la `.github/workflows/ci.yml`. La fiecare push pe `main` și la fiecare pull request, acesta pornește un server SeaRM efemer în runner (prin acțiunea `Vatsa10/Harbour/.github/actions/spawn-searm-app-dev-test`), apoi rulează `yarn lint`, `yarn typecheck`, `yarn test:unit` și `yarn test` cu `SEARM_API_URL` / `SEARM_API_KEY` îndreptate către acel server. Nu sunt necesare secrete și poți fixa versiunea serverului prin variabila de mediu `SEARM_VERSION` din partea de sus a workflow-ului.
