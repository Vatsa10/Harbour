...
icon: laptop-code
description: Ghidul pentru contribuitori (sau dezvoltatori curioși) care doresc să ruleze SeaRM local.
---
...
Înainte de a instala și utiliza SeaRM, asigurați-vă că instalați următoarele pe computerul dvs.:
...
SeaRM necesită un cache Redis pentru a oferi cea mai bună performanță.
...
## Pasul 8: Utilizați SeaRM

**Front-end**

Frontend-ul SeaRM va rula la [http://localhost:3001](http://localhost:3001).
Vă puteți loga folosind contul demo implicit: `tim@apple.dev` (parolă: `tim@apple.dev`)

**Back-end**

* Serverul SeaRM va fi operativ la [http://localhost:3000](http://localhost:3000)
