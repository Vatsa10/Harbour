---
title: Conceitos
description: Como as aplicações SeaRM funcionam — modelo de entidade, sandboxing e ciclo de vida da instalação.
icon: sitemap
---

As aplicações SeaRM são pacotes TypeScript que estendem seu espaço de trabalho com objetos personalizados, lógica, componentes de UI e recursos de IA. Elas são executadas na plataforma SeaRM com sandboxing completo e controles de permissão.

## Como as aplicações funcionam

Uma aplicação é uma coleção de **entidades** declaradas usando funções `defineEntity()` do pacote `searm-sdk`. O SDK detecta essas declarações via análise de AST no momento da compilação e produz um **manifesto** — uma descrição completa do que seu aplicativo adiciona a um espaço de trabalho. Essas funções validam sua configuração em tempo de compilação e oferecem autocompletar na IDE e segurança de tipos.
...
| **Componente de front-end**   | UI React em sandbox dentro do SeaRM                | [Componentes de front-end](/l/pt/developers/extend/apps/layout/front-components)        |
...
* **Componentes de front-end** executam em Web Workers usando Remote DOM — isolados da página principal, mas renderizando elementos DOM nativos (não iframes). Eles se comunicam com o SeaRM por meio de uma API de host com passagem de mensagens.
* **Permissões** são aplicadas no nível da API. O token de tempo de execução (`SEARM_APP_ACCESS_TOKEN`) é derivado do papel definido em `defineApplication()`.
...
* **`yarn searm dev`** — observa seus arquivos-fonte e sincroniza ao vivo as alterações com um servidor SeaRM conectado. O cliente de API tipado é regenerado automaticamente quando o esquema muda.
* **`yarn searm dev:build`** — compila TypeScript, empacota funções de lógica e componentes de front-end com o esbuild e produz um manifesto.
