...
description: Guia completo passo a passo para importar empresas para o SeaRM.
---

## Visão Geral

Este guia explica como importar as suas empresas para o SeaRM. **As empresas devem ser importadas primeiro** porque Pessoas e Oportunidades se ligam a Empresas.
...
<Note>
**Dica profissional:** Clique em **Download sample file** durante a importação para ver os nomes de colunas exatos que o SeaRM espera.
</Note>
...
1. O SeaRM mostra os seus valores ao lado das opções existentes
2. Relacione cada valor no seu ficheiro a uma opção do SeaRM
...
Antes de concluir a importação, o SeaRM valida os seus dados:
...
| Erro                            | Causa                          | Solução                                            |
| ------------------------------- | ------------------------------ | -------------------------------------------------- |
| **Domínio duplicado**           | O domínio já existe no SeaRM  | Remova do ficheiro ou atualize o registo existente |
...
2. O SeaRM faz a correspondência dos registos por este identificador único
...
<AccordionGroup>
  <Accordion title="Porque é que duas Empresas não podem ter o mesmo domínio?">
    O domínio é um identificador único no SeaRM. Isto evita empresas duplicadas e garante que a sincronização de e-mail associe corretamente os e-mails à empresa certa.
  </Accordion>
  ...
  <Accordion title="O que acontece se importar um domínio que já existe?">
    Se incluir um identificador único (domínio ou id) que corresponda a uma empresa existente, a SeaRM atualiza essa empresa em vez de criar um duplicado.
  </Accordion>
