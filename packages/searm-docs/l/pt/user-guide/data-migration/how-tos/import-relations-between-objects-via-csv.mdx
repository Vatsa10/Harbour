...
O pai (lado "um") deve existir no SeaRM antes de poder referenciá-lo.
...
1. Exporte os registos pai do SeaRM
...
Use `https://domain.com` (não apenas `domain.com`). Isto corresponde à forma como a SeaRM armazena domínios de Empresas e evita erros de correspondência.
</Warning>
...
A SeaRM ligará automaticamente cada registo filho ao pai correspondente.
...
| **Registos inexistentes**                      | O registo pai não existe no SeaRM               | Importe primeiro os registos pai ou verifique se há erros ortográficos |
| **Incompatibilidade de maiúsculas/minúsculas** | `Acme.com` no ficheiro mas `acme.com` no SeaRM  | Garanta correspondência exata de maiúsculas/minúsculas                 |
...
<AccordionGroup>
  <Accordion title="E se o meu registo pai não tiver um domínio ou email?">
    Tem duas opções:
    1. Use o `id` da SeaRM (exporte os registos pai para obter os respetivos IDs)
