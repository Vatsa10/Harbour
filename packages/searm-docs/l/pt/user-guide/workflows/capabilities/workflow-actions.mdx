...
description: Saiba mais sobre as ações disponíveis nos fluxos de trabalho do SeaRM.
---
...
<Note>
Você só pode enviar e-mails de caixas de correio sincronizadas com a sua própria conta SeaRM. O envio a partir das caixas de correio de outros membros da equipe (por exemplo, o e-mail do proprietário da conta) está no roadmap.
</Note>
...
Nos bastidores, cada ação de código é respaldada por sua própria **função lógica** — uma função TypeScript do lado do servidor que é executada na plataforma SeaRM. Quando você adiciona uma ação de código, o SeaRM cria uma função lógica dedicada para essa etapa e exibe seu editor em linha, de modo que o código que você escreve fica junto com o fluxo de trabalho.
...
<Tip>
Para a documentação de referência voltada para desenvolvedores sobre como definir funções lógicas e expô-las no construtor de fluxos de trabalho, consulte [Logic Functions](/l/pt/developers/extend/apps/logic/logic-functions) — em particular a seção **Exposing a function as an AI tool or workflow action** e `workflowActionTriggerSettings`.
</Tip>
