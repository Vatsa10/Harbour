...
<AccordionGroup>
<Accordion title="Quais funções o SeaRM traz por padrão?">
...
Você pode controlar:
* **Enviar e-mail**: Capacidade de enviar e-mails a partir do SeaRM
...
Com o provisionamento JIT, as contas de usuário são criadas automaticamente no SeaRM quando alguém faz login via SSO pela primeira vez. A função padrão é atribuída automaticamente.
</Accordion>
