...
description: Risolvi gli identificatori universali deterministici dei metadati che SeaRM predispone automaticamente su ogni oggetto, così la tua app può farvi riferimento senza doverli codificare in modo statico.
icon: gears
---

Ogni oggetto in SeaRM include **metadati di sistema** che non dichiari mai direttamente, come ad esempio un insieme di campi e una vista elenco principale con le sue colonne. Il server crea tutto questo quando l'oggetto viene predisposto e l'insieme cresce man mano che SeaRM cresce.

Poiché non lo dichiari, non esiste alcuna costante `universalIdentifier` da importare. Invece, il server **deriva** ogni identificatore in modo deterministico e `searm-sdk` espone la stessa derivazione affinché il tuo manifest possa risolvere il valore esatto utilizzato dal server.
...
A partire da SeaRM 2.19, l'identificatore universale di un campo di sistema viene derivato in modo deterministico dal server sulla base di tre input: l'identificatore universale dell'applicazione, l'identificatore universale dell'oggetto e il nome del campo. Inventare un id e hardcodarlo non funziona: non corrisponde a nulla sul server e la sincronizzazione rifiuta il riferimento orfano:
...
<Note>
  `getFieldUniversalIdentifier` è disponibile da `searm-sdk` 2.21 in poi.
</Note>
...
<Note>
  `getSystemRelationFieldUniversalIdentifier` è disponibile da `searm-sdk`
  2.23 in poi e richiede un server SeaRM dalla versione 2.23 o successiva.
</Note>
...
<Note>
  ...
  sono disponibili da `searm-sdk` 2.26 in poi e richiedono un server SeaRM
...
Nota il primo argomento: una colonna appartiene allo spazio dei nomi dell’applicazione proprietaria del **campo che visualizza**, non di quella proprietaria della vista. Un campo che la tua app aggiunge a un oggetto standard ottiene la propria colonna derivata sotto la tua applicazione, su una vista di proprietà di SeaRM.
...
## Oggetti standard di SeaRM

Per un oggetto **standard** di SeaRM (Person, Company, Opportunity, …), non hai bisogno di derivare nulla: gli identificatori sono costanti pre-calcolate che puoi importare direttamente, sia per i campi che per le viste.
