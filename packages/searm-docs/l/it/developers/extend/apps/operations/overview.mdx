...
Il **livello operativo** è tutto ciò che fai *alla* tua app piuttosto che *con* essa: eseguire comandi CLI, eseguire test di integrazione contro un server SeaRM reale, configurare la CI e distribuire le release — sia come tarball distribuito su un singolo server, sia come pacchetto npm elencato nel marketplace.

```text
   develop ─▶ test ─▶ build ─▶ deploy / publish
   ───────    ────    ─────    ─────────────────
   yarn       yarn    yarn     yarn searm app:publish --private  (tarball → one server)
   searm     test    searm
   dev                dev:build    yarn searm app:publish         (npm → marketplace)
```
...
<CardGroup cols={2}>
  <Card title="CLI" icon="terminal" href="/l/it/developers/extend/apps/operations/cli">
    Riferimento a `yarn searm` — exec, logs, uninstall, remotes.
  </Card>
