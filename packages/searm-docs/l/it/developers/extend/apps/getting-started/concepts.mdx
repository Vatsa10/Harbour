---
title: Concetti
description: Come funzionano le app SeaRM — modello di entità, sandboxing e ciclo di vita dell’installazione.
icon: sitemap
---

Le app di SeaRM sono pacchetti TypeScript che estendono il tuo spazio di lavoro con oggetti personalizzati, logica, componenti dell'interfaccia utente (UI) e funzionalità di IA. Vengono eseguite sulla piattaforma SeaRM con sandboxing completo e controlli delle autorizzazioni.

## Come funzionano le app

Un'app è una raccolta di **entità** dichiarate utilizzando le funzioni `defineEntity()` del pacchetto `searm-sdk`. L'SDK rileva queste dichiarazioni tramite analisi dell'AST in fase di build e produce un **manifest** — una descrizione completa di ciò che la tua app aggiunge a uno spazio di lavoro. Queste funzioni convalidano la configurazione in fase di build e offrono il completamento automatico nell'IDE e la sicurezza dei tipi.
...
| **Componente front-end**         | UI React in sandbox all’interno di SeaRM              | [Componenti front-end](/l/it/developers/extend/apps/layout/front-components)              |
...
* **I componenti front-end** vengono eseguiti in Web Workers utilizzando il Remote DOM — isolati dalla pagina principale ma renderizzando elementi DOM nativi (non iframe). Comunicano con SeaRM tramite un'API host basata sul passaggio di messaggi.
* **Le autorizzazioni** vengono applicate a livello di API. Il token di runtime (`SEARM_APP_ACCESS_TOKEN`) è derivato dal ruolo definito in `defineApplication()`.
...
* **`yarn searm dev`** — monitora i file sorgente e sincronizza in tempo reale le modifiche su un server SeaRM connesso. Il client API tipizzato viene rigenerato automaticamente quando lo schema cambia.
* **`yarn searm dev:build`** — compila TypeScript, crea i bundle delle funzioni logiche e dei componenti front-end con esbuild e produce un manifest.
