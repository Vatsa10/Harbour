...
description: Affida il lavoro lungo o soggetto a limitazioni di frequenza ai worker di SeaRM accodando un'altra esecuzione di una logic function invece di fare tutto in linea.
icon: layer-group
---
...
`enqueueJob` fa esattamente questo: chiede ai worker di SeaRM di eseguire in un secondo momento una delle logic function della tua app, in un proprio processo, con il proprio budget di timeout. Il chiamante restituisce immediatamente.
...
Importa `enqueueJob` da `searm-sdk/logic-function` e puntalo all'`universalIdentifier` della logic function che vuoi eseguire.
...
| ------------ | ----------- | -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `retryLimit` | `0`         | `0`–`10`                   | Tentativi extra se l'esecuzione genera un'eccezione. Aumenta questo valore solo per gli handler che possono essere eseguiti due volte in sicurezza. |
...
import { defineLogicFunction } from 'searm-sdk/define';
import { enqueueJob, kv } from 'searm-sdk/logic-function';
