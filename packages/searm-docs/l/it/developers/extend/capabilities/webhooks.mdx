...
I webhook inviano dati ai tuoi sistemi in tempo reale quando si verificano eventi in SeaRM — senza necessità di polling. Usali per mantenere sincronizzati i sistemi esterni, attivare automazioni o inviare avvisi.
...
SeaRM invia webhook per questi tipi di eventi:
...
SeaRM firma ogni richiesta webhook per motivi di sicurezza. Convalida le firme per assicurarti che le richieste siano autentiche.
...
| `X-SeaRM-Webhook-Timestamp` | Timestamp della richiesta |

### Passaggi di convalida

1. Ottieni il timestamp da `X-SeaRM-Webhook-Timestamp`
2. Crea la stringa: `{timestamp}:{JSON payload}`
3. Calcola HMAC SHA256 usando il segreto del tuo webhook
4. Confronta con `X-SeaRM-Webhook-Signature`
...
| **Trigger webhook del Workflow** | IN        | Ricevi dati in SeaRM da sistemi esterni                                 |
