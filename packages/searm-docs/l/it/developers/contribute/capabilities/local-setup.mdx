...
icon: laptop-code
description: La guida per i collaboratori (o sviluppatori curiosi) che vogliono eseguire SeaRM localmente.
---
...
Prima di poter installare e usare SeaRM, assicurati di installare quanto segue sul tuo computer:
...
SeaRM richiede una cache Redis per offrire le migliori prestazioni.
...
## Passo 8: Utilizza SeaRM

**Frontend**

Il frontend di SeaRM sarà in esecuzione su [http://localhost:3001](http://localhost:3001).
Puoi accedere utilizzando l'account demo predefinito: `tim@apple.dev` (password: `tim@apple.dev`)

**Backend**

* Il server di SeaRM sarà attivo e funzionante su [http://localhost:3000](http://localhost:3000)
