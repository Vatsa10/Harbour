...
description: Guida passo-passo per migrare i tuoi dati da qualsiasi CRM a SeaRM.
---

## Panoramica

Questa guida ti accompagna nella migrazione dei tuoi dati da qualsiasi CRM a SeaRM. Il processo prevede la revisione dei tuoi dati, la preparazione del tuo spazio di lavoro SeaRM, l'esportazione dal sistema attuale e l'importazione in SeaRM.
...
Crea un documento di mappatura tra il tuo CRM attuale e SeaRM:
...
* Il campo di destinazione di SeaRM
...
## Passaggio 3: Configura il tuo spazio di lavoro SeaRM

Prima di importare i dati, prepara il tuo spazio di lavoro SeaRM:
...
Se i tuoi dati includono riferimenti a utenti (Proprietario dell'account, Assegnatario, ecc.), tali utenti devono esistere in SeaRM prima dell'importazione. In caso contrario, tali relazioni non possono essere mappate.
</Warning>
...
Apri ogni file CSV esportato in un'applicazione di fogli di calcolo e preparalo per SeaRM.

### Rimuovi i duplicati

1. Ordina per il campo univoco (email per Persone, dominio per Aziende)
2. Rimuovi o unisci le righe duplicate
3. Verifica che non esistano già duplicati in SeaRM
...
Usa `https://domain.com` (non `domain.com` o `www.domain.com`). Questo corrisponde al formato di SeaRM ed evita duplicati quando colleghi la sincronizzazione di email e calendario.
</Warning>
...
2. Usa gli stessi indirizzi email che gli utenti hanno utilizzato per unirsi al tuo spazio di lavoro SeaRM
...
* SeaRM inizierà a sincronizzare le email con i record dei contatti
...
<AccordionGroup>
  <Accordion title="Posso migrare i flussi di lavoro automaticamente?">
    Al momento no. I flussi di lavoro devono essere ricreati manualmente in SeaRM.
  </Accordion>
...
<Tip>
  Vuoi aiuto con la tua migrazione? [Trova un partner SeaRM certificato](https://searm.com/partners/list?categories=SOLUTIONING\&ref=docs-migrate-crm) per spostare i dati, le pipeline e gli utenti end-to-end.
</Tip>
