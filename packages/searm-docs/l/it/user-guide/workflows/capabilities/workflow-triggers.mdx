...
* **Non è consigliato per i record creati manualmente**: con questo attivatore, i flussi di lavoro iniziano non appena il record viene creato. Dal momento che SeaRM UI offre il salvataggio automatico sul volo (non c'è alcuna modalità di modifica e quindi una convalida per salvare i record), il flusso di lavoro verrà attivato prima che l'utente inserisca tutti i campi.
...
* Per le richieste POST, definisci la struttura del body prevista in modo che SeaRM sappia quali dati aspettarsi. Aggiungi qui i campi che riceverai e che saranno necessari più avanti nel tuo flusso di lavoro.
