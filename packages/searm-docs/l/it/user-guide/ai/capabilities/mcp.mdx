---
title: Server MCP
description: Collega gli assistenti AI al tuo spazio di lavoro di SeaRM utilizzando il Model Context Protocol.
---

SeaRM espone un server [MCP](https://modelcontextprotocol.io/) affinché gli assistenti AI — Claude Desktop, Claude Code, Cursor, ChatGPT e altri — possano leggere e scrivere i dati del tuo CRM in linguaggio naturale.

Usa l'**URL dello spazio di lavoro** (l'URL che usi per accedere a SeaRM) come endpoint MCP. Su SeaRM Cloud, l'URL del tuo spazio di lavoro potrebbe essere `https://{mycompany}.searm.com` oppure un dominio personalizzato. Il server è disponibile all'indirizzo:

| Ambiente          | Endpoint MCP                                                                   |
| ----------------- | ------------------------------------------------------------------------------ |
| **Cloud**         | `https://{your-workspace-url}/mcp` (ad es. `https://mycompany.searm.com/mcp`) |
...
Aggiungi questo alla configurazione del tuo client MCP, sostituendo `{your-workspace-url}` con l'host del tuo spazio di lavoro (ad es. `mycompany.searm.com`):
...
1. Scoprire i metadati OAuth di SeaRM tramite `/.well-known/oauth-protected-resource` e `/.well-known/oauth-authorization-server`
...
Vai su **Impostazioni → MCP e API → MCP** in SeaRM. Scegli il metodo di autenticazione (OAuth o Chiave API), copia lo snippet JSON (userà già l'URL del tuo spazio di lavoro) e incollalo nel file di configurazione del tuo client MCP.
...
Riavvia il tuo client MCP (o ricarica la configurazione). Se usi OAuth verrai reindirizzato a SeaRM per autorizzare l'accesso. Se usi una chiave API la connessione è immediata.
...
Una volta connesso, il server MCP espone strumenti che rispecchiano l'API di SeaRM. Il flusso di lavoro consigliato è:
...
Per le istanze self-hosted, sostituisci `{your-workspace-url}` con l'URL del tuo server. Assicurati che `SERVER_URL` nel tuo ambiente corrisponda all'URL pubblico della tua istanza SeaRM — viene utilizzato per generare i metadati di discovery OAuth.
