...
* **Utilisés pour les métadonnées de marketplace** — les champs `logo` et `galleryImages` dans `defineApplication()` référencent des fichiers de ce dossier (p. ex., `public/logo.png`). Celles-ci sont affichées dans la marketplace lorsque votre application est publiée. Les URL absolues externes sont ignorées pour ces champs — intégrez plutôt les images dans `public/`. Chaque image ne doit pas dépasser 10 Mo.
...
* **Inclus dans les builds** — `yarn searm dev:build` regroupe toutes les ressources publiques dans la sortie de distribution.

## Accéder aux ressources publiques avec `getPublicAssetUrl`

Utilisez l'utilitaire `getPublicAssetUrl` de `searm-sdk` pour obtenir l'URL complète d'un fichier dans votre répertoire `public/`. Cela fonctionne à la fois dans les **fonctions logiques** et les **composants frontaux**.
