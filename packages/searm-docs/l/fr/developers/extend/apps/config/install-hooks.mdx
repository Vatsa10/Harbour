...
* L'environnement habituel des fonctions logiques (`APPLICATION_ID`, `APP_ACCESS_TOKEN`, `API_URL`) est injecté, ce qui vous permet d'appeler l'API SeaRM avec le jeton de votre application.
...
* **Non exécuté en mode dev** : `yarn searm dev` ignore le flux d'installation et synchronise directement les fichiers, donc les hooks ne s'exécutent jamais dans ce cas. Déclenchez-les manuellement à la place :
...
import { defineUninstallLogicFunction, type UninstallPayload } from 'searm-sdk/define';
import { CoreApiClient } from 'searm-client-sdk/core';
