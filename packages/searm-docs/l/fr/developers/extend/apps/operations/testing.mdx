...
**Les composants frontaux** s'exécutent dans un Web Worker. Les modules intégrés de Node ne sont **pas** disponibles — seuls les packages npm qui fonctionnent dans un environnement navigateur sont pris en charge. Notez que le bac à sable implémente un DOM *partiel*, de sorte qu’un package peut être compilé sans erreur tout en échouant à l’exécution ; voir [Limitations actuelles](/l/fr/developers/extend/apps/layout/front-components#current-limitations).

Les deux environnements disposent de `searm-client-sdk/core` et `searm-client-sdk/metadata` en tant que modules pré-fournis — ils ne sont pas intégrés au bundle mais résolus à l'exécution par le serveur.
...
Créez un fichier de configuration global qui vérifie que le serveur est joignable, écrit une configuration de test pour le SDK (`~/.searm/config.test.json`) et synchronise l’application avant l’exécution des tests :
...
Le sous-chemin `searm-sdk/cli` exporte des fonctions que vous pouvez appeler directement depuis le code de test :
...
| `appDevOnce`   | Construire et synchroniser l’application une fois (identique à `yarn searm apply`) |
...
Assurez-vous que votre serveur SeaRM local est en cours d'exécution, puis :
...
Le générateur crée un workflow prêt à l’emploi dans `.github/workflows/ci.yml`. À chaque push sur `main` et à chaque pull request, il lance un serveur SeaRM éphémère dans le runner (via l’action `Vatsa10/Harbour/.github/actions/spawn-searm-app-dev-test`), puis exécute `yarn lint`, `yarn typecheck`, `yarn test:unit` et `yarn test` avec `SEARM_API_URL` / `SEARM_API_KEY` pointant vers ce serveur. Aucun secret n’est requis, et vous pouvez fixer la version du serveur via la variable d’environnement `SEARM_VERSION` en haut du workflow.
