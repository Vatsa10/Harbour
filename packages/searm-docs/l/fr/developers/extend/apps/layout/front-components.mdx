...
description: Créez des composants React qui s’affichent dans l’interface utilisateur (UI) de SeaRM avec une isolation en bac à sable.
icon: window-maximize
---

Les composants frontaux sont des composants React qui s'affichent directement dans l'interface utilisateur de SeaRM. Ils s'exécutent dans un **Web Worker isolé** en utilisant Remote DOM — votre code s'exécute dans un iframe à origine opaque et sandboxé, mais son interface utilisateur continue de s'afficher nativement dans la page plutôt que d'être confinée à cet iframe.
...
Les composants frontaux peuvent s'afficher à trois emplacements au sein de SeaRM :
...
Après la synchronisation avec `yarn searm dev` (ou en exécutant une seule fois `yarn searm apply`), l'action rapide apparaît dans le coin supérieur droit de la page :
...
Le composant est affiché comme une section **à l’intérieur** de l’onglet Settings, et non comme un remplacement de l’onglet entier. Les sections gérées par le système de SeaRM — mise à niveau automatique, App URL et connexions — sont toujours affichées au-dessus et ne peuvent pas être remplacées par l’application.
...
Comme le composant retourne `null`, SeaRM n'affiche pas de conteneur pour celui-ci — aucun espace vide n'apparaît dans la mise en page. Le composant a toujours accès à tous les hooks et à l'API de communication de l'hôte.

## Composants Command du SDK

Le package `searm-sdk` fournit quatre composants utilitaires Command conçus pour les composants frontaux headless. Chaque composant exécute une action au montage, gère les erreurs en affichant une notification snackbar et démonte automatiquement le composant frontal une fois terminé.

Importez-les depuis `searm-sdk/front-component` :
...
Une fonction logique déclarée avec `httpRouteTriggerSettings` est accessible via HTTP à son chemin de route. `RestApiClient` traite les chemins commençant par `/s/` comme des routes d’application, les résout vers l’URL à partir de laquelle vos fonctions sont servies et les authentifie avec `SEARM_APP_ACCESS_TOKEN`.

> **Sur SeaRM Cloud, les fonctions logiques déclenchées par HTTP sont servies sur un domaine dédié par espace de travail** à l’adresse `https://\<your-workspace-subdomain>.withsearm.com\<path>`. Pour les appelants externes, copiez l’URL exacte à partir des paramètres **HTTP trigger** de la fonction ou de l’onglet **Settings** de l’application.
...
Le chemin transmis à `RestApiClient` est la propriété `httpRouteTriggerSettings.path` de la fonction logique, préfixée par `/s`. Conservez `isAuthRequired: true` ; le `SEARM_APP_ACCESS_TOKEN` que SeaRM génère pour votre composant authentifie la requête :
...
<Note>
`SEARM_APP_ACCESS_TOKEN` est injecté automatiquement — voir [Variables d’application](#application-variables). Comme les variables d’application secrètes ne sont jamais exposées aux composants front, conservez les clés d’API et les autres éléments sensibles dans la fonction logique, et non dans le composant front.
</Note>

### Appeler l’API REST de SeaRM

Pour appeler des routes HTTP d’application ou lire et écrire des enregistrements SeaRM depuis un composant frontal, utilisez `RestApiClient` depuis `searm-client-sdk/rest`. Il envoie les chemins `/s/...` vers l’URL de base des fonctions de votre espace de travail et tous les autres chemins, y compris `/rest/...`, vers `SEARM_API_URL`.
...
| ------------------------- | --------------------------------------------------------- |
| `SEARM_API_URL`          | URL de base de l’API principale de SeaRM                 |
| `SEARM_APP_ACCESS_TOKEN` | Jeton de courte durée limité au rôle de votre application |

### `SEARM_FUNCTIONS_URL`

SeaRM injecte également `SEARM_FUNCTIONS_URL` dans les composants frontaux et les fonctions logiques : l’URL de base à partir de laquelle les fonctions logiques déclenchées par HTTP de votre application sont servies.

Elle existe parce que cette URL n’est pas toujours le serveur SeaRM lui-même. Sur SeaRM Cloud, les routes d’application sont servies sur un domaine dédié par espace de travail (`https://\<your-workspace-subdomain>.withsearm.com`, ou le domaine public principal de l’application lorsqu’il est configuré) afin que les réponses créées par l’application s’exécutent sur une origine isolée plutôt que sur l’origine de l’application SeaRM. Les instances auto-hébergées et locales servent les routes d’application sous le préfixe `/s` sur le serveur lui-même et peuvent ne pas définir la variable du tout. Comme l’URL de base varie selon l’espace de travail et l’instance, votre code ne peut pas la coder en dur — le serveur injecte la bonne valeur à l’exécution.

Vous avez rarement besoin de la lire directement. Appelez vos routes via `RestApiClient` avec un chemin préfixé par `/s/` et le client résout l’URL pour vous : il retire le préfixe `/s` et cible `SEARM_FUNCTIONS_URL`, en revenant à `\<SEARM_API_URL>/s` lorsque la variable n’est pas définie. Utilisez `resolveUrl('/s/\<path>')` pour obtenir l’URL absolue sans envoyer de requête, par exemple pour un lien. Lisez la variable directement uniquement lorsque vous construisez une URL manuellement :
...
Les composants frontaux peuvent déclencher la navigation, des modales et des notifications en utilisant des fonctions de `searm-sdk` :
...
* **Composants d’interface utilisateur SeaRM** — la bibliothèque de composants propre à SeaRM ; voir [Utilisation des composants d’interface utilisateur SeaRM](#using-searm-ui-components) ci-dessous
...
## Utilisation des composants d’interface utilisateur SeaRM

SeaRM fournit sa bibliothèque de composants sous forme de package [`searm-ui`](https://www.npmjs.com/package/searm-ui/v/1.0.0-alpha.1). Les composants frontaux peuvent l’utiliser pour les boutons, tags, pastilles de statut, chips, avatars, icônes, typographie et jetons de thème qui s’adaptent automatiquement aux thèmes clair et sombre de l’espace de travail.

### Installation

Ajoutez le package à votre application, épinglé à la version livrée avec votre instance SeaRM :

```bash
yarn add searm-ui@1.0.0-alpha.1
```

`searm-ui` est intégré à votre composant front au moment du build, il n’a donc besoin d’être qu’une dépendance de votre application — il n’y a rien à configurer à l’exécution.
...
| --------------------------- | -------------------------------------------------------- |
| `searm-ui/input`           | `Button` et champs de formulaire                         |
| `searm-ui/data-display`    | `Tag`, `Status`, `Chip`, `Avatar`, et plus encore        |
| `searm-ui/feedback`        | `Callout`, `Banner`, `Info`, et plus encore              |
| `searm-ui/typography`      | `H1Title`, `H2Title`, `H3Title`, `Label`, et plus encore |
| `searm-ui/icon`            | Composants `Icon*` (par exemple `IconCheck`)             |
...
Importez des icônes individuelles depuis `searm-ui/icon` :
...
Les composants SeaRM UI correspondent automatiquement au thème clair et sombre de l’espace de travail — le moteur de rendu applique le jeu de couleurs actif sur l’hôte, et les composants résolvent leurs couleurs par rapport à celui-ci.

Pour utiliser les mêmes jetons de design dans vos propres styles inline, appelez le hook `useTheme()`. Il renvoie les jetons de thème de SeaRM (espacement, couleurs, rayons, polices) connectés au thème actif, sans qu’aucune configuration de `ThemeProvider` ne soit nécessaire dans votre composant :
...
Pour bifurquer explicitement selon le jeu de couleurs actif, lisez-le avec `useColorScheme()` depuis `searm-sdk/front-component`, qui renvoie `'light'` ou `'dark'`.
...
Si l’un de ces points vous bloque, [ouvrez un ticket](https://github.com/Vatsa10/Harbour/issues/new/choose) afin qu’il soit priorisé.
...
* Les appels à l’API SeaRM et aux routes de votre application sont proxifiés par l’hôte, donc privilégiez [`RestApiClient`](#calling-the-searm-rest-api). Sur les appels proxifiés, `AbortSignal` et les autres options `RequestInit` sont ignorées, et seuls les corps `string` et `URLSearchParams` sont pris en charge.
...
* `fetch('/rest/people')` n’est jamais associé à l’API SeaRM, car le bac à sable n’a pas d’URL de page pour résoudre un chemin relatif.
