...
Les webhooks envoient des données à vos systèmes en temps réel lorsque des événements se produisent dans SeaRM — aucune interrogation n'est nécessaire. Utilisez-les pour maintenir les systèmes externes synchronisés, déclencher des automatisations ou envoyer des alertes.
...
SeaRM envoie des webhooks pour ces types d'événements :
...
SeaRM signe chaque requête de webhook pour des raisons de sécurité. Validez les signatures pour garantir l'authenticité des requêtes.

### En-têtes

| En-tête                      | Description              |
| ---------------------------- | ------------------------ |
| `X-SeaRM-Webhook-Signature` | Signature HMAC SHA256    |
| `X-SeaRM-Webhook-Timestamp` | Horodatage de la requête |

### Étapes de validation

1. Récupérez l'horodatage depuis `X-SeaRM-Webhook-Timestamp`
2. Créez la chaîne : `{timestamp}:{JSON payload}`
3. Calculez le HMAC SHA256 en utilisant votre secret de webhook
4. Comparez avec `X-SeaRM-Webhook-Signature`
...
const timestamp = req.headers["x-searm-webhook-timestamp"];
...
const isValid = expectedSignature === req.headers["x-searm-webhook-signature"];
```
...
| **Déclencheur de webhook de workflow** | ENTRÉE    | Recevoir des données dans SeaRM depuis des systèmes externes                         |
