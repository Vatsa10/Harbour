...
description: Guide pas à pas pour migrer vos données de n'importe quel CRM vers SeaRM.
---

## Vue d'ensemble

Ce guide vous accompagne pour migrer vos données depuis n'importe quel CRM vers SeaRM. Le processus consiste à auditer vos données, à préparer votre espace de travail sur SeaRM, à exporter depuis votre système actuel et à importer dans SeaRM.
...
Créez un document de correspondance entre votre CRM actuel et SeaRM :
...
## Étape 3 : Configurez votre espace de travail SeaRM

Avant d'importer des données, préparez votre espace de travail SeaRM :
...
Si vos données incluent des références à des utilisateurs (propriétaire du compte, assigné, etc.), ces utilisateurs doivent exister dans SeaRM avant l'importation. Sinon, ces relations ne pourront pas être mises en correspondance.
</Warning>
...
Ouvrez chaque CSV exporté dans une application de tableur et préparez-le pour SeaRM.
...
3. Vérifiez qu'il n'existe pas déjà de doublons dans SeaRM
...
Utilisez `https://domain.com` (pas `domain.com` ni `www.domain.com`). Cela correspond au format de SeaRM et évite les doublons lorsque vous connectez la synchronisation e-mail/calendrier.
</Warning>
...
2. Utilisez les mêmes adresses e-mail que celles utilisées par les utilisateurs pour rejoindre votre espace de travail SeaRM
...
1. Accédez à l'objet dans SeaRM
...
* SeaRM commencera à synchroniser les e-mails avec les enregistrements de contacts
...
<AccordionGroup>
  <Accordion title="Puis-je migrer les flux de travail automatiquement ?">
    Pas pour le moment. Les flux de travail doivent être recréés manuellement dans SeaRM.
  </Accordion>
...
<Tip>
  Vous voulez de l'aide pour votre migration ? [Trouver un partenaire certifié Vingt partenaires] (https://searm.com/partners/list?categories=SOLUTIONING&ref=docs-migrate-crm) pour déplacer vos données, vos pipelines et vos utilisateurs de bout en bout.
</Tip>
