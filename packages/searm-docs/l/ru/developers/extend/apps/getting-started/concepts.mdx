---
title: Понятия
description: Как работают приложения SeaRM — модель сущностей, изоляция в песочнице и жизненный цикл установки.
icon: sitemap
---

Приложения SeaRM — это пакеты TypeScript, которые расширяют ваше рабочее пространство пользовательскими объектами, логикой, компонентами интерфейса и возможностями ИИ. Они работают на платформе SeaRM с полной изоляцией в песочнице и контролем прав доступа.

## Как работают приложения

Приложение — это набор **сущностей**, объявленных с помощью функций `defineEntity()` из пакета `searm-sdk`. SDK обнаруживает эти объявления посредством анализа AST на этапе сборки и формирует **манифест** — полное описание того, что ваше приложение добавляет в рабочее пространство. Эти функции проверяют вашу конфигурацию на этапе сборки и обеспечивают автодополнение в IDE и безопасность типов.
...
| **Компонент фронтенда**   | Изолированный в песочнице интерфейс React внутри SeaRM | [Компоненты фронтенда](/l/ru/developers/extend/apps/layout/front-components)         |
...
* **Компоненты фронтенда** запускаются в Web Workers с использованием Remote DOM — изолированы от основной страницы, но при этом рендерят нативные элементы DOM (не iframes). Они взаимодействуют с SeaRM через хостовый API обмена сообщениями.
* **Права доступа** применяются на уровне API. Токен времени выполнения (`SEARM_APP_ACCESS_TOKEN`) выводится из роли, определённой в `defineApplication()`.
...
* **`yarn searm dev`** — следит за исходными файлами и синхронизирует изменения в реальном времени с подключённым сервером SeaRM. Типизированный клиент API автоматически пересоздаётся при изменении схемы.
* **`yarn searm dev:build`** — компилирует TypeScript, упаковывает логические функции и фронтенд-компоненты с помощью esbuild и формирует манифест.
