...
В обеих средах доступны как предварительно предоставленные модули `searm-client-sdk/core` и `searm-client-sdk/metadata` — они не включаются в бандл, а подставляются сервером во время выполнения.
...
export default defineConfig({
  ...
  test: {
    ...
    env: {
      SEARM_API_URL: 'http://localhost:2020',
      SEARM_API_KEY: 'your-api-key',
    },
  },
});
```
...
const TEST_CONFIG_DIR = path.join(os.tmpdir(), '.searm-sdk-test');

beforeAll(async () => {
  // Verify the server is running
  const response = await fetch(`${SEARM_API_URL}/healthz`);

  if (!response.ok) {
    throw new Error(
      `SeaRM server is not reachable at ${SEARM_API_URL}. ` +
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      remotes: {
        local: {
          apiUrl: process.env.SEARM_API_URL,
          apiKey: process.env.SEARM_API_KEY,
        },
      },
      defaultRemote: 'local',
    }, null, 2),
...
Подпуть `searm-sdk/cli` экспортирует функции, которые можно вызывать напрямую из тестового кода:
...
Убедитесь, что ваш локальный сервер SeaRM запущен, затем:
...
1. Извлекает ваш код
2. Поднимает временный сервер SeaRM с помощью экшена `Vatsa10/Harbour/.github/actions/spawn-searm-docker-image`
3. Устанавливает зависимости с помощью `yarn install --immutable`
4. Запускает `yarn test` с `SEARM_API_URL` и `SEARM_API_KEY`, переданными из выходных данных экшена
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Spawn SeaRM instance
        id: searm
        uses: Vatsa10/Harbour/.github/actions/spawn-searm-docker-image@main
        with:
          searm-version: ${{ env.SEARM_VERSION }}
      ...
      - name: Run integration tests
        run: yarn test
        env:
          SEARM_API_URL: ${{ steps.searm.outputs.server-url }}
          SEARM_API_KEY: ${{ steps.searm.outputs.access-token }}
```

Вам не нужно настраивать секреты — экшен `spawn-searm-docker-image` запускает эфемерный сервер SeaRM прямо в раннере и выводит данные для подключения. Секрет `GITHUB_TOKEN` предоставляется GitHub автоматически.

Чтобы закрепить конкретную версию SeaRM вместо `latest`, измените переменную окружения `SEARM_VERSION` в начале рабочего процесса.
