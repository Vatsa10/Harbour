...
description: CRM verilerinizden kişiselleştirilmiş belgeler oluşturan gerçek bir SeaRM uygulaması geliştirin.
---
...
`2020` portunda çalışan yerel bir SeaRM sunucusu ve ona kimliği doğrulanmış CLI.

Değilse, şimdi bir tane iskeletini oluşturup başlatın:

```bash filename="Terminal"
npx create-searm-app@latest document-generator
...
Tüm eğitim boyunca bir terminalde `yarn searm dev` komutunu çalışır durumda bırakın. `src/` altında her dosya eklediğinizde veya düzenlediğinizde, birkaç saniye içinde sunucunuzla yeniden eşitlenir; böylece siz oluştururken her yeteneğin kullanıcı arayüzünde nasıl ortaya çıktığını izleyebilirsiniz.
