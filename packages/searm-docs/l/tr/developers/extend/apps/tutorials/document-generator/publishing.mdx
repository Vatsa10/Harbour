...
export default defineApplication({
  ...
  websiteUrl: 'https://docs.searm.com/l/tr/developers/extend/apps',
...
Ayrıca uygulamanın bulunabilir olması için `package.json` dosyasına `searm-app` anahtar sözcüğünü ekleyin:
...
yarn searm plan   # preview the metadata diff
```
...
yarn searm app:publish --private -r <remote>
```

`app:publish` varsayılan olarak oluşturur ve npm'e yayımlar; `--private` bunun yerine bir tarball dosyasını bir SeaRM sunucusunun özel kaydına yükler. Yayımlanmış bir uygulamayı bir örneğin pazaryerinde görünür kılmak için, bir katalog eşitlemesini tetikleyin:

```bash filename="Terminal"
yarn searm dev:catalog-sync -r <remote>
```
...
[`packages/searm-apps/examples/document-generator`](https://github.com/Vatsa10/Harbour/tree/main/packages/searm-apps/examples/document-generator).
