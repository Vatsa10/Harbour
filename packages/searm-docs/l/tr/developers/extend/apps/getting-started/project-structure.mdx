---
title: Proje yapısı
description: Önceden hazırlanmış bir SeaRM uygulamasının içinde neler var — dosyalar, klasörler ve her birinin ne yaptığı.
icon: folder-tree
---

`npx create-searm-app` ile oluşturulan yeni bir uygulama şöyle görünür:

```text filename="my-searm-app/"
my-searm-app/
  package.json
  src/
...
Her iki SeaRM SDK paketi de `dependencies` değil, `devDependencies` altında olmalıdır:

```json filename="package.json"
{
  "dependencies": {},
  "devDependencies": {
    "searm-client-sdk": "2.20.0",
    "searm-sdk": "2.20.0",
...
İskele oluşturucu, `searm-sdk` ve `searm-client-sdk` paketlerini kendi sürümüne sabitler — yükseltme yaparken bu ikisini senkron tutun.

* **`searm-sdk`**, `searm` CLI'yi ve derleme/iskelet oluşturma araçlarını sağlar. Yalnızca geliştirme ve derleme zamanında çalışır ve yayımlanmış uygulamanızın çalışma zamanında asla içe aktarılmaz.
* **`searm-client-sdk`** uygulama kodunuz tarafından (`CoreApiClient`, `MetadataApiClient`, `RestApiClient`) içe aktarılır, ancak SeaRM bunu çalışma zamanında sağlar — mantık fonksiyonları bunu oluşturulan bir SDK katmanından alır ve ön yüz bileşenleri bunu sunucu tarafından sunulan modüllerden çözümler. Yüklediğiniz kopya yalnızca tür denetimi ve dağıtım zamanındaki derleme için kullanılır, bu yüzden dağıtılan paket ile birlikte gönderilmesine gerek yoktur.

`dependencies` altında bu paketlerden herhangi birinin tutulması, onu kurulu uygulamanın çalışma zamanı paketine dahil eder ve orada gereksiz yük haline getirir. `searm dev:build`, bunlardan herhangi biri hâlâ `dependencies` altında listelendiğinde bir uyarı verir.
