...
* `serverVariables` kullanın (`applicationVariables` değil) — OAuth kimlik bilgileri sunucu genelidir ve her SeaRM sunucusu için bir OAuth uygulaması vardır.
...
4. Yüklü uygulamayı SeaRM'de bir sunucu yöneticisi olarak açın → karşılık gelen `serverVariables` üzerinde değerleri ayarlayın.
