...
title: Yerel Kurulum
icon: laptop-code
description: SeaRM'i yerel olarak çalıştırmak isteyen katkıda bulunanlar (veya meraklı geliştiriciler) için kılavuz.
---
...
SeaRM'i yüklemeden ve kullanmadan önce bilgisayarınıza aşağıdakileri yüklediğinizden emin olun:
...
SeaRM, en iyi performansı sağlamak için bir Redis önbelleği gerektirir.
...
## Adım 8: SeaRM'yi kullanın

**Ön Uç**

SeaRM'nin ön ucu [http://localhost:3001](http://localhost:3001) adresinde çalışıyor olacak.
Varsayılan demo hesabıyla giriş yapabilirsiniz: `tim@apple.dev` (şifre: `tim@apple.dev`)

**Arka Uç**

* SeaRM'nin sunucusu [http://localhost:3000](http://localhost:3000) adresinde çalışır durumda olacak
