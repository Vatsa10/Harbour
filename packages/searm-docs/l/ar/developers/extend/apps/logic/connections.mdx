...
* استخدم `serverVariables` (وليس `applicationVariables`) — بيانات اعتماد OAuth على مستوى الخادم، ويوجد تطبيق OAuth واحد لكل خادم SeaRM.
...
4. افتح التطبيق المُثبَّت في SeaRM كمسؤول خادم → عيّن القيم على `serverVariables` المقابلة.
