---
title: المفاهيم
description: كيفية عمل تطبيقات SeaRM — نموذج الكيان، العزل (sandboxing)، ودورة حياة التثبيت.
icon: sitemap
---

تطبيقات SeaRM هي حزم TypeScript توسّع مساحة عملك بكائنات مخصّصة، ومنطق، ومكوّنات واجهة مستخدم (UI)، وقدرات ذكاء اصطناعي. تعمل على منصة SeaRM مع عزل كامل وضوابط الأذونات.

## كيف تعمل التطبيقات

التطبيق عبارة عن مجموعة من **الكيانات** يتم إعلانها باستخدام دوال `defineEntity()` من حزمة `searm-sdk`. يكتشف SDK هذه التصريحات عبر تحليل AST وقت البناء وينتج **ملف بيان** — وصفًا كاملًا لما يضيفه تطبيقك إلى مساحة العمل. تتحقق هذه الدوال من تكوينك وقت البناء وتوفّر إكمالًا تلقائيًا في بيئة التطوير وأمان الأنواع.
...
| **مكوّن أمامي**        | واجهة مستخدم React معزولة داخل SeaRM                            | [المكوّنات الأمامية](/l/ar/developers/extend/apps/layout/front-components)       |
...
* **المكوّنات الأمامية** تعمل ضمن Web Workers باستخدام Remote DOM — معزولة عن الصفحة الرئيسية لكنها تعرض عناصر DOM الأصلية (وليس iframes). تتواصل مع SeaRM عبر واجهة API للمضيف تعتمد تمرير الرسائل.
* **الأذونات** تُطبَّق على مستوى واجهة API. يُشتق رمز وقت التشغيل (`SEARM_APP_ACCESS_TOKEN`) من الدور المعرَّف في `defineApplication()`.
...
* **`yarn searm dev`** — يراقب ملفات المصدر لديك ويزامن التغييرات مباشرةً إلى خادم SeaRM متصل. يُعاد توليد عميل API مضبوط الأنواع تلقائيًا عند تغيّر المخطط.
* **`yarn searm dev:build`** — يجمّع TypeScript، ويضمّن الدوال المنطقية والمكوّنات الأمامية باستخدام esbuild، وينتج ملف بيان.
