...
description: 실제 SeaRM 앱을 만들어 CRM 데이터에서 개인 맞춤형 문서를 생성해 보세요.
---
...
포트 `2020`에서 실행 중인 로컬 SeaRM 서버와, 해당 서버에 인증된 CLI가 있어야 합니다.
...
튜토리얼 전체 동안 터미널에서 `yarn searm dev`를 실행 상태로 두세요. `src/` 아래에 파일을 추가하거나 편집할 때마다 몇 초 안에 서버와 다시 동기화되므로, 빌드하면서 각 기능이 UI에 나타나는 과정을 확인할 수 있습니다.
