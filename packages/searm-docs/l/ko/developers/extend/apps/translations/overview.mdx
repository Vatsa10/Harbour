...
SeaRM 앱은 **영어**로 작성됩니다. 소스와 매니페스트의 문자열은 영어 원문이며, `en`은 번역의 기준이 되는 소스 로케일입니다. 번역되지 않은 로케일은 `en`을 기본값으로 사용합니다.
...
`searm-sdk/front-component`에서 번역 헬퍼를 import 하세요:
...
`searm dev:build`는 카탈로그를 컴파일하고 현재 사용자에게 맞는 언어를 제공합니다.
...
`searm dev:build`를 다시 실행하고(및 재배포) 다른 변경 사항과 마찬가지로 처리해야 합니다.

<Note>
번역은 `searm dev:build`(및 `searm apply`)에 의해 컴파일됩니다. 연속 실행되는 `searm dev` watch에서는 소스 문자열이 표시되므로,
