...
icon: laptop-code
description: Der Leitfaden für Mitwirkende (oder neugierige Entwickler), die SeaRM lokal ausführen möchten.
---
...
Bevor Sie SeaRM installieren und verwenden können, stellen Sie sicher, dass Sie Folgendes auf Ihrem Computer installiert haben:
...
SeaRM benötigt einen Redis-Cache, um die beste Leistung zu bieten.
...
## Schritt 8: Verwenden Sie SeaRM

**Frontend**

Das Frontend von SeaRM läuft unter [http://localhost:3001](http://localhost:3001).
Sie können sich mit dem Standard-Demokonto anmelden: `tim@apple.dev` (Passwort: `tim@apple.dev`)

**Backend**

* Der Server von SeaRM wird unter [http://localhost:3000](http://localhost:3000) verfügbar sein.
