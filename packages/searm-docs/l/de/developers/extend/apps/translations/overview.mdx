...
SeaRM-Apps werden auf **Englisch** erstellt: Die Zeichenketten in Ihrem Quellcode und im Manifest
...
Importieren Sie die Übersetzungshelfer aus `searm-sdk/front-component`:

```tsx
import { Trans, t, msg, useTranslate } from 'searm-sdk/front-component';
...
```bash
searm dev:translations-extract                  # collect strings into locales/en.json
searm dev:translations-extract --locale fr-FR   # also scaffold a target locale
```
...
`searm dev:build` kompiliert die Kataloge und stellt die richtige Sprache für den aktuellen Benutzer bereit: Manifest-Labels werden serverseitig aufgelöst und Front-Komponenten-Kataloge werden in jedes Komponenten-Bundle eingebunden. Zur Laufzeit liest eine Komponente die
...
`searm dev:build` erneut auszuführen (und erneut bereitzustellen), genau wie bei jeder anderen Änderung.

<Note>
Übersetzungen werden durch `searm dev:build` (und `searm apply`) kompiliert. Der
kontinuierliche `searm dev`-Watch zeigt Quellzeichenketten an, daher sollten Sie die lokalisierte Ausgabe
