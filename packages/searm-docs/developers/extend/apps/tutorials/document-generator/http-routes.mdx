...
and self-hosted instances, that path is served by your SeaRM server
(e.g. `http://localhost:2020/s/documents/generate`). On SeaRM Cloud, `RestApiClient`
...
[See the helper](https://github.com/Vatsa10/Harbour/blob/main/packages/searm-apps/examples/document-generator/src/utils/render-document.ts).
...
`yarn searm dev:function:logs`, or invoke it directly with
