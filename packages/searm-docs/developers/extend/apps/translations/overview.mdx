...
SeaRM apps are authored in **English**: the strings in your source and manifest
...
Import the translation helpers from `searm-sdk/front-component`:
...
`searm dev:build` compiles the catalogs and serves the right language for the
...
re-running `searm dev:build` (and redeploying), the same as any other change.

<Note>
Translations are compiled by `searm dev:build` (and `searm apply`). The
continuous `searm dev` watch shows source strings, so test localized output
