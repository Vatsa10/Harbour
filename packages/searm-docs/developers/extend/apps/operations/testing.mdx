---
title: Testing
description: Vitest setup, integration tests against a real SeaRM server, type checking, and CI with GitHub Actions.
icon: "flask"
---

The SDK provides programmatic APIs that let you build, deploy, install, and uninstall your app from test code. Combined with [Vitest](https://vitest.dev/) and the typed API clients, you can write integration tests that verify your app works end-to-end against a real SeaRM server.
...
Both environments have `searm-client-sdk/core` and `searm-client-sdk/metadata` available as pre-provided modules — these are not bundled but resolved at runtime by the server.
...
Create a global setup file that verifies the server is reachable, writes a test config for the SDK (`~/.searm/config.test.json`), and syncs the app before tests run:
...
The `searm-sdk/cli` subpath exports functions you can call directly from test code:

| Function | Description |
|----------|-------------|
| `appBuild` | Build the app and optionally pack a tarball |
| `appDeploy` | Upload a tarball to the server |
| `appDevOnce` | Build and sync the app once (same as `yarn searm apply`) |
...
Make sure your local SeaRM server is running, then:
...
The scaffolder generates a ready-to-use workflow at `.github/workflows/ci.yml`. On every push to `main` and every pull request, it spawns an ephemeral SeaRM server in the runner (via the `Vatsa10/Harbour/.github/actions/spawn-searm-app-dev-test` action), then runs `yarn lint`, `yarn typecheck`, `yarn test:unit`, and `yarn test` with `SEARM_API_URL` / `SEARM_API_KEY` pointing at that server. No secrets are required, and you can pin the server version via the `SEARM_VERSION` env at the top of the workflow.
