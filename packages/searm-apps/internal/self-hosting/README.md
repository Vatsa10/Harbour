...
The Self Hosting app collects sign-up telemetry from self-hosted SeaRM instances and turns it into structured CRM records. Each time a user signs up on a self-hosted instance, the app records who they are, which instance they belong to, and automatically links them to a matching Person in your workspace.
...
- **Enrichment state**: Is Enriched, Tried To Be Enriched, Is Personal Email, Is SeaRM.
...
This app requires no server or application variables. To send data into it, point a self-hosted SeaRM instance's telemetry at the webhook path above on the workspace where this app is installed.
