// Single demo seed for the searm-partners app. Idempotent UPSERT by natural key.
...
//
// Run from this app directory, against a running SeaRM server with the app
// installed (deploy first; do NOT run `yarn test` after — its teardown wipes the app).
// Credentials from shell env or a gitignored .env.local (SEARM_PARTNERS_API_URL/KEY).
//
//   yarn searm dev --once
...
Nine Dots helps scaling teams launch SeaRM as an operational system of record, not just a CRM.
...
Elevate is a revenue-operations studio for **B2B SaaS teams** moving from seed to Series C. We replace brittle CRM setups with SeaRM pipelines your GTM team actually maintains.
...
Meridian Craft is an **APAC implementation studio** for fintech and logistics operators running high-volume customer workflows on SeaRM.
...
const PRIMARY_DEMO_PARTNER_SERVICES: DemoPartnerService[] = [
  {
    ...
    description:
      'Designs the full object model, lifecycle stages, and permissions so teams can run on SeaRM with clean data from day one.',
    position: 0,
  },
  {
...
const PRIMARY_DEMO_PARTNER_CASE_STUDIES: DemoPartnerContent[] = [
  {
    ...
    bodyMarkdown:
      'Migrated sales and account-management teams from two CRMs into SeaRM, reducing pipeline update lag from days to hours.',
...
const PARTNERS: Partner[] = [
  { slug: 'nine-dots-ventures', name: 'Nine Dots Ventures', validationStage: 'VALIDATED', availability: 'AVAILABLE', introduction: 'Boutique CRM implementer for real-estate workflows and WhatsApp automation. Nine Dots runs end-to-end SeaRM rollouts for property managers and brokerages across Europe and MENA, with deep multi-language data models and AI-assisted lead intake.', calendarLink: CAL, deploymentExpertise: ['CLOUD', 'SELF_HOST'], region: ['EUROPE', 'MENA'], languagesSpoken: ['ENGLISH', 'FRENCH', 'ARABIC'], partnerTier: 'ADVANCED', partnerScope: ['SOLUTIONING', 'DEVELOPMENT'], typeOfTeam: 'AGENCY', country: 'FRANCE', city: 'Paris', hourlyRateUsd: 250, projectBudgetMinUsd: 15000, skills: ['Real estate', 'WhatsApp', 'Multi-language', 'Workflows', 'Integrations', 'AI'] },
  { slug: 'elevate-consulting', name: 'Elevate Consulting', validationStage: 'VALIDATED', availability: 'AVAILABLE', introduction: 'Revenue-operations partner for B2B SaaS teams scaling seed to Series C. Elevate moves teams off legacy CRMs onto SeaRM with a four-week migration playbook, pipeline rebuilds, and analytics handoff.', calendarLink: CAL, deploymentExpertise: ['CLOUD'], region: ['US', 'LATAM'], languagesSpoken: ['ENGLISH', 'SPANISH'], partnerTier: 'INTERMEDIATE', partnerScope: ['SOLUTIONING'], typeOfTeam: 'AGENCY', country: 'UNITED_STATES', city: 'Austin', hourlyRateUsd: 200, projectBudgetMinUsd: 20000, skills: ['RevOps', 'B2B SaaS', 'Data migration', 'Pipelines', 'Salesforce migration', 'HubSpot migration'] },
  { slug: 'w3villa-technologies', name: 'W3Villa Technologies', validationStage: 'VALIDATED', availability: 'AVAILABLE', introduction: 'Engineering-heavy partner running large self-hosted SeaRM deployments. Specializes in hardened Kubernetes hosting, custom integrations, and 24/7 support contracts for regulated industries across APAC and the Gulf.', calendarLink: CAL, deploymentExpertise: ['CLOUD', 'SELF_HOST'], region: ['APAC', 'MENA'], languagesSpoken: ['ENGLISH', 'HINDI'], partnerTier: 'ADVANCED', partnerScope: ['HOSTING', 'DEVELOPMENT', 'SOLUTIONING'], typeOfTeam: 'AGENCY', country: 'INDIA', city: 'Bangalore', hourlyRateUsd: 120, projectBudgetMinUsd: 10000, skills: ['Self-hosting', 'Kubernetes', 'DevOps', 'Integrations', 'Workflows', 'Enterprise support'] },
...
async function main() {
  const client = new CoreApiClient({
    url: `${requireEnv('SEARM_PARTNERS_API_URL').replace(/\/$/, '')}/graphql`,
    headers: { Authorization: `Bearer ${requireEnv('SEARM_PARTNERS_API_KEY')}` },
  });
