...
import {
  ...
  SEARM_EXPERIENCE_NOTES_MIN_LENGTH,
  SEARM_EXPERIENCE_VALUES,
...
export const submitPartnerApplicationSchema = z
  .object({
    ...
    searmExperience: z.array(z.enum(SEARM_EXPERIENCE_VALUES)).optional(),
    // Apply narrative floor at the API boundary (defense in depth vs website).
    searmExperienceNotes: z
      .string()
      .trim()
      .min(SEARM_EXPERIENCE_NOTES_MIN_LENGTH)
      .optional(),
    searmExperienceProofLink: z
      .string()
      .trim()
  ...
  .superRefine((data, context) => {
    const hasMilestones = (data.searmExperience?.length ?? 0) > 0;
    const hasNotes = data.searmExperienceNotes !== undefined;
    const hasProof = data.searmExperienceProofLink !== undefined;
    ...
    if (!hasMilestones) {
      context.addIssue({
        code: 'custom',
        message: 'At least one SeaRM experience milestone is required.',
        path: ['searmExperience'],
      });
    }
    ...
    if (!hasNotes) {
      context.addIssue({
        code: 'custom',
        message: `SeaRM experience notes must be at least ${SEARM_EXPERIENCE_NOTES_MIN_LENGTH} characters.`,
        path: ['searmExperienceNotes'],
      });
    }
    ...
    if (!hasProof) {
      context.addIssue({
        code: 'custom',
        message: 'SeaRM experience proof link is required.',
        path: ['searmExperienceProofLink'],
      });
    }
  });
