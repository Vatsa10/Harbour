...
const longNotes = (suffix = ''): string => {
  const base =
    'Built a SeaRM workspace for Acme with custom objects, roles, and a sales pipeline. ';
  let notes = '';
  while (notes.length < SEARM_EXPERIENCE_NOTES_MIN_LENGTH) {
    notes += base;
  }
  return `${notes}${suffix}`;
};
...
const validRow = (
  ...
  "What you've built in SeaRM": 'Custom apps, Workflows',
...
describe('mapTallyExperienceCsvRow', () => {
  it('maps a valid Tally row to an update intent by partnerId', () => {
    ...
    expect(result).toEqual({
      ...
      intent: {
        partnerId: VALID_PARTNER_ID,
        searmExperience: ['CUSTOM_APPS', 'WORKFLOWS'],
        searmExperienceNotes: notes.trim(),
        searmExperienceProofLink: 'https://www.loom.com/share/abc123',
      },
    });
  });
  ...
  it('rejects unknown milestone labels on the row', () => {
    const result = mapTallyExperienceCsvRow(
      validRow({
        "What you've built in SeaRM": 'Custom apps, Self-hosting',
      }),
    );
  ...
  it('resolves Tally-style headers with helper copy via partial match', () => {
    const result = mapTallyExperienceCsvRow({
      partnerId: VALID_PARTNER_ID,
      "What you've built in SeaRM (select every area)": 'Data models',
    ...
    if (result.ok) {
      expect(result.intent.searmExperience).toEqual(['DATA_MODELS']);
      expect(result.intent.searmExperienceProofLink).toBe(
