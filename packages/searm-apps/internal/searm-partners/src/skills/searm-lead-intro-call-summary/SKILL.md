...
implementation partner. Designed for SeaRM's partner/CRM pipeline, tuned for messy,
...
- If the transcript has NO speaker labels, infer from context who is the vendor (SeaRM)
...
3. PEOPLE ON THE CALL — name, role/title, side (SeaRM vs prospect); infer roles if
   unlabeled and flag uncertain names
4. CURRENT SITUATION — what CRM/tools they use today; specific pains
5. WHY THEY'RE INTERESTED IN SEARM
...
sibling of the source (e.g. next to the recording) or, for SeaRM work, the
...
- This is tuned for SeaRM discovery calls, but the schema generalizes — swap the vendor
