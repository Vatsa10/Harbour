...
// ---------------------------------------------------------------------------
// Call Recorder end-to-end behavior against a live SeaRM server.
//
...
const WORKSPACE_API_KEY_ENV = 'SEARM_API_KEY';
...
const workspaceGraphql = async (
  ...
  const response = await fetch(`${process.env.SEARM_API_URL}/graphql`, {
    method: 'POST',
    headers: {
...
// ---------------------------------------------------------------------------
// Recall API fake, installed as a fetch interceptor. SeaRM API traffic is
...
class FakeRecallApi {
  ...
  botForCallRecording(callRecordingId: string): FakeRecallBot | undefined {
    return [...this.bots.values()].find(
      (bot) => bot.metadata.searmCallRecordingId === callRecordingId,
    );
  }
...
describe('call recorder app lifecycle (integration)', () => {
  ...
  // SeaRM API traffic always uses the real fetch.
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('SEARM_FUNCTIONS_URL', FUNCTIONS_URL);
