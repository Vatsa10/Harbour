...
const buildBot = ({
  ...
  id,
  metadata: {
    ...(searmCallRecordingId === undefined ? {} : { searmCallRecordingId }),
    ...(searmWorkspaceId === undefined ? {} : { searmWorkspaceId }),
  },
});
...
const buildCurrentWorkspaceBot = ({
  ...
  buildBot({
    id,
    searmCallRecordingId,
    searmWorkspaceId: CURRENT_WORKSPACE_ID,
  });
...
describe('cleanupOrphanedRecallBots', () => {
  ...
  beforeEach(() => {
    ...
    process.env.SEARM_APP_ACCESS_TOKEN = buildAccessToken({
    ...
    stubRecallApi({
      bots: [
        buildCurrentWorkspaceBot({
          id: 'claimed-bot',
          searmCallRecordingId: 'call-recording-1',
        }),
      ],
    });
  ...
  it('lists only bots claimed by the current workspace', async () => {
    ...
    expect(listRequestParameters.get('metadata__searmWorkspaceId')).toBe(
  ...
  it('cancels bots whose call recording no longer exists', async () => {
    stubRecallApi({
      bots: [
        buildCurrentWorkspaceBot({
          id: 'orphan-bot',
          searmCallRecordingId: 'call-recording-gone',
        }),
      ],
    });
    ...
    stubRecallApi({
      bots: [
        buildBot({
          ...
          searmCallRecordingId: 'call-recording-gone',
          searmWorkspaceId: OTHER_WORKSPACE_ID,
        }),
      ],
    });
