...
describe('retryFailedRecallCancellations', () => {
  ...
  it('recovers and cancels a provider bot when endsAt is missing after the meeting starts', async () => {
    ...
    expect(listRequestParameters.has('metadata__searmCallRecordingId')).toBe(
      false,
    );
    expect(fetchMock).toHaveBeenCalledWith(
              ...
              results: [
                {
                  ...
                  metadata: {
                    searmWorkspaceId: WORKSPACE_ID,
                    searmCallRecordingId: 'call-recording-recent',
                  },
                },
                {
                  id: 'recall-bot-aged',
                  metadata: {
                    searmWorkspaceId: WORKSPACE_ID,
                    searmCallRecordingId: 'call-recording-aged',
                  },
                },
              ],
            }),
