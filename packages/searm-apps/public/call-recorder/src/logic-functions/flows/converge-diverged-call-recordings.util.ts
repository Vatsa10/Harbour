...
import { SEARM_PAGE_SIZE } from 'src/logic-functions/constants/searm-page-size';
...
const listRecallBotsByIdForConvergence = async (
  ...
  const listResult = await listScheduledRecallBots({
    ...
    metadata: { searmWorkspaceId: currentWorkspaceId },
  });
...
const isCurrentWorkspaceManagedBot = ({
  ...
  const claimedWorkspaceId = bot.metadata.searmWorkspaceId;
