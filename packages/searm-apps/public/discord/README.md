...
Connect Discord to your SeaRM workflows. Post messages, edit them, react,
...
> your SeaRM admin needs to follow the [Self-hosting setup](#self-hosting-setup-admin-only)
...
- **Slash commands / interactions.** The bot doesn't register or respond to
  `/commands`.
- **Per-workspace identity.** All SeaRM workspaces in the same SeaRM
...
1. Visit https://discord.com/developers/applications and click
   **New Application**.
2. Name it (e.g. *"SeaRM Bot"*) and create. Use a dedicated app — don't
...
### 3. Wire the bot token into SeaRM

1. In SeaRM: **Settings → Applications → Discord → Settings tab**
   (gear icon).
