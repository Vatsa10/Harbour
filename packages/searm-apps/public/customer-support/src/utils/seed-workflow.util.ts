...
//   packages/searm-server/src/modules/workflow/workflow-templates/
...
// (packages/searm-server/src/modules/workflow/workflow-trigger/types/
...
export type WorkflowStepTemplate = {
  ...
  // lives in `searm-shared/workflow`, and searm-shared is not a dependency
  // an installable app can resolve (it is not published alongside searm-sdk /
  // searm-client-sdk). See WORKFLOW_ACTION_TYPE below.
  type: string;
  name: string;
  valid?: boolean;
...
// The subset of searm-shared's WorkflowActionType an app can reach without
// depending on searm-shared. Values are the enum's string values, verified
// against packages/searm-shared/src/workflow/types/WorkflowActionType.ts and
// against the shipped built-in templates in
// packages/searm-server/src/modules/workflow/workflow-templates/constants/
...
export const seedWorkflow = async (
  ...
  // in searm-client-sdk/dist/core/generated/index.d.ts), so the response is
