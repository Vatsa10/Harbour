...
const program = new Command(packageJson.name)
  .description('CLI tool to initialize a new SeaRM application')
  .version(
    packageJson.version,
    '-v, --version',
    'Output the current version of create-searm-app.',
  )
  .argument('[directory]')
  .option('-n, --name <name>', 'Application name')
  .option('-d, --display-name <displayName>', 'Application display name')
  .option('--description <description>', 'Application description')
  .option('--url <url>', 'SeaRM server URL (default: http://localhost:2020)')
