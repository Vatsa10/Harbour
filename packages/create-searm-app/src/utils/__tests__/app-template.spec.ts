...
import createSearmAppPackageJson from 'package.json';
...
describe('copyBaseApplicationProject', () => {
  ...
  beforeEach(async () => {
    testAppDirectory = join(
      tmpdir(),
      `test-searm-app-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it('should update package.json with app name and SDK versions', async () => {
    ...
    expect(packageJson.devDependencies['searm-sdk']).toBe(
      createSearmAppPackageJson.version,
    );
    expect(packageJson.devDependencies['searm-client-sdk']).toBe(
      createSearmAppPackageJson.version,
    );
  });
