...
const APP_ACCESS_TOKEN_ENV_KEY = 'SEARM_APP_ACCESS_TOKEN';
const API_KEY_ENV_KEY = 'SEARM_API_KEY';

type SearmGeneratedClientOptions = ClientOptions;
...
const defaultOptions: SearmGeneratedClientOptions = {
  url: '__SEARM_DEFAULT_URL__',
  headers: {
    'Content-Type': 'application/json',
  },
};

export class SearmGeneratedClient {
  ...
  constructor(options?: SearmGeneratedClientOptions) {
    const merged: SearmGeneratedClientOptions = {
      ...defaultOptions,
      ...options,
    };
    ...
    if (!this.fetchImplementation) {
      throw new Error(
        'Global `fetch` function is not available, ' +
          'pass a fetch implementation to the SeaRM client',
      );
    }
    ...
    if (!this.refreshAccessTokenPromise) {
      this.refreshAccessTokenPromise = refreshAccessTokenFunction()
        ...
        .catch((refreshError: unknown) => {
          console.error('SeaRM client: token refresh failed', refreshError);
