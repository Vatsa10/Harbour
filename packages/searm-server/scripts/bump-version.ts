 ...
 *  |_|   \\_/\\_/ \\___|_| |_|\\__|\\__, | Generated by: npx nx version:bump searm-server
 *                              |___/
 */
...
const getCurrentVersion = (): string => {
  const filePath = `${CONSTANTS_DIR}/searm-current-version.constant.ts`;
...
const getPreviousVersions = (): string[] => {
  const filePath = `${CONSTANTS_DIR}/searm-previous-versions.constant.ts`;
  const content = readFileSync(filePath, 'utf-8');
...
const getNextVersions = (): string[] => {
  const filePath = `${CONSTANTS_DIR}/searm-next-versions.constant.ts`;
  const content = readFileSync(filePath, 'utf-8');
  const matches = content.match(/'[0-9.]+'/g);
...
const updateCurrentVersion = (newVersion: string): void => {
  ...
  writeFileSync(
    filePath,
    `${AUTO_GENERATED_HEADER}export const SEARM_CURRENT_VERSION = '${newVersion}' as const;\n`,
  );

  console.log(`Updated SEARM_CURRENT_VERSION to '${newVersion}'`);
};
...
const updatePreviousVersions = (versions: string[]): void => {
  ...
  writeFileSync(
    filePath,
    `${AUTO_GENERATED_HEADER}export const SEARM_PREVIOUS_VERSIONS = [\n${formattedVersions}\n] as const;\n`,
  );

  console.log(
    `Updated SEARM_PREVIOUS_VERSIONS with ${versions.length} versions`,
  );
};
...
const updateNextVersions = (versions: string[]): void => {
  ...
  writeFileSync(
    filePath,
    `${AUTO_GENERATED_HEADER}export const SEARM_NEXT_VERSIONS = [\n${formattedVersions}\n] as const;\n`,
  );

  console.log(`Updated SEARM_NEXT_VERSIONS with ${versions.length} versions`);
};
...
const resolveNewVersion = ({
  ...
  if (nextVersions.length === 0) {
    ...
    console.log(
      `\nNo version provided and SEARM_NEXT_VERSIONS is empty, computed from current: '${computed}'`,
    );
  ...
  console.log(
    `\nNo version provided, using SEARM_NEXT_VERSIONS[0]: '${nextVersions[0]}'`,
  );
...
const bumpVersion = (newVersionArg?: string): void => {
  ...
  console.log('\nCurrent state:');
  console.log(`  SEARM_CURRENT_VERSION: '${currentVersion}'`);
  console.log(
    `  SEARM_PREVIOUS_VERSIONS: [${previousVersions.map((v) => `'${v}'`).join(', ')}]`,
  );
  console.log(
    `  SEARM_NEXT_VERSIONS: [${nextVersions.map((v) => `'${v}'`).join(', ')}]`,
  );
  ...
  console.log('\nNew state:');
  console.log(`  SEARM_CURRENT_VERSION: '${newVersion}'`);
  console.log(
    `  SEARM_PREVIOUS_VERSIONS: [${updatedPreviousVersions.map((v) => `'${v}'`).join(', ')}]`,
  );
  console.log(
    `  SEARM_NEXT_VERSIONS: [${updatedNextVersions.map((v) => `'${v}'`).join(', ')}]`,
  );
