...
if (process.env.EXCEPTION_HANDLER_DRIVER === ExceptionHandlerDriver.SENTRY) {
  Sentry.init({
    ...
    beforeSendSpan: (span) => {
      const searmContext = Sentry.getIsolationScope().getScopeData().contexts
        ?.searm as
        | {
            workspace_id?: string;
      ...
      if (!searmContext?.workspace_id) {
        return span;
      }

      span.data = {
        ...span.data,
        'searm.workspace.id': searmContext.workspace_id,
        ...(searmContext.user_workspace_id && {
          'searm.user_workspace.id': searmContext.user_workspace_id,
        }),
      };
