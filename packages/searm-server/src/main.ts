...
import { configTransformers } from 'src/engine/core-modules/searm-config/utils/config-transformers.util';
...
const bootstrap = async () => {
  ...
  const searmConfigService = app.get(SearmConfigService);
  ...
  const trustProxyRaw = searmConfigService.get('TRUST_PROXY');
  ...
  applyCredentialedCors(app, searmConfigService);

  app.use(session(getSessionStorageOptions(searmConfigService)));
  ...
  const keepAliveTimeout = searmConfigService.get(
  ...
  await app.listen(searmConfigService.get('NODE_PORT'));
};
