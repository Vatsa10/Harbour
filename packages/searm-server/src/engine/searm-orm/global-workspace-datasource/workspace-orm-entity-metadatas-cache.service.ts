...
import { buildEntitySchemaMetadataMaps } from 'src/engine/searm-orm/global-workspace-datasource/types/entity-schema-metadata.type';
...
> {
  ...
  async computeForCache(workspaceId: string): Promise<EntityMetadata[]> {
    const [objectMetadatas, fieldMetadatas, searmStandardApplication] =
      await Promise.all([
