...
import { SearmStandardApplicationService } from 'src/engine/workspace-manager/searm-standard-application/services/searm-standard-application.service';
...
export class DevSeederService {
  constructor(
    ...
    private readonly searmStandardApplicationService: SearmStandardApplicationService,
  ...
  public async seedDev(
    ...
    const appVersion = this.searmConfigService.get('APP_VERSION') ?? 'unknown';
    ...
    const { workspaceCustomFlatApplication, searmStandardFlatApplication } =
    ...
    await this.searmStandardApplicationService.synchronizeSearmStandardApplicationOrThrow(
      {
        workspaceId,
      },
    );
    ...
    await this.sdkClientGenerationService.generateSdkClientForApplication({
      workspaceId,
      applicationId: searmStandardFlatApplication.id,
    ...
    await seedPageLayouts({
      workspaceId,
      flatApplication: searmStandardFlatApplication,
      objectMetadataItems,
  ...
  private async seedCoreSchema({
    ...
    try {
      ...
      await this.applicationService.createSearmStandardApplication(
        {
          workspaceId,
