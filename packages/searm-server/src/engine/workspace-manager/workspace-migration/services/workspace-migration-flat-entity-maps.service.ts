...
import { AllMetadataName } from 'searm-shared/metadata';
...
export class WorkspaceMigrationFlatEntityMapsService {
  ...
  private computeAllInvolvedApplicationIds({
    ...
    if (!isDefined(searmStandardApplicationId)) {
      throw new FlatEntityMapsException(
        'SeaRM standard application not found in workspace',
    ...
    const isBuildingSearmStandardApplication =
      applicationUniversalIdentifier ===
      SEARM_STANDARD_APPLICATION.universalIdentifier;

    if (!isBuildingSearmStandardApplication) {
      applicationIds.add(searmStandardApplicationId);
    }
