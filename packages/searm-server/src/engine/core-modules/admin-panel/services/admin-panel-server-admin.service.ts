...
import { ServerAdminAccessChangedEmail, renderEmail } from 'searm-emails';
import { SOURCE_LOCALE } from 'searm-shared/translations';
...
export class AdminPanelServerAdminService {
  ...
  private async assertFreshStepUpAuthentication({
    ...
    const isDevelopment =
      this.searmConfigService.get('NODE_ENV') === NodeEnvironment.DEVELOPMENT;
  ...
  private async notifyAdministrators({
    ...
    try {
      ...
      const from = `${this.searmConfigService.get('EMAIL_FROM_NAME')} <${this.searmConfigService.get('EMAIL_FROM_ADDRESS')}>`;
