...
export class EmailDriverFactory extends DriverFactoryBase<EmailDriverInterface> {
  ...
  protected buildConfigKey(): string {
    const driver = this.searmConfigService.get('EMAIL_DRIVER');
  ...
  protected createDriver(): EmailDriverInterface {
    ...
    switch (driver) {
      ...
      case EmailDriver.SMTP: {
        const host = this.searmConfigService.get('EMAIL_SMTP_HOST');
        const port = this.searmConfigService.get('EMAIL_SMTP_PORT');
        const user = this.searmConfigService.get('EMAIL_SMTP_USER');
        const pass = this.searmConfigService.get('EMAIL_SMTP_PASSWORD');
        const noTLS = this.searmConfigService.get('EMAIL_SMTP_NO_TLS');
