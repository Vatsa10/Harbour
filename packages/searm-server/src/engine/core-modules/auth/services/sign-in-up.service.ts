...
import { SEARM_ICONS_BASE_URL } from 'searm-shared/constants';
...
@Injectable()
// oxlint-disable-next-line searm/inject-workspace-repository
      ...
      [
        {
          ...
          serverUrl: this.searmConfigService.get('SERVER_URL'),
          serverId: this.searmConfigService.get('SERVER_ID'),
        },
      ],
      undefined,
    );
    ...
    try {
      const { user, workspace } = await this.dataSource.transaction(
        async (entityManager) => {
          ...
          if (isWorkEmailFound) {
            const logoUrl = `${SEARM_ICONS_BASE_URL}/${getDomainFromEmailOrThrow(email)}`;
            const logoFile =
          ...
          // ToS/signup, so acceptance = execution. Only relevant on SeaRM's
          // managed cloud (multi-workspace), where SeaRM is the Processor
          // hosting the data; on self-hosted deployments SeaRM is not the
          ...
          if (
            this.searmConfigService.get('IS_MULTIWORKSPACE_ENABLED') === true
          ) {
            await queryRunner.manager.save(
              ...
              buildDpaAgreementRecord({
                ...
                region:
                  this.searmConfigService.get('DPA_DEPLOYMENT_REGION') ??
                  DEFAULT_DPA_REGION,
                acceptedAt: new Date(),
