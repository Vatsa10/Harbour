...
const eventSinksProvider = {
  ...
  useFactory: (
    ...
    const configuredSinkNames = searmConfigService.get('EVENT_SINKS');
    ...
    return getAvailableSinkNames(configuredSinkNames, {
      hasClickhouseUrl: Boolean(searmConfigService.get('CLICKHOUSE_URL')),
    })
      .map((name) => sinkByName[name.toLowerCase()])
      .filter(isDefined);
  },
  inject: [SearmConfigService, ClickHouseEventSink, ConsoleEventSink],
};
