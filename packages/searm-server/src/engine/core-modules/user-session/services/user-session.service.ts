...
export class UserSessionService {
  ...
  async issueSessionForTokenPair({
    ...
    if (!this.searmConfigService.get('AUTH_COOKIE_SESSIONS_ENABLED')) {
      return;
    }
  ...
  async createSession(input: CreateUserSessionInput): Promise<{
    ...
    const lifetime =
      input.isImpersonating === true
        ...
        : this.searmConfigService.get('SESSION_ABSOLUTE_LIFETIME');
  ...
  private getIdleTimeoutMs(): number {
    return ms(this.searmConfigService.get('SESSION_IDLE_TIMEOUT'));
  }
