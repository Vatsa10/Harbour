...
import {
  SEARM_ALL_VERSIONS,
  type SearmAllVersion,
} from 'src/engine/core-modules/upgrade/constants/searm-all-versions.constant';
import { SEARM_CROSS_UPGRADE_SUPPORTED_VERSIONS } from 'src/engine/core-modules/upgrade/constants/searm-cross-upgrade-supported-version.constant';
...
export class UpgradeCommandRegistryService implements OnModuleInit {
  ...
  onModuleInit(): void {
    for (const version of SEARM_ALL_VERSIONS) {
    ...
    for (const [version, bundle] of this.bundlesByVersion) {
      ...
      if (totalCount > 0) {
        const crossUpgradeLabel = (
          SEARM_CROSS_UPGRADE_SUPPORTED_VERSIONS as readonly string[]
        ).includes(version)
  ...
  getBundleForVersion(version: SearmAllVersion): VersionBundle {
  ...
  getCrossUpgradeSupportedFastInstanceCommands(): RegisteredFastInstanceCommand[] {
    return SEARM_CROSS_UPGRADE_SUPPORTED_VERSIONS.flatMap(
  ...
  private validateAtLeastOneVersionBundleHasWorkspaceCommands(): void {
    ...
    for (const version of SEARM_CROSS_UPGRADE_SUPPORTED_VERSIONS) {
      const bundle = this.getBundleForVersion(version);
  ...
  private validateNoVersionDuplicatesAcrossConstants(): void {
    ...
    if (uniqueVersions.size !== allVersions.length) {
      ...
      throw new Error(
        `Duplicate version(s) across SEARM_PREVIOUS_VERSIONS, SEARM_CURRENT_VERSION, and SEARM_NEXT_VERSIONS: ${duplicates.join(', ')}`,
      );
    }
  }

  private validatePreviousVersionsNotEmpty(): void {
    if ((SEARM_PREVIOUS_VERSIONS as readonly string[]).length === 0) {
      throw new Error(
        'SEARM_PREVIOUS_VERSIONS must contain at least one version before SEARM_CURRENT_VERSION',
      );
    }
  }
}
