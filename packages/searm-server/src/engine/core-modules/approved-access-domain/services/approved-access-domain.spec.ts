...
import { type WorkspaceScopedRepository } from 'src/engine/searm-orm/workspace-scoped-repository/workspace-scoped-repository';
...
jest.mock('searm-emails', () => ({
  ...jest.requireActual('searm-emails'),
  ...
  describe('sendApprovedAccessDomainValidationEmail', () => {
    ...
    it('should send a validation email if all conditions are met', async () => {
      ...
      jest
        ...
        .mockReturnValue(new URL('https://sub.searm.com'));
      ...
      expect(emailService.send).toHaveBeenCalledWith({
        from: 'John Doe (via SeaRM) <no-reply@example.com>',
        to: email,
