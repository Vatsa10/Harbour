...
export const SEARM_MCP_HELPER = `# Auto-injected SeaRM MCP helper - provides access to SeaRM tools
import os
import json
...
class SearmMCP:
    """Helper for calling SeaRM tools from sandboxed code.
    ...
    def __init__(self):
        self.url = os.environ.get('SEARM_SERVER_URL', '')
        self.token = os.environ.get('SEARM_API_TOKEN', '')
    ...
    def call_tool(self, name: str, arguments: dict = None):
        ...
        Example:
            companies = searm.call_tool('find_many_companies', {'limit': 5})
            # companies == {'records': [...], 'count': '5'}
        """
        if not self._available:
            raise RuntimeError('SeaRM MCP bridge not available. Missing requests library or credentials.')
    ...
    def bulk_upsert(self, plural: str, records: list, batch_size: int = 200):
        ...
        Example:
            summary = searm.bulk_upsert('people', people_rows)
        """
...
# --------------------------------------------------------------------------
# \`searm\` is a pre-built instance of the SearmMCP class above. It is
# already bound in this module scope — DO NOT \`import searm\`. There is
# no Python package by that name. Just use it directly, e.g.:
#     companies = searm.call_tool('find_many_companies', {'limit': 10})
# --------------------------------------------------------------------------
searm = SearmMCP()
`;
