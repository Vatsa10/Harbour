...
import { replaceCoreClient } from 'searm-client-sdk/generate';
...
const SDK_CLIENT_ARCHIVE_NAME = 'searm-client-sdk.zip';
...
export class SdkClientGenerationService {
  ...
  async enqueueSdkClientGenerationForWorkspace(
    ...
    await Promise.all(
      [searmStandardFlatApplication, workspaceCustomFlatApplication].map(
        (application) =>
  ...
  private async generateAndStore({
    ...
    try {
      ...
      const tempPackageRoot = join(sourceTemporaryDir, 'searm-client-sdk');
