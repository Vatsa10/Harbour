...
import { type AwsRegion } from 'src/engine/core-modules/searm-config/interfaces/aws-region.interface';
...
import { CastToLogLevelArray } from 'src/engine/core-modules/searm-config/decorators/cast-to-log-level-array.decorator';
import { CastToMeterDriverArray } from 'src/engine/core-modules/searm-config/decorators/cast-to-meter-driver.decorator';
import { CastToPositiveNumber } from 'src/engine/core-modules/searm-config/decorators/cast-to-positive-number.decorator';
import { CastToTypeORMLogLevelArray } from 'src/engine/core-modules/searm-config/decorators/cast-to-typeorm-log-level-array.decorator';
import { CastToUpperSnakeCase } from 'src/engine/core-modules/searm-config/decorators/cast-to-upper-snake-case.decorator';
import { ConfigVariablesMetadata } from 'src/engine/core-modules/searm-config/decorators/config-variables-metadata.decorator';
import { IsAWSRegion } from 'src/engine/core-modules/searm-config/decorators/is-aws-region.decorator';
...
import { IsNonNegativeDuration } from 'src/engine/core-modules/searm-config/decorators/is-non-negative-duration.decorator';
import { IsPositiveDuration } from 'src/engine/core-modules/searm-config/decorators/is-positive-duration.decorator';
import { IsOptionalOrEmptyString } from 'src/engine/core-modules/searm-config/decorators/is-optional-or-empty-string.decorator';
import { IsStrictlyLowerThan } from 'src/engine/core-modules/searm-config/decorators/is-strictly-lower-than.decorator';
import { IsSearmSemVer } from 'src/engine/core-modules/searm-config/decorators/is-searm-semver.decorator';
import { ConfigVariableType } from 'src/engine/core-modules/searm-config/enums/config-variable-type.enum';
import { ConfigVariablesGroup } from 'src/engine/core-modules/searm-config/enums/config-variables-group.enum';
import {
  ConfigVariableException,
  ConfigVariableExceptionCode,
} from 'src/engine/core-modules/searm-config/searm-config.exception';
...
export class ConfigVariables {
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.ADVANCED_SETTINGS,
    description:
      "Enable or disable requests to searm-icons to get companies' icons",
    type: ConfigVariableType.BOOLEAN,
  })
  ALLOW_REQUESTS_TO_SEARM_ICONS = true;
  ...
  EMAIL_FROM_NAME = 'Felix from SeaRM';
  ...
  @ConfigVariablesMetadata({
    ...
    description:
      'Deployment region that determines the contracting DPA Processor entity, hosting region and governing law. EU (default) = SeaRM.com SAS / Frankfurt / France; US = SeaRM, Inc. / United States. Must match where Customer Personal Data actually lives.',
    type: ConfigVariableType.ENUM,
  ...
  @CastToLogLevelArray()
  @IsOptional()
  LOG_LEVELS: SearmLogLevel[] = ['log', 'error', 'warn', 'performance'];
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SERVER_CONFIG,
    description:
      'ISO date from which HTTP logic functions are no longer served on the legacy /s/ route. Functions created on or after this date are only reachable on the isolated public domain (*.withsearm.com). Only enforced when PUBLIC_DOMAIN_URL is set; leave empty to keep serving every function on /s/ (default for self-hosting).',
    type: ConfigVariableType.STRING,
  })
  @IsDateString()
  @IsOptional()
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SERVER_CONFIG,
    description: 'Base URL for the Enterprise API on searm.com',
    isHiddenInAdminPanel: true,
    type: ConfigVariableType.STRING,
  })
  @IsOptional()
  ENTERPRISE_API_URL: string = 'https://searm.com/api/enterprise';
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SERVER_CONFIG,
    description: 'SeaRM server version',
    type: ConfigVariableType.STRING,
    isEnvOnly: true,
    isHiddenInAdminPanel: true,
  })
