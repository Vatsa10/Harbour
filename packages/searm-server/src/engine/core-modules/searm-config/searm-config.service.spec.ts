...
import { type ConfigVariables } from 'src/engine/core-modules/searm-config/config-variables';
...
jest.mock(
  'src/engine/core-modules/searm-config/constants/config-variables-masking-config',
  () => ({
...
jest.mock(
  'src/engine/core-modules/searm-config/utils/is-env-only-config-var.util',
  () => ({
    isEnvOnlyConfigVar: jest.fn(),
  }),
);

type SearmConfigServicePrivateProps = {
...
const setupTestModule = async (isDatabaseConfigEnabled = true) => {
  ...
  return {
    service: module.get<SearmConfigService>(SearmConfigService),
...
const setPrivateProps = (
  service: SearmConfigService,
  props: Partial<SearmConfigServicePrivateProps>,
) => {
...
describe('SearmConfigService', () => {
  let service: SearmConfigService;
