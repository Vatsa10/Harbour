...
export class FileStorageDriverFactory extends DriverFactoryBase<StorageDriver> {
  ...
  protected buildConfigKey(): string {
    const storageType = this.searmConfigService.get('STORAGE_TYPE');

    if (storageType === StorageDriverType.LOCAL) {
      const storagePath = this.searmConfigService.get('STORAGE_LOCAL_PATH');
  ...
  protected createDriver(): StorageDriver {
    ...
    switch (storageType) {
      ...
      case StorageDriverType.S_3: {
        const bucketName = this.searmConfigService.get('STORAGE_S3_NAME');
        const endpoint = this.searmConfigService.get('STORAGE_S3_ENDPOINT');
        const region = this.searmConfigService.get('STORAGE_S3_REGION');
        const accessKeyId = this.searmConfigService.get(
          'STORAGE_S3_ACCESS_KEY_ID',
        );
        const secretAccessKey = this.searmConfigService.get(
          'STORAGE_S3_SECRET_ACCESS_KEY',
        );
        const presignEnabled = this.searmConfigService.get(
          'STORAGE_S3_PRESIGNED_URL_ENABLED',
        );
        const presignEndpointOverride = this.searmConfigService.get(
