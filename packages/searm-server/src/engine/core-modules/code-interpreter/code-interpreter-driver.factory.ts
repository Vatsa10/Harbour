...
export class CodeInterpreterDriverFactory extends DriverFactoryBase<CodeInterpreterDriver> {
  ...
  protected buildConfigKey(): string {
    const driverType = this.searmConfigService.get('CODE_INTERPRETER_TYPE');
  ...
  protected createDriver(): CodeInterpreterDriver {
    ...
    switch (driverType) {
      ...
      case CodeInterpreterDriverType.LOCAL: {
        const nodeEnv = this.searmConfigService.get('NODE_ENV');
        ...
        return new LocalDriver({
          timeoutMs,
          idleTimeoutMs: this.searmConfigService.get(
      ...
      case CodeInterpreterDriverType.E_2_B: {
        const apiKey = this.searmConfigService.get('E2B_API_KEY');
