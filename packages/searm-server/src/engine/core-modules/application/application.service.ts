...
export class ApplicationService {
  ...
  async findWorkspaceSearmStandardAndCustomApplicationOrThrow({
    ...
    const searmStandardFlatApplication =
      flatApplicationMaps.byId[searmStandardApplicationId];
    const workspaceCustomFlatApplication =
      flatApplicationMaps.byId[workspace.workspaceCustomApplicationId];

    if (
      !isDefined(searmStandardFlatApplication) ||
  ...
  async findSearmStandardApplicationOrThrow(workspaceId: string): Promise<{
    ...
    const { searmStandardFlatApplication } =
      await this.findWorkspaceSearmStandardAndCustomApplicationOrThrow({
        workspace,
      });

    const application = await this.applicationRepository.findOne({
      where: {
        id: searmStandardFlatApplication.id,
  ...
  async createSearmStandardApplication(
    {
      workspaceId,
    ...
    const existingApplication = await this.findByUniversalIdentifier({
      universalIdentifier: SEARM_STANDARD_APPLICATION.universalIdentifier,
      workspaceId,
    });
    ...
    const searmStandardApplication = await this.create(
      {
        ...SEARM_STANDARD_APPLICATION,
    ...
    return searmStandardApplication;
  }
