...
export class ApplicationRegistrationClaimService {
  ...
  async buildGithubAuthorizationUrl(params: {
    ...
    const clientId = this.searmConfigService.get('APP_CLAIM_GITHUB_CLIENT_ID');
  ...
  private async fetchProvenancePublisherLogin(params: {
    ...
    const registryUrl = this.searmConfigService.get('APP_REGISTRY_URL');
    const version =
      params.version ??
    ...
    try {
      const response = await axios.get(
        ...
        {
          headers: { 'User-Agent': 'SeaRM-Marketplace' },
          timeout: 10_000,
        },
      );
  ...
  private async exchangeGithubCode(code: string): Promise<string> {
    const clientId = this.searmConfigService.get('APP_CLAIM_GITHUB_CLIENT_ID');
    const clientSecret = this.searmConfigService.get(
