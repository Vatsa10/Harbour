...
import { getSystemRelationFieldUniversalIdentifier } from 'searm-shared/application';
import {
  DEFAULT_RELATIONS_OBJECTS_STANDARD_IDS,
  STANDARD_OBJECTS,
} from 'searm-shared/metadata';
...
// API transpiler, searm-standard) stamps the reverse morph field with the
...
export class ReconcileSystemRelationFieldUniversalIdentifierCommand extends ProvisionedWorkspaceCommandRunner {
  ...
  override async runOnWorkspace({
    ...
    for (const flatFieldMetadata of Object.values(
      ...
      // for custom objects, by searm-standard for standard ones), so it must
