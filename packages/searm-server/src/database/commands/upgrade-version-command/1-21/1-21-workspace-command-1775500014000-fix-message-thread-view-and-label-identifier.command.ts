...
@Command({
  ...
  description:
    'Sync the allMessageThreads standard view fields with the current standard definition (adds subject and updatedAt columns) and repoint messageThread.labelIdentifierFieldMetadataId to the subject field. Fixes workspaces upgraded from <1.21 where PR #19351 changes were not applied because the searm-standard application is not re-synced on existing workspaces.',
})
export class FixMessageThreadViewAndLabelIdentifierCommand extends ProvisionedWorkspaceCommandRunner {
  ...
  private computeMessageThreadViewToCreate({
    ...
    standardAllFlatEntityMaps: ReturnType<
      typeof computeSearmStandardApplicationAllFlatEntityMaps
    >['allFlatEntityMaps'];
