...
describe('DatabasePoolMetricsService', () => {
  ...
  it('reports pool connection state for every registered data source', async () => {
    ...
    await expect(
      gaugeCallbacks.get('searm_database_pool_total_connections')?.(),
    ).resolves.toEqual([
      {
        value: 10,
        attributes: {
    ...
    await expect(
      gaugeCallbacks.get('searm_database_pool_idle_connections')?.(),
    ).resolves.toEqual([
      {
        value: 3,
        attributes: {
    ...
    await expect(
      gaugeCallbacks.get('searm_database_pool_waiting_requests')?.(),
    ).resolves.toEqual([
      {
        value: 2,
        attributes: {
    ...
    await expect(
      gaugeCallbacks.get('searm_database_pool_max_connections')?.(),
    ).resolves.toEqual([
      {
        value: 10,
        attributes: {
