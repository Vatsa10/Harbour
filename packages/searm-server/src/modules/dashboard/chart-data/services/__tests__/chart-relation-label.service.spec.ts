...
jest.mock(
  'src/engine/searm-orm/storage/orm-workspace-context.storage',
  () => ({
...
jest.mock(
  'src/engine/searm-orm/utils/resolve-role-permission-config.util',
  () => ({
...
describe('ChartRelationLabelService', () => {
  ...
  it('should preserve the record id through the real formatResult so labels resolve', async () => {
    const actualFormatResult = jest.requireActual(
      'src/engine/searm-orm/utils/format-result.util',
    ).formatResult;
