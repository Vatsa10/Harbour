...
describe('SesOutboundSendingStateHandlerService.handle', () => {
  ...
  // the event, since every searm-managed resource shares the same prefix.
  describe.each([
    {
  ...
  // searm-managed name prefix; the handler must noop in that case.
  it('does not update any workspace when the ARN does not carry the searm-managed prefix', async () => {
    const { service, emailingDomainTenantStatusService } = setUp();
