...
// rotation cron job/service (no SeaRM Enterprise source consulted; written
...
describe('rotate signing keys cron job (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    searmConfigService = getAppProviderByClassName<SearmConfigService>(
      'SearmConfigService',
    );
    ...
    await searmConfigService.set('SIGNING_KEY_ROTATION_DAYS', ROTATION_DAYS);
  });

  afterAll(async () => {
    await searmConfigService.delete('SIGNING_KEY_ROTATION_DAYS');
