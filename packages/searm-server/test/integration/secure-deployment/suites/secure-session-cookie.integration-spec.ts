...
// `nx run searm-server:test:integration:secure`. It exercises the exact
...
describe('session cookie on a production-like secure deployment (integration)', () => {
  ...
  beforeAll(() => {
    if (!IS_SECURE_DEPLOYMENT) {
      throw new Error(
        'This suite requires an https SERVER_URL; run it via nx run searm-server:test:integration:secure',
      );
    }
  });
  ...
  it('should deliver the production cookie: __Host- name, Secure, SameSite=Lax', async () => {
    ...
    expect(
      ...
      '__Host-searm-session=sess_<redacted>; Path=/; Expires=<redacted>; HttpOnly; Secure; SameSite=Lax',
    );
  });
