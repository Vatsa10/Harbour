...
const NavFocusBlurFrontComponent = () => {
  const [interactionCount, setInteractionCount] = useState(0);
  const { entries, pushEvent } = useEventLog();

  return (
    <FrontComponentCard title="nav:focus-blur">
      <nav
        data-testid="subject"
        onFocus={(event) => {
...
export default defineFrontComponent({
  universalIdentifier: 'fc-nav-fb-fb-00000000-0000-0000-0000-000000000020',
  name: 'nav-focus-blur-front-component',
  description: 'Front component covering focus-blur on <nav>',
  component: NavFocusBlurFrontComponent,
});
