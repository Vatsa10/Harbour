...
describe('setWorkerEnvironmentVariables', () => {
  ...
  it('should set process.env on globalThis', () => {
    setWorkerEnvironmentVariables({
      SEARM_APP_ACCESS_TOKEN: 'test-key',
      SEARM_API_URL: 'https://api.example.com',
    });
    ...
    expect(processEnvironment['SEARM_APP_ACCESS_TOKEN']).toBe('test-key');
    expect(processEnvironment['SEARM_API_URL']).toBe(
  ...
  it('should let later calls win when the same variable is set twice', () => {
    setWorkerEnvironmentVariables({
      SEARM_API_URL: 'https://application-provided.example.com',
    });
    setWorkerEnvironmentVariables({
      SEARM_API_URL: 'https://system-provided.example.com',
    });
