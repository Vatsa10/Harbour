...
describe('getUrlFromFetchRequestInput', () => {
  it('should return the string when input is a string', () => {
    expect(getUrlFromFetchRequestInput('https://api.searm.test/graphql')).toBe(
  ...
  it('should return the href when input is a URL instance', () => {
    expect(
      getUrlFromFetchRequestInput(new URL('https://api.searm.test/graphql')),
    ).toBe('https://api.searm.test/graphql');
  });
