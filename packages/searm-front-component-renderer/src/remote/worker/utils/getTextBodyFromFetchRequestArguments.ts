...
const getRequestInputBody = async (
  ...
  if (isDefined(contentType) && !isTextContentType(contentType)) {
    throw new TypeError(
      `The front component fetch bridge only supports text request bodies for SeaRM API requests, got content type: ${contentType}`,
    );
  }
...
export const getTextBodyFromFetchRequestArguments = async (
  ...
  throw new TypeError(
    'The front component fetch bridge only supports string and URLSearchParams request bodies for SeaRM API requests',
  );
};
