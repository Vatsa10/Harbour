...
const writeTempPackageJson = async (
  ...
  const appPath = await mkdtemp(join(tmpdir(), 'searm-sdk-deps-'));
...
describe('validatePackageJsonDependencies', () => {
  ...
  it('should warn when searm-sdk is listed under dependencies', async () => {
    appPath = await writeTempPackageJson({
      dependencies: { 'searm-sdk': '^2.8.0' },
    });

    const warnings = await validatePackageJsonDependencies(appPath);

    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('searm-sdk');
    expect(warnings[0]).toContain('devDependencies');
  });

  it('should warn when searm-client-sdk is listed under dependencies', async () => {
    appPath = await writeTempPackageJson({
      dependencies: { 'searm-client-sdk': '^2.8.0' },
    });

    const warnings = await validatePackageJsonDependencies(appPath);

    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('searm-client-sdk');
    expect(warnings[0]).toContain('devDependencies');
  });

  it('should warn for both SDK packages when both are listed under dependencies', async () => {
    appPath = await writeTempPackageJson({
      dependencies: { 'searm-sdk': '^2.8.0', 'searm-client-sdk': '^2.8.0' },
    });

    const warnings = await validatePackageJsonDependencies(appPath);

    expect(warnings).toHaveLength(2);
    expect(warnings.some((warning) => warning.includes('searm-sdk'))).toBe(
      true,
    );
    expect(
      warnings.some((warning) => warning.includes('searm-client-sdk')),
    ).toBe(true);
  });
