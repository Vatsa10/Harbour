...
// Everything in searm-sdk/define is build-time metadata for the manifest
// extractor and is dead code in the user-app bundle. Stubbing it drops zod,
// searm-shared and ~1MB per logic-function bundle.
//
// Exports are derived from `Object.keys(searmSdkDefine)` so new ones land
...
export const SEARM_SDK_DEFINE_STUBBED_EXPORTS = partitionDefineExports(
  searmSdkDefine as unknown as Record<string, unknown>,
);

const VIRTUAL_NAMESPACE = 'searm-sdk-define-stub';
const STUB_RESOLVED_PATH = '__searm-sdk-define-stub__';

const STUB_PRELUDE = `
// Auto-generated stub for searm-sdk/define injected by the SDK CLI build.
// Real implementations would pull in zod, searm-shared and ~1MB of code; at
...
const buildStubModuleSource = (): string => {
  ...
  for (const name of SEARM_SDK_DEFINE_STUBBED_EXPORTS.any) {
...
export const createStubSearmSdkDefinePlugin = (): esbuild.Plugin => ({
  name: 'searm-sdk-define-stub',
  setup(build) {
    build.onResolve({ filter: /^searm-sdk\/define$/ }, () => ({
