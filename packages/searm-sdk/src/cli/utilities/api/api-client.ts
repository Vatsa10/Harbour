...
export class ApiClient {
  ...
  constructor(options?: {
    ...
    this.client.interceptors.request.use(async (config) => {
      const searmConfig = await this.configService.getConfig();

      config.baseURL = this.serverUrlOverride ?? searmConfig.apiUrl;
    ...
    this.client.interceptors.response.use(
      ...
      async (error) => {
        if (error.response?.status === 401 && error.config) {
          ...
          console.error(
            chalk.red('Cannot connect to SeaRM server. Is it running?'),
          );
        }
        throw error;
      },
  ...
  async refreshToken(): Promise<string | null> {
    ...
    if (
      !config.searmCLIRefreshToken ||
      !config.searmCLIRegistrationClientId
    ) {
      return null;
    }
    ...
    try {
      const tokenResponse = await axios.post(`${config.apiUrl}/oauth/token`, {
        ...
        refresh_token: config.searmCLIRefreshToken,
        client_id: config.searmCLIRegistrationClientId,
      });
      ...
      await this.configService.setConfig({
        searmCLIAccessToken: newAccessToken,
        ...(newRefreshToken ? { searmCLIRefreshToken: newRefreshToken } : {}),
      });
  ...
  async resolveAuthToken(): Promise<string | undefined> {
    ...
    const config = await this.configService.getConfig();
    const cliToken = config.searmCLIAccessToken;
