...
import { type Manifest, OUTPUT_DIR } from 'searm-shared/application';
import { type MetadataValidationErrorResponse } from 'searm-shared/metadata';
import { isPlainObject } from 'searm-shared/utils';
  ...
  if (!validateAuth.serverUp) {
    return {
      ...
      error: {
        code: APP_ERROR_CODES.SYNC_FAILED,
        message:
          'Cannot reach SeaRM server.\n\n' +
          '  Start a local server:\n' +
          '    yarn searm docker:start\n\n' +
          '  Check server status:\n' +
          '    yarn searm docker:status',
      },
    };
  }
    ...
    if (outcome !== 'reauthenticated') {
      return {
        ...
        error: {
          code: APP_ERROR_CODES.SYNC_FAILED,
          message:
            'Authentication failed. Run `yarn searm remote:add` to authenticate.',
        },
      };
    }
  }
