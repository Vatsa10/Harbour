// Authed GET against the SeaRM partners API, cached at the fetch layer (the
...
export async function partnersApiFetch(
  ...
  const baseUrl = process.env.SEARM_PARTNERS_API_URL;
  const apiKey = process.env.SEARM_PARTNERS_API_KEY;
  if (baseUrl === undefined || apiKey === undefined) {
    throw new Error('SEARM_PARTNERS_API_URL / SEARM_PARTNERS_API_KEY unset');
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `SeaRM partners API ${response.status} ${path}: ${body.slice(0, 300)}`,
    );
  }
