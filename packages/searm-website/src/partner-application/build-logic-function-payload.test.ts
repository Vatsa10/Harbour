...
describe('buildLogicFunctionPayload', () => {
  ...
  it('forwards searm experience fields to the webhook payload', () => {
    const payload = buildLogicFunctionPayload(fullValid);
    expect(payload.searmExperience).toEqual(['WORKFLOWS']);
    expect(payload.searmExperienceNotes).toBe(validExperienceNotes);
    expect(payload.searmExperienceProofLink).toBe(
