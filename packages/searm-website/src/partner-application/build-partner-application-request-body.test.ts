...
describe('buildPartnerApplicationRequestBody', () => {
  ...
  it('trims the required string fields', () => {
    ...
    expect(body.searmExperienceNotes).toBe(validExperienceNotes);
    expect(body.searmExperienceProofLink).toBe(
  ...
  it('omits enum/array fields when unset and forwards them when set', () => {
    ...
    const filled = buildPartnerApplicationRequestBody({
      ...
      searmExperience: ['CUSTOM_APPS', 'FRONT_COMPONENTS'],
    });
    ...
    expect(filled.searmExperience).toEqual([
      'CUSTOM_APPS',
      'FRONT_COMPONENTS',
    ]);
  });
