...
// only serve by URL (plus searm.svg, the data layer's static export of
// src/icons/searm-logo.tsx for the mockup's brand-image-by-URL path).
const PUBLIC_SVG_BRAND_FILES = new Set([
  ...
  'public/images/shared/companies/logos/searm.svg',
  ...
  'public/images/shared/halftone/searm-logo.svg',
]);
...
// paths keep resolving once this package takes over the searm-website
...
const ROW_GAP_MULTI_AXIS_FILES = new Set([
  ...
  'sections/why-searm-editorial/Editorial.tsx',
]);
...
function walk(directory) {
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    ...
    // searm-ui's theme is pure data, baked by Linaria at build time — consume
    ...
    // the theme.
    const badSearmUiSubpath = [
      ...content.matchAll(/from 'searm-ui(\/[a-z-]+)?'/g),
    ]
      .map((match) => match[1] ?? '')
      .find(
        (subpath) => subpath !== '/theme' && subpath !== '/theme-constants',
      );
    if (badSearmUiSubpath !== undefined) {
      failures.push(
        `src/${relativePath}: only searm-ui/theme is importable (pure data, baked at build); searm-ui${badSearmUiSubpath} pulls React runtime into the bundle — build a lean primitive instead.`,
      );
    }
