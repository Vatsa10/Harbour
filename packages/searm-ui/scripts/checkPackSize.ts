...
if (unpackedSize > MAX_UNPACKED_BYTES) {
  process.stderr.write(
    `searm-ui unpacked size ${formatMegabytes(unpackedSize)} exceeds budget ${formatMegabytes(
      MAX_UNPACKED_BYTES,
...
process.stdout.write(
  `searm-ui pack OK: tarball ${formatMegabytes(size)}, unpacked ${formatMegabytes(
    unpackedSize,
  )}, ${entryCount} files\n`,
);
